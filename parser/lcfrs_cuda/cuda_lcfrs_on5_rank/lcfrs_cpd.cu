...
__global__ void kernel_forward_d2(float *  head_cd,
                                ...
                                float * alpha_dd,
                                float * alpha_cd,
                                int B,  int L,  int width,   int r,   int r2)
{
      int b_idx = blockIdx.x / (L-width);
      int start = blockIdx.x % (L-width);
      int gap_start_minus_start = (blockIdx.y) / (L-width);
     ...
     result[s_R] = tmp_result + head_cd[b_idx*r*r2 + s_A*r + s_R];
     result2[s_R] = tmp_result + head_dd[b_idx*r*r2 + s_A*r + s_R];
     __syncthreads();
     ...
     if(s_R==0){
        ...
        alpha_cd[b_idx*L*L*L*L*r2 + start*L*L*L*r2 + gap_start*L*L*r2 + gap_end*L*r2 + end*r2 + s_A] = logsumexpf(alpha_cd[b_idx*L*L*L*L*r2 + start*L*L*L*r2 + gap_start*L*L*r2 + gap_end*L*r2 + end*r2 + s_A] ,final_result);
...

{
    for(int w=2; w<L-1; w++){
      ...
      dim3 gridDim11(B*(L-w),  (w-1)*(L-w), max(r2, r4));
      dim3 blockDim11(r3);
