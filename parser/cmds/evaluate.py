...
class Evaluate(CMD):
    ...
    def __call__(self, args, eval_dep=False, decode_type='mbr'):
        ...
        test_loader = dataset.test_dataloader(max_len=40)
        test_loader_autodevice = DataPrefetcher(test_loader, device=self.device)
        if not eval_dep:
            metric_f1, likelihood = self.evaluate(test_loader_autodevice, eval_dep=eval_dep, decode_type=decode_type, save_dir=self.args.load_from_dir)
        else:
