...
class NeuralLCFRS_rankspace(nn.Module):
    def __init__(self, args, dataset):
        ...
        if self.use_char:
            self.term_mlp = CharProbRNN(dataset.char_vocab_size)
        ...
        if not self.args.share_r:
            self.left_c = nn.Sequential( nn.Linear(self.s_dim, tmp), nn.ReLU(), nn.Linear(tmp, self.ratio_c))
            self.right_c = nn.Sequential(nn.Linear(self.s_dim, tmp), nn.ReLU(), nn.Linear(tmp, self.ratio_c))
            self.parent_c = nn.Sequential(nn.Linear(self.s_dim, tmp), nn.ReLU(), nn.Linear(tmp, self.r))
            self.cd_mlp = nn.Sequential( nn.Linear(self.s_dim, tmp), nn.ReLU(), nn.Linear(tmp, self.r-self.ratio_c))
            self.cc_mlp = nn.Sequential( nn.Linear(self.s_dim, tmp), nn.ReLU(), nn.Linear(tmp, self.r-self.ratio_c))

            self.parent_d = nn.Sequential( nn.Linear(self.s_dim, tmp), nn.ReLU(), nn.Linear(tmp, self.r))
            self.left_d = nn.Sequential( nn.Linear(self.s_dim, tmp), nn.ReLU(),nn.Linear(tmp, self.ratio_c))
            self.right_d = nn.Sequential( nn.Linear(self.s_dim, tmp), nn.ReLU(), nn.Linear(tmp, self.ratio_c))
            self.dd_mlp = nn.Sequential( nn.Linear(self.s_dim, tmp), nn.ReLU(), nn.Linear(tmp, self.r-self.ratio_c))
            self.dc_mlp = nn.Sequential( nn.Linear(self.s_dim, tmp), nn.ReLU(), nn.Linear(tmp, (self.r-self.ratio_c)))
            ...
            self.r_emb2 = nn.Parameter(torch.randn(self.s_dim, self.r))

        self.nonterm_emb2 = nn.Parameter(torch.randn((self.NT + self.T) * 4, self.s_dim))

        if not self.sep_emb:
            self.dc_mlp1 =  nn.Sequential(nn.Linear(self.s_dim, tmp), nn.ReLU(), nn.Linear(tmp, (self.r-self.ratio_c)))
            self.dc_mlp2 =  nn.Sequential(nn.Linear(self.s_dim, tmp), nn.ReLU(), nn.Linear(tmp, (self.r-self.ratio_c)))
            self.dc_mlp3 =  nn.Sequential(nn.Linear(self.s_dim, tmp), nn.ReLU(), nn.Linear(tmp, (self.r-self.ratio_c)))
            self.dc_mlp4 =  nn.Sequential(nn.Linear(self.s_dim, tmp), nn.ReLU(), nn.Linear(tmp, (self.r-self.ratio_c)))
    ...
    def forward(self, input, evaluating=False):
        ...
        def terms():
            if self.use_char:
                term_prob = self.term_mlp(input['char'],self.term_emb)
                return term_prob.contiguous()
        ...
        def rules():
            ...
            if not self.share_r:
                ...
                cd = (self.cd_mlp(d_emb)).log_softmax(0)
                head_d = self.parent_d(d_emb).log_softmax(-1)
                ...
                dd = (self.dd_mlp(d_emb)).log_softmax(0)
                # dd = cd
            else:
                head = (self.parent_c(self.nonterm_emb[:self.NT]) @ self.r_emb).log_softmax(-1)
                head_c1 = head[:, :self.ratio_c].unsqueeze(0).repeat(b, 1, 1).contiguous()
                head_c2 = head[:, self.ratio_c:].unsqueeze(0).repeat(b, 1, 1).contiguous()
                left_c = (self.left_c(self.nonterm_emb) @ self.r_emb[:, :self.ratio_c]).log_softmax(0).unsqueeze(
                    0).repeat(b, 1, 1).contiguous()
                right_c = (self.right_c(self.nonterm_emb) @ self.r_emb[:, :self.ratio_c]).log_softmax(0).unsqueeze(
                    0).repeat(b, 1, 1).contiguous()

                cc = (self.cc_mlp(self.nonterm_emb) @ self.r_emb[:, self.ratio_c:]).log_softmax(0).unsqueeze(0).repeat(
                    b, 1, 1).contiguous()
                cd = (self.cd_mlp(self.d_emb) @ self.r_emb[:, self.ratio_c:]).log_softmax(0)
                head_d = (self.parent_d(self.d_emb) @ self.r_emb).log_softmax(-1)
                head_d1 = head_d[:, :self.ratio_c].contiguous()
                head_d2 = head_d[:, self.ratio_c:].contiguous()
                dc = (self.dc_mlp(self.nonterm_emb2) @ self.r_emb[:, self.ratio_c:]).log_softmax(0).unsqueeze(
                    0).repeat(b, 1, 1).contiguous()
                dd = (self.dd_mlp(self.d_emb) @ self.r_emb[:, self.ratio_c:]).log_softmax(0)
