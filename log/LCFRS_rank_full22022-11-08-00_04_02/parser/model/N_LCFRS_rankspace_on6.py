...
from ..pcfgs.lcfrs_on6_cpd_rankspace import PCFG
...
class NeuralLCFRS_rankspace_on6(nn.Module):
    def __init__(self, args, dataset):
        super(NeuralLCFRS_rankspace_on6, self).__init__()
        self.pcfg = PCFG()
        ...
        if not self.args.share_r:
            ...
            self.parent_d3 = nn.Sequential(nn.Linear(self.s_dim, tmp), self.activate)
            ...
            self.ill_in = nn.Sequential(nn.Linear(self.s_dim, tmp),self.activate)
            self.ill_out = nn.Sequential(nn.Linear(self.s_dim, tmp),self.activate)
    ...
    def forward(self, input, evaluating=False):
        ...
        if not self.share_r:
            ...
            left_d = softmax(self.left_c(self.nonterm_emb), r3_emb, 0)
            right_d = softmax(self.right_c(self.nonterm_emb), r3_emb, 0)

        if not self.share_r:
            head_d = torch.cat([self.parent_d(d_emb), self.parent_d2(d_emb)], -1)

        else:
            head_d = torch.cat([self.parent_d(d_emb) @ self.r3_emb,
                                self.parent_d2(d_emb) @ self.r4_emb,
                                self.parent_d3(d_emb) @ self.r5_emb,
                                ], -1)
        ...
        if not self.args.share_r:
            ...
            if not self.sep_emb:
                ...
                if self.args.merge:
                    ...
                    dc = softmax(self.dc_mlp(self.nonterm_emb4), r4_emb, 0)
                    ...
                    ill_in = softmax(self.ill_in(self.d_emb), self.r5_emb, 0)
                    ill_out = softmax(self.ill_out(self.d_emb), self.r5_emb, 0)
        ...
        head_d2 = head_d[:, self.r3:self.r3+self.r4].contiguous()
        head_d3 = head_d[:, self.r3+self.r4:].contiguous()

        # dd = cd
        head_cd1 = (torch.einsum('xi, xj -> ji', head_d1, cd) + 1e-9).log()
        head_cd2 = (torch.einsum('xi, xj -> ji', head_d2, cd) + 1e-9).log()
        head_cd3 = (torch.einsum('xi, xj -> ji', head_d3, cd) + 1e-9).log()
        head_cd = torch.cat([head_cd1, head_cd2, head_cd3], -1).unsqueeze(0).repeat(b, 1, 1).contiguous()

        head_dd1 = (torch.einsum('xi, xj -> ji', head_d1, dd) + 1e-9).log()
        head_dd2 = (torch.einsum('xi, xj -> ji', head_d2, dd) + 1e-9).log()
        head_dd3 = (torch.einsum('xi, xj -> ji', head_d3, dd) + 1e-9).log()
        head_dd = torch.cat([head_dd1, head_dd2, head_dd3], -1).unsqueeze(0).repeat(b, 1, 1).contiguous()

        head_ill_in1 = (torch.einsum('xi, xj -> ji', head_d1, ill_in) + 1e-9).log()
        head_ill_in2 = (torch.einsum('xi, xj -> ji', head_d2, ill_in) + 1e-9).log()
        head_ill_in3 = (torch.einsum('xi, xj -> ji', head_d3, ill_in) + 1e-9).log()

        head_ill_in = torch.cat([head_ill_in1, head_ill_in2, head_ill_in3], -1).unsqueeze(0).repeat(b, 1, 1).contiguous()


        head_ill_out1 = (torch.einsum('xi, xj -> ji', head_d1, ill_out) + 1e-9).log()
        head_ill_out2 = (torch.einsum('xi, xj -> ji', head_d2, ill_out) + 1e-9).log()
        head_ill_out3 = (torch.einsum('xi, xj -> ji', head_d3, ill_out) + 1e-9).log()
        head_ill_out = torch.cat([head_ill_out1, head_ill_out2, head_ill_out3], -1).unsqueeze(0).repeat(b, 1, 1).contiguous()
        ...
        # assert ~torch.isnan(root_c).any()
            ...
            #
        return {'head_dd': head_dd,
                'head_cd': head_cd,
                'head_ill_in': head_ill_in,
                'head_ill_out': head_ill_out,
