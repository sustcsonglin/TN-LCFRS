...
from ..pcfgs.lcfrs_on3_cuda import LCFRS_ON3


class CompoundLCFRS_on3(nn.Module):
    def __init__(self, args, dataset):
        super(CompoundLCFRS_on3, self).__init__()
        ...
        self.rule_mlp2 = nn.Linear(input_dim, self.NT_T*self.NT_T)
        self._initialize()
    ...
    def forward(self, input, evaluating=False):
        ...
        def rules():
            ...
            binary_dc = rule_prob2.reshape(b,self.D, self.NT_T, self.NT_T).expand(b, self.D, self.NT_T, self.NT_T).contiguous()
            return binary, binary_close, binary_dc

        (binary, binary_close, binary_dc), unary, root = rules(), terms(), roots()
