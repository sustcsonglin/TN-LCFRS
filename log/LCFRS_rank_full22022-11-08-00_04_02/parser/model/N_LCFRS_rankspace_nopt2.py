...
class NeuralLCFRS_rankspace_nopt(nn.Module):
    def __init__(self, args, dataset):
        ...
        self.split_mlp_c = nn.Sequential(
                                      nn.Linear(self.s_dim, self.s_dim),
                                      ResLayer(self.s_dim, self.s_dim),
                                      ResLayer(self.s_dim, self.s_dim),
                                      nn.Linear(self.s_dim, 3))

        self.split_mlp_d = nn.Sequential(
                                      nn.Linear(self.s_dim, self.s_dim),
                                      ResLayer(self.s_dim, self.s_dim),
                                      ResLayer(self.s_dim, self.s_dim),
        ...
        tmp = self.s_dim
        self.left_c = nn.Sequential(nn.Dropout(0.15), nn.Linear(self.s_dim, tmp), nn.ReLU(), nn.Linear(tmp, self.ratio_c))
        self.right_c = nn.Sequential(nn.Dropout(0.15), nn.Linear(self.s_dim, tmp), nn.ReLU(), nn.Linear(tmp, self.ratio_c))
        self.parent_c = nn.Sequential(nn.Dropout(0.15),nn.Linear(self.s_dim, tmp), nn.ReLU(), nn.Linear(tmp, self.r))
        self.cd_mlp = nn.Sequential(nn.Dropout(0.15), nn.Linear(self.s_dim, tmp), nn.ReLU(), nn.Linear(tmp, self.r-self.ratio_c))
        self.cc_mlp = nn.Sequential(nn.Dropout(0.15), nn.Linear(self.s_dim, tmp), nn.ReLU(), nn.Linear(tmp, self.r-self.ratio_c))

        self.parent_d = nn.Sequential(nn.Dropout(0.15), nn.Linear(self.s_dim, tmp), nn.ReLU(), nn.Linear(tmp, self.r))
        self.left_d = nn.Sequential(nn.Dropout(0.15), nn.Linear(self.s_dim, tmp), nn.ReLU(),nn.Linear(tmp, self.ratio_c))
        self.right_d = nn.Sequential(nn.Dropout(0.15), nn.Linear(self.s_dim, tmp), nn.ReLU(), nn.Linear(tmp, self.ratio_c))
        self.dd_mlp = nn.Sequential(nn.Dropout(0.15), nn.Linear(self.s_dim, tmp), nn.ReLU(), nn.Linear(tmp, self.r-self.ratio_c))
        self.dc_mlp = nn.Sequential(nn.Dropout(0.15), nn.Linear(self.s_dim, tmp), nn.ReLU(), nn.Linear(tmp, (self.r-self.ratio_c)))
    ...
    def forward(self, input, evaluating=False):
        ...
        (unary, head_c1, head_c2, left_c, right_c, left_d, right_d, cc, dc, head_cd1, head_cd2, head_dd1, head_dd2),  root = rules(),  roots()
