...
from ..pcfgs.lcfrs_on6_cpd import PCFG

class NeuralLCFRS_on6(nn.Module):
    def __init__(self, args, dataset):
        super(NeuralLCFRS_on6, self).__init__()
        self.pcfg = PCFG()
        ...
        self.parent_d = nn.Sequential(nn.Linear(self.s_dim, self.s_dim), nn.ReLU(), nn.Linear(self.s_dim, self.r+self.r-self.ratio_c))
        ...
        self.dc_mlp = nn.Sequential(nn.Linear(self.s_dim, self.s_dim), nn.ReLU(), nn.Linear(self.s_dim, 4*(self.r-self.ratio_c)))

        self.ddl_mlp = nn.Sequential(nn.Linear(self.s_dim, self.s_dim), nn.ReLU(), nn.Linear(self.s_dim, (self.r-self.ratio_c)))
        self.ddr_mlp = nn.Sequential(nn.Linear(self.s_dim, self.s_dim), nn.ReLU(), nn.Linear(self.s_dim, (self.r-self.ratio_c)))
    ...
    def forward(self, input, evaluating=False):
        ...
        def rules():
            ...
            cd = (self.cd_mlp(self.nonterm_emb[:self.NT]) ).log_softmax(0).unsqueeze(0).repeat(b, 1, 1).contiguous()
            head_d = (self.parent_d(self.d_emb)).log_softmax(-1)
            head_d1 = head_d[:, :self.ratio_d].unsqueeze(0).repeat(b, 1, 1).contiguous()
            head_d2 = head_d[:, self.ratio_d:self.ratio_d+10].unsqueeze(0).repeat(b, 1, 1).contiguous()
            head_d3 = head_d[:, self.ratio_d+10:].unsqueeze(0).repeat(b, 1, 1).contiguous()
            # dc = (self.dc_mlp(self.nonterm_emb2) ).log_softmax(0).unsqueeze(0).repeat(b,1,1).contiguous()
            dc = (self.dc_mlp(self.nonterm_emb).reshape(self.NT_T, 4, -1).reshape(self.NT_T*4, -1)).log_softmax(0).unsqueeze(0).repeat(b,1,1).contiguous()
            dd = (self.dd_mlp(self.d_emb)).log_softmax(0).unsqueeze(0).repeat(b, 1, 1).contiguous()
            ddl = (self.ddl_mlp(self.d_emb)).log_softmax(0).unsqueeze(0).repeat(b, 1, 1).contiguous()
            ddr = (self.ddr_mlp(self.d_emb)).log_softmax(0).unsqueeze(0).repeat(b, 1, 1).contiguous()

            return head_c1, head_c2, head_d1, head_d2, head_d3, left_c, right_c,  \
                    left_d, right_d, cc, cd,dc, dd, ddl, ddr

        (head_c1, head_c2, head_d1, head_d2, head_d3, left_c, right_c,  \
                    left_d, right_d, cc, cd,dc, dd, ddl, ddr), unary, root = rules(), terms(), roots()
