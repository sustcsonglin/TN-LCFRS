...
from ..pcfgs.lcfrs_on5_cpd_v2 import PCFG

class NeuralLCFRS_v2(nn.Module):
    def __init__(self, args, dataset):
        super(NeuralLCFRS_v2, self).__init__()
        self.pcfg = PCFG()
        ...
        self.left_c = nn.Sequential(nn.Linear(self.s_dim, self.s_dim), nn.ReLU(),nn.Linear(self.s_dim, self.s_dim))
        self.right_c = nn.Sequential(nn.Linear(self.s_dim, self.s_dim),nn.ReLU(),nn.Linear(self.s_dim, self.s_dim))
        self.parent_c = nn.Sequential(nn.Linear(self.s_dim, self.s_dim), nn.ReLU(),nn.Linear(self.s_dim, self.s_dim))
        self.parent_c2 = nn.Sequential(nn.Linear(self.s_dim, self.s_dim),nn.ReLU(),nn.Linear(self.s_dim, self.s_dim))
        self.cd_mlp = nn.Sequential(nn.Linear(self.s_dim, self.s_dim),nn.ReLU(),nn.Linear(self.s_dim, self.s_dim))
        self.cc_mlp = nn.Sequential(nn.Linear(self.s_dim, self.s_dim),nn.ReLU(),nn.Linear(self.s_dim, self.s_dim))

        self.parent_d = nn.Sequential(nn.Linear(self.s_dim, self.s_dim),nn.ReLU(),nn.Linear(self.s_dim, self.s_dim))
        self.left_d = nn.Sequential(nn.Linear(self.s_dim, self.s_dim),nn.ReLU(),nn.Linear(self.s_dim, self.s_dim))
        self.right_d = nn.Sequential(nn.Linear(self.s_dim, self.s_dim),nn.ReLU(),nn.Linear(self.s_dim, self.s_dim))
        self.dd_mlp = nn.Sequential(nn.Linear(self.s_dim, self.s_dim),nn.ReLU(),nn.Linear(self.s_dim, self.s_dim))
        self.dc_mlp = nn.Sequential(nn.Linear(self.s_dim, self.s_dim),nn.ReLU(),nn.Linear(self.s_dim, self.s_dim))
    ...
    def forward(self, input, evaluating=False):
        ...
        def rules():
            ...
            dc = (self.dc_mlp(self.nonterm_emb) @ self.rd_emb[:, self.ratio_d:]).log_softmax(0).unsqueeze(0).repeat(b,1,1).contiguous()
            dd = (self.dd_mlp(self.d_emb2) @ self.rd_emb[:, self.ratio_d:]).log_softmax(0).unsqueeze(0).repeat(b,1,1).contiguous()
