...
from ..pcfgs.lcfrs_on5_ill import PCFG


class CompoundLCFRS_ill(nn.Module):
    def __init__(self, args, dataset):
        super(CompoundLCFRS_ill, self).__init__()
        ...
        self.rule_mlp = nn.Linear(input_dim, (self.NT_T) * self.NT_T + self.NT_T * self.D + self.D * self.D)
    ...
    def forward(self, input, evaluating=False):
        ...
        def rules():
            ...
            binary = rule_prob[:, :, :self.NT_T*self.NT_T].reshape(b,self.NT, self.NT_T, self.NT_T).contiguous()
            binary_close = rule_prob[:, :, self.NT_T*self.NT_T: self.NT_T*self.NT_T+self.D*self.NT_T].reshape(b,self.NT, self.D, self.NT_T).contiguous()
            binary_ill = rule_prob[:, :, self.NT_T*self.NT_T+self.D*self.NT_T:].reshape(b,self.NT, self.D, self.D).contiguous()

            rule_prob2 = self.rule_mlp2(d_emb).log_softmax(-1)
            binary_dc = rule_prob2[:, :, :self.NT_T*self.NT_T].reshape(b,self.D, self.NT_T, self.NT_T).contiguous()
            binary_d = rule_prob2[:, :, self.NT_T*self.NT_T:].reshape(b,self.D, self.D, self.NT_T, 4).contiguous()
            return binary, binary_close, binary_dc, binary_d, binary_ill

        (binary, binary_close, binary_dc, binary_d, binary_ill), unary, root = rules(), terms(), roots()
