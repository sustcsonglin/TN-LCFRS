...
from ..pcfgs.lcfrs_on5_cpd_v3 import PCFG

class NeuralLCFRS_v3(nn.Module):
    def __init__(self, args, dataset):
        super(NeuralLCFRS_v3, self).__init__()
        self.pcfg = PCFG()
        ...
        self.ratio_c = int(0.92 * self.r)

        # assert self.r2 % 5 == 0
        self.ratio_d = int(0.92 * self.r)
        ...
        self.left_d = nn.Sequential(nn.Linear(self.s_dim, self.s_dim), nn.ReLU(), nn.Linear(self.s_dim, self.ratio_d))
        self.right_d = nn.Sequential(nn.Linear(self.s_dim, self.s_dim), nn.ReLU(), nn.Linear(self.s_dim, self.ratio_d))
        self.dd_mlp = nn.Sequential(nn.Linear(self.s_dim, self.s_dim), nn.ReLU(), nn.Linear(self.s_dim, self.r - self.ratio_d))
        self.dc_mlp = nn.Sequential(nn.Linear(self.s_dim, self.s_dim), nn.ReLU(), nn.Linear(self.s_dim, self.r - self.ratio_d))
    ...
    def forward(self, input, evaluating=False):
        ...
        def rules():
            ...
            head_d2 = head_d[:, self.ratio_d:].reshape(self.D, 4, -1).unsqueeze(0).repeat(b,1,1,1).contiguous()
            ...
            dc = self.dc_mlp(self.nonterm_emb).reshape(self.NT_T, 4, -1).log_softmax(0).unsqueeze(0).repeat(b,1,1, 1).contiguous()
            dd = self.dd_mlp(self.nonterm_emb[:self.NT]).reshape(self.D, 4, -1).log_softmax(0).unsqueeze(0).repeat(b,1,1,1).contiguous()

            assert dd.shape[-1] == head_d2.shape[-1] == dc.shape[-1]
