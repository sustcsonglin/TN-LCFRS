...
from ..pcfgs.lcfrs_on3_cpd import PCFG

class NeuralLCFRS_on3(nn.Module):
    def __init__(self, args, dataset):
        super(NeuralLCFRS_on3, self).__init__()
        self.pcfg = PCFG()
        ...
        self.parent_c = nn.Sequential(nn.Linear(self.s_dim, self.r1+self.r3))
    ...
    def forward(self, input, evaluating=False):
        ...
        def rules():
            head_c = (self.parent_c(self.nonterm_emb2)).log_softmax(-1)
            head_c1 = (head_c[:, :self.r1]).unsqueeze(0).repeat(b,1,1).contiguous()
            head_c2 = (head_c[:, self.r1:]).unsqueeze(0).repeat(b,1,1).contiguous()
            ...
            head_d1 = (self.parent_d(self.d_emb)).log_softmax(-1).unsqueeze(0).repeat(b, 1, 1).contiguous()
            ...
            return head_c1, head_c2, left_c, right_c, cc, cd, \
                   head_d1, left_d, right_d

        (head_c1, head_c2, left_c, right_c, cc, cd, head_d1,  left_d, right_d), unary, root = rules(), terms(), roots()
