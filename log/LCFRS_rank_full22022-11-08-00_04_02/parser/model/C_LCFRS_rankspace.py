...
class CompoundLCFRS_rankspace(nn.Module):
    def __init__(self, args, dataset):
        super(CompoundLCFRS_rankspace, self).__init__()
        self.pcfg = PCFG()
    ...
    def forward(self, input, evaluating=False):
        ...
        def terms():
            term_emb = self.nonterm_emb[self.NT:]
            term_emb = term_emb.unsqueeze(0).expand(
                b, self.T, self.s_dim
            )
