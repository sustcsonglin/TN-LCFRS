...
template <typename F>
__global__ void kernel_forward_d1( F *__restrict__  head_cd,
                                   F *__restrict__ head_dd,
                                   ...
                                   int B,  int L,   int width,   int d,   int r)
{
    ...
    result2[s_R] = tmp + head_dd[b_idx*d*r*4 + s_R*d*4 + s_A];
    result3[s_R] = tmp + head_dd[b_idx*d*r*4 + s_R*d*4 + d + s_A];
    result4[s_R] = tmp + head_dd[b_idx*d*r*4 + s_R*d*4 + 2*d + s_A];
    result5[s_R] = tmp + head_dd[b_idx*d*r*4 + s_R*d*4 + 3*d + s_A];
    __syncthreads();
    ...
    for(int i=0;i<r;i++){
        ...
        final_result5 = logsumexp(final_result5, result5[i]);
    }
    alpha_cd[b_idx*L*L*L*L*d + start*L*L*L*d + gap_start*L*L*d + gap_end*L*d + end*d + s_A] = final_result;
    alpha_dd[b_idx*L*L*L*L*d*4 + start*L*L*L*d*4 + gap_start*L*L*d*4 + gap_end*L*d*4 + end*d*4 + s_A] = final_result2;
    alpha_dd[b_idx*L*L*L*L*d*4 + start*L*L*L*d*4 + gap_start*L*L*d*4 + gap_end*L*d*4 + end*d*4 + d + s_A] = final_result3;
    alpha_dd[b_idx*L*L*L*L*d*4 + start*L*L*L*d*4 + gap_start*L*L*d*4 + gap_end*L*d*4 + end*d*4 + 2*d + s_A] = final_result4;
    alpha_dd[b_idx*L*L*L*L*d*4 + start*L*L*L*d*4 + gap_start*L*L*d*4 + gap_end*L*d*4 + end*d*4 + 3*d + s_A] = final_result5;
    }
}
...
template <typename F>
__global__ void kernel_backward_d1(  F *__restrict__   head_cd, F *__restrict__   head_cd_grd,
                                 F *__restrict__   head_dd, F *__restrict__ head_dd_grd,
                               F *__restrict__ alpha_ld, F *__restrict__ alpha_rd,F *__restrict__  alpha_cd,F *__restrict__  alpha_dd,
                                int B,   int L,   int width,   int d,   int r)
{
      int b_idx = blockIdx.x / (L-width);
      int start = blockIdx.x % (L-width);
     ...
     F *__restrict__   s_l = alpha_ld + b_idx * L * L * r;
     F *__restrict__   s_r = alpha_rd + b_idx * L * L * r;
     ...
     parent_inside = alpha_dd[b_idx*L*L*L*L*d*4 + start*L*L*L*d*4 + gap_start*L*L*d*4 + gap_end*L*d*4 + end*d*4 + s_A];
     parent_grd = alpha_dd[b_idx*L*L*L*L*d*4 + gap_start*L*L*L*d*4 + start*L*L*d*4 + gap_end*L*d*4 + end*d*4 + s_A];
     float tmp_grd2 = exp(tmp + head_dd[b_idx*d*r*4 + s_R*d*4 + s_A] - parent_inside)*parent_grd;

     parent_inside = alpha_dd[b_idx*L*L*L*L*d*4 + start*L*L*L*d*4 + gap_start*L*L*d*4 + gap_end*L*d*4 + end*d*4 + d + s_A];
     parent_grd = alpha_dd[b_idx*L*L*L*L*d*4 + gap_start*L*L*L*d*4 + start*L*L*d*4 + gap_end*L*d*4 + end*d*4 + d + s_A];
     float tmp_grd3 = exp(tmp + head_dd[b_idx*d*r*4 + s_R*d*4 + d + s_A] - parent_inside)*parent_grd;

     parent_inside = alpha_dd[b_idx*L*L*L*L*d*4 + start*L*L*L*d*4 + gap_start*L*L*d*4 + gap_end*L*d*4 + end*d*4 + 2*d + s_A];
     parent_grd = alpha_dd[b_idx*L*L*L*L*d*4 + gap_start*L*L*L*d*4 + start*L*L*d*4 + gap_end*L*d*4 + end*d*4 + 2*d + s_A];
     float tmp_grd4 = exp(tmp + head_dd[b_idx*d*r*4 + s_R*d*4 + 2*d + s_A] - parent_inside)*parent_grd;

     parent_inside = alpha_dd[b_idx*L*L*L*L*d*4 + start*L*L*L*d*4 + gap_start*L*L*d*4 + gap_end*L*d*4 + end*d*4 + 2*d + s_A];
     parent_grd = alpha_dd[b_idx*L*L*L*L*d*4 + gap_start*L*L*L*d*4 + start*L*L*d*4 + gap_end*L*d*4 + end*d*4 + 2*d + s_A];
     float tmp_grd4 = exp(tmp + head_dd[b_idx*d*r*4 + s_R*d*4 + 2*d + s_A] - parent_inside)*parent_grd;

     atomicAdd(s_l + gap_start*L*r + start*r + s_R, tmp_grd+tmp_grd2+tmp_grd3+tmp_grd4);
     atomicAdd(s_r + end*L*r + gap_end*r + s_R, tmp_grd+tmp_grd2+tmp_grd3+tmp_grd4);
     atomicAdd(head_dd_grd + b_idx*d*r*4 + s_R*d*4 + s_A, tmp_grd2);
     atomicAdd(head_dd_grd + b_idx*d*r*4 + s_R*d*4 + d + s_A, tmp_grd3);
     atomicAdd(head_dd_grd + b_idx*d*r*4 + s_R*d*4 + 2*d + s_A, tmp_grd4);
     atomicAdd(head_dd_grd + b_idx*d*r*4 + s_R*d*4 + 3*d + s_A, tmp_grd5);
}


template <typename F>
__global__ void kernel_forward_d2(F *__restrict__  head_cd,
                                F *__restrict__  head_dd,
                                ...
                                int B,  int L,  int width,   int d,   int r)
{
    ...
    __shared__ float result3[200];
    __shared__ float result4[200];
    __syncthreads();
    for(int split=start+1; split< gap_start; split++)
    {
        tmp_result = logsumexp(tmp_result, s_d[split*L3*4 + gap_start*L2*4 + gap_end*L1*4 + end*r*4 + 0*r + s_R] + s[start*L*r + split*r + s_R]);
        tmp_result = logsumexp(tmp_result, s_d[start*L3*4 + split*L2*4 + gap_end*L1*4 + end*r*4 + 1*r + s_R] + s[split*L*r + gap_start*r + s_R]);
    }

    for(int split = gap_end+1; split <end; split++){
        tmp_result = logsumexp(tmp_result, s_d[start*L3*4 + gap_start*L2*4 + split*L1*4 + end*r*4 + 2*r + s_R] + s[gap_end*L*r + split*r + s_R]);
        tmp_result = logsumexp(tmp_result, s_d[start*L3*4 + gap_start*L2*4 + gap_end*L1*4 + split*r*4 + 3*r + s_R] + s[split*L*r + end*r + s_R]);
    }

    result[s_R] = tmp_result + head_cd[b_idx*d*r + s_R*r + s_A];
    result2[s_R] = tmp_result + head_dd[b_idx*d*r*4 + s_R*r*4 + s_A];
    result3[s_R] = tmp_result + head_dd[b_idx*d*r*4 + s_R*r*4 + r + s_A];
    result4[s_R] = tmp_result + head_dd[b_idx*d*r*4 + s_R*r*4 + 2*r + s_A];
    result5[s_R] = tmp_result + head_dd[b_idx*d*r*4 + s_R*r*4 + 3*r + s_A];
    __syncthreads();
    ...
    for(int i=0;i<r;i++){
        ...
        final_result3 = logsumexp(final_result3, result3[i]);
        final_result4 = logsumexp(final_result4, result4[i]);
        final_result5 = logsumexp(final_result4, result5[i]);
    }
    alpha_cd[b_idx*L*L*L*L*d + start*L*L*L*d + gap_start*L*L*d + gap_end*L*d + end*d + s_A] = logsumexp(alpha_cd[b_idx*L*L*L*L*d + start*L*L*L*d + gap_start*L*L*d + gap_end*L*d + end*d + s_A],final_result);
    alpha_dd[b_idx*L*L*L*L*d*4 + start*L*L*L*d*4 + gap_start*L*L*d*4 + gap_end*L*d*4 + end*d*4 + s_A] = logsumexp(  alpha_dd[b_idx*L*L*L*L*d*4 + start*L*L*L*d*4 + gap_start*L*L*d*4 + gap_end*L*d*4 + end*d*4 + s_A],final_result2);
    alpha_dd[b_idx*L*L*L*L*d*4 + start*L*L*L*d*4 + gap_start*L*L*d*4 + gap_end*L*d*4 + end*d*4 + 1*r + s_A] = logsumexp(  alpha_dd[b_idx*L*L*L*L*d*4 + start*L*L*L*d*4 + gap_start*L*L*d*4 + gap_end*L*d*4 + end*d*4 + 1*r + s_A],final_result3);
    alpha_dd[b_idx*L*L*L*L*d*4 + start*L*L*L*d*4 + gap_start*L*L*d*4 + gap_end*L*d*4 + end*d*4 + 2*r + s_A] = logsumexp(  alpha_dd[b_idx*L*L*L*L*d*4 + start*L*L*L*d*4 + gap_start*L*L*d*4 + gap_end*L*d*4 + end*d*4 + 2*r + s_A],final_result4);
    alpha_dd[b_idx*L*L*L*L*d*4 + start*L*L*L*d*4 + gap_start*L*L*d*4 + gap_end*L*d*4 + end*d*4 + 3*r + s_A] = logsumexp(  alpha_dd[b_idx*L*L*L*L*d*4 + start*L*L*L*d*4 + gap_start*L*L*d*4 + gap_end*L*d*4 + end*d*4 + 3*r + s_A],final_result5);
   }
}

template <typename F>
__global__ void kernel_backward_d2(F *__restrict__  head_cd, F *__restrict__  head_cd_grd,
                                F *__restrict__  head_dd, F *__restrict__  head_dd_grd,
                                F *__restrict__ alpha_dc,
                                F *__restrict__ alpha_dd,
                                F *__restrict__ alpha_cd,
                                int B,  int L,  int width,   int d,   int r)
{
    ...
    float rule_score2 = head_dd[b_idx*d*r*4 + s_R*d*4 + s_A];
    float rule_score3 = head_dd[b_idx*d*r*4 + s_R*d*4 + d + s_A];
    float rule_score4 = head_dd[b_idx*d*r*4 + s_R*d*4 + 2*d + s_A];
    float rule_score5 = head_dd[b_idx*d*r*4 + s_R*d*4 + 3*d + s_A];
    ...
    float parent_inside2 = alpha_dd[b_idx*L*L*L*L*d*4 + start*L3*4 + gap_start*L2*4 + gap_end*L1*4 + end*d*4 + s_A];
    float parent_grd2 = alpha_dd[b_idx*L*L*L*L*d*4 + gap_start*L3*4 + start*L2*4 + gap_end*L1*4 + end*d*4 + s_A];

    float parent_inside3 = alpha_dd[b_idx*L*L*L*L*d*4 + start*L3*4 + gap_start*L2*4 + gap_end*L1*4 + end*d*4 + d + s_A];
    float parent_grd3 = alpha_dd[b_idx*L*L*L*L*d*4 + gap_start*L3*4 + start*L2*4 + gap_end*L1*4 + end*d*4 + d +s_A];

    float parent_inside4 = alpha_dd[b_idx*L*L*L*L*d*4 + start*L3*4 + gap_start*L2*4 + gap_end*L1*4 + end*d*4 + 2*d + s_A];
    float parent_grd4 = alpha_dd[b_idx*L*L*L*L*d*4 + gap_start*L3*4 + start*L2*4 + gap_end*L1*4 + end*d*4 + 2*d + s_A];

    float parent_inside5 = alpha_dd[b_idx*L*L*L*L*d*4 + start*L3*4 + gap_start*L2*4 + gap_end*L1*4 + end*d*4 + 3*d + s_A];
    float parent_grd5 = alpha_dd[b_idx*L*L*L*L*d*4 + gap_start*L3*4 + start*L2*4 + gap_end*L1*4 + end*d*4 + 3*d + s_A];
    ...
    for(int split=start+1; split< gap_start; split++)
    {
        float tmp = s_d[split*L3 * 4 + gap_start*L2 *4 + gap_end*L1*4 + end*r*4 + s_R] + s[start*L*r + split*r + s_R];
                   ...
                   int B, int L, int m, int p, int d, int r1, int r2, int r3, int r4)
{
    ...
    kernel_forward_cpr<<<gridDim13, blockDim>>>(dc, alpha_dc, unary, B, L, m, p, r4);
    ...
    for(int w=2; w<L; w++){
      ...
      if(w<L-1){
        ...
        dim3 blockDim9(m);
        kernel_forward_cmr<<<gridDim9, blockDim9>>>(dc, alpha, alpha_dc, B, L, w, m, p, r4);
                   ...
                   int B, int L, int m, int p, int d, int r1, int r2, int r3, int r4)
{
    ...
    dim3 gridDim13(B, L-1, r4);
    kernel_backward_cpr<<<gridDim13, blockDim>>>(dc, dc_grd, alpha_dc, unary, unary_grd, B, L, m, p, r4);
