...
template <typename F>
__global__ void kernel_forward( F *__restrict__   head_l,
                                F *__restrict__   head_r,
                                F *__restrict__   head_cc,
                                F *__restrict__   head_dc,
                                F *__restrict__ alpha_l,
                                F *__restrict__ alpha_r,
                                F *__restrict__ alpha_cc,
                                F *__restrict__ alpha_dc,
                                int B,   int L,   int width,   int m,   int r, int r2)
{
    ...
    {
        tmp_result = logsumexp(tmp_result, s_l[start*L*r + split * r + s_R] + s_r[split*L*r + end*r + s_R]);
    }

    result[s_R] = tmp_result + head_l[b_idx*m*r + s_A*r + s_R];
    result2[s_R] = tmp_result + head_r[b_dix*m*r + s_A*r + s_R];
    if(s_R <
    __syncthreads();
