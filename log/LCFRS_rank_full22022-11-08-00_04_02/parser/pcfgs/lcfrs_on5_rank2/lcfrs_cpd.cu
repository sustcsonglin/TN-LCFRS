                                   ...
                                   int B,  int L,   int width,   int r,   int r1, int r2)
{
    ...
    if(s_R==0){
        ...
        if(s_A < r2){
            alpha_dd[b_idx*L*L*L*L*r2*4 + start*L*L*L*r2*4 + gap_start*L*L*r2*4 + gap_end*L*r2*4 + end*r2*4 + s_A] = result2[0];
            alpha_dd[b_idx*L*L*L*L*r2*4 + start*L*L*L*r2*4 + gap_start*L*L*r2*4 + gap_end*L*r2*4 + end*r2*4 + r2 + s_A] = result3[0];
                                  ...
                                  int B,   int L,   int width,   int r,  int r1, int r2)
{
     ...
     if(s_A < r2){
        parent_inside = alpha_dd[b_idx*L*L*L*L*r2*4 + start*L*L*L*r2*4 + gap_start*L*L*r2*4 + gap_end*L*r2*4 + end*r2*4 + s_A];
        parent_grd = alpha_dd[b_idx*L*L*L*L*r2*4 + gap_start*L*L*L*r2*4 + start*L*L*r2*4 + gap_end*L*r2*4 + end*r2*4 + s_A];
        float tmp_grd2 = exp(tmp + head_dd[b_idx*r*r2*4 +s_A*r*4 + s_R] - parent_inside) * parent_grd;
        tmp_grd += tmp_grd2;
        atomicAdd(head_dd_grd + b_idx*r*r2*4 + s_A*r*4 + s_R, tmp_grd2);

        parent_inside = alpha_dd[b_idx*L*L*L*L*r2*4 + start*L*L*L*r2*4 + gap_start*L*L*r2*4 + gap_end*L*r2*4 + end*r2*4 + r2 + s_A];
        parent_grd = alpha_dd[b_idx*L*L*L*L*r2*4 + gap_start*L*L*L*r2*4 + start*L*L*r2*4 + gap_end*L*r2*4 + end*r2*4 + r2 + s_A];
        tmp_grd2 = exp(tmp + head_dd[b_idx*r*r2*4 +s_A*r*4 + r + s_R] - parent_inside) * parent_grd;
        tmp_grd += tmp_grd2;
        atomicAdd(head_dd_grd + b_idx*r*r2*4 + s_A*r*4 + r + s_R, tmp_grd2);
                                ...
                                int B,  int L,  int width,   int r)
{
     ...
     {
        tmp_result = logsumexpf(tmp_result, s_d[split*L3 + gap_start*L2 + gap_end*L1 + end*r*4 + 0*r + s_R] + s[start*L*r*4 + split*r*4 +  0*r + s_R]);
        tmp_result = logsumexpf(tmp_result, s_d[start*L3 + split*L2 + gap_end*L1 + end*r*4 + 1*r + s_R] + s[split*L*r*4 + gap_start*r*4 + 1*r + s_R]);
     }

     for(int split = gap_end+1; split <end; split++){
        tmp_result = logsumexpf(tmp_result, s_d[start*L3 + gap_start*L2 + split*L1 + end*r*4 + 2*r+ s_R] + s[gap_end*L*r*4 + split*r*4 + 2*r + s_R]);
        tmp_result = logsumexpf(tmp_result, s_d[start*L3 + gap_start*L2 + gap_end*L1 + split*r*4 + 3*r+ s_R] + s[split*L*r*4 + end*r*4 + 3*r + s_R]);
     }

     s_d[start*L3 + gap_start*L2 + end*L1 + gap_end*r*4 + s_R] = tmp_result;
}
                                ...
                                int B,  int L,  int width, int r, int r1, int r2)
{
      ...
      __shared__ float result11[100];
      __shared__ float result12[100];
      __shared__ float result13[100];
      __shared__ float result14[100];
      __shared__ float result21[100];
      __shared__ float result22[100];
      __shared__ float result23[100];
      __shared__ float result24[100];
      __syncthreads();

      float score1 =  alpha_dd[b_idx*L*L*L*L*r*4 + start*L*L*L*r*4 + gap_start*L*L*r*4 + end*L*r*4 + gap_end*r*4 + s_R];
      float score2 =  alpha_dd[b_idx*L*L*L*L*r*4 + start*L*L*L*r*4 + gap_start*L*L*r*4 + end*L*r*4 + gap_end*r*4 + r + s_R];
      float score3 =  alpha_dd[b_idx*L*L*L*L*r*4 + start*L*L*L*r*4 + gap_start*L*L*r*4 + end*L*r*4 + gap_end*r*4 + 2*r + s_R];
      float score4 =  alpha_dd[b_idx*L*L*L*L*r*4 + start*L*L*L*r*4 + gap_start*L*L*r*4 + end*L*r*4 + gap_end*r*4 + 3*r + s_R];

      result11[s_R] =  score1 + head_cd[b_idx*r*r1*4 + s_A*r*4 + s_R];
      result12[s_R] =  score2 + head_cd[b_idx*r*r1*4 + s_A*r*4 + r + s_R];
      result13[s_R] =  score3 + head_cd[b_idx*r*r1*4 + s_A*r*4 + 2*r + s_R];
      result14[s_R] =  score4 + head_cd[b_idx*r*r1*4 + s_A*r*4 + 3*r + s_R];
      float tmp1 = 0;
      float tmp2 = 0;
      float tmp3 = 0;
      float tmp4 = 0;
      ...
      if(s_A < r2){
         result21[s_R] = score1 + head_dd[b_idx*r*r2*4 + s_A * r *4 + s_R];

         result22[s_R] = score2 + head_dd[b_idx*r*r2*4 + s_A * r *4 + r + s_R];
         result23[s_R] = score3 + head_dd[b_idx*r*r2*4 + s_A * r *4 + 2*r + s_R];
         result24[s_R] = score4 + head_dd[b_idx*r*r2*4 + s_A * r *4 + 3*r + s_R];
      }
      __syncthreads();
      ...
      if(s_R==0){
        ...
        float final_result2 = logf(0);
        for(int i=0;i<r;i++){
            final_result = logsumexpf(final_result, result1[i]);
            final_result = logsumexpf(final_result, result2[i]);
            final_result = logsumexpf(final_result, result3[i]);
            final_result = logsumexpf(final_result, result4[i]);
                                ...
                                int B,  int L,  int width, int r, int r1, int r2)
{
      ...
      float tmp_grd = exp(inside + head_cd[b_idx*r*r1 + s_A * r + s_R] - parent_inside);
      tmp_grd *= parent_grd;
...

{
    for(int w=2; w<L-1; w++){
      ...
      dim3 gridDim13(B*(L-w),  (w-1)*(L-w), r2);
      kernel_forward_d2_step2<<<gridDim13, blockDim12>>>(head_cd2, head_dd2, alpha_dd, alpha_cd, B, L, w, r4, r2, r4);
      }
    }
