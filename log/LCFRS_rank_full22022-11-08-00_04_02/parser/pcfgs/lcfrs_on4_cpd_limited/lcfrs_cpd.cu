                                ...
                                int B,  int L,  int width,   int d,   int r)
{
    ...
    if(gap_start != (start+1)){
        tmp_result = logsumexp(tmp_result, s_d[(start+1)*L3 + gap_start*L2 + gap_end*L1 + end*r + s_R] rule_score + s[start*L*r*4 + (start+1)*r*4 +  0*r + s_R]);
        tmp_result = logsumexp(tmp_result, s_d[start*L3 + (gap_start-1)*L2 + gap_end*L1 + end*r + s_R] + rule_score + s[(gap_start-1)*L*r*4 + gap_start*r*4 + 1*r + s_R]);
    }

    if( (gap_end+1)!=end){
        tmp_result = logsumexp(tmp_result, s_d[start*L3 + gap_start*L2 + (gap_end+1)*L1 + end*r + s_R] + rule_score + s[gap_end*L*r*4 + (gap_end+1) *r*4 + 2*r + s_R]);
        tmp_result = logsumexp(tmp_result, s_d[start*L3 + gap_start*L2 + gap_end*L1 + (end-1)*r + s_R] + rule_score + s[(end-1)*L*r*4 + end*r*4 + 3*r + s_R]);
    }
                                ...
                                const int B, const int L, const int width, const int d, const int r)
{
    ...
    if(gap_start != (start+1)){
        float tmp = exp(s_d[(start+1)*L3 + gap_start*L2 + gap_end*L1 + end*r + s_R] + rule_score + s[start*L*r*4 + (start+1)*r*4 + 0*r + s_R] - parent_inside) * parent_grd;
        rule_grd+=tmp;
        atomicAdd(s + (start+1)*L*r*4 + start*r*4 + s_R , tmp);
        atomicAdd(s_d +  gap_start*L3 + (start+1)*L2 + gap_end*L1 + end*r + s_R, tmp);



        tmp = exp(s_d[start*L3 + (gap_start-1)*L2 + gap_end*L1 + end*r + s_R] + rule_score + s[(gap_start-1)*L*r*4 + gap_start*r*4 + 1*r + s_R] - parent_inside) * parent_grd;
        rule_grd += tmp;
        atomicAdd(s + gap_start*L*r*4 + (gap_start-1)*r*4 + s_R + r*1, tmp);
        atomicAdd(s_d +  (gap_start-1)*L3 + start*L2 + gap_end*L1 + end*r + s_R, tmp);
    }

    if((gap_end+1)!=end){
        float tmp = exp(s_d[start*L3 + gap_start*L2 + (gap_end+1)*L1 + end*r + s_R] + rule_score + s[gap_end*L*r*4 + (gap_end+1)*r*4 + 2*r + s_R] - parent_inside) * parent_grd;
        rule_grd+=tmp;
        atomicAdd(s_d + gap_start*L3 + start*L2 + (gap_end+1)*L1 + end*r + s_R, tmp);
        atomicAdd(s + split*L*r*4 + (gap_end+1)*r*4 + 2*r + s_R , tmp);

        tmp = exp(s_d[start*L3 + gap_start*L2 + gap_end*L1 + (end-1)*r + s_R] + rule_score + s[(end-1)*L*r*4 + end*r*4 + 3*r + s_R] - parent_inside) * parent_grd;
        rule_grd+=tmp;
        atomicAdd(s_d + gap_start*L3 + start*L2 + gap_end*L1 + (end-1)*r + s_R, tmp);
        atomicAdd(s + end*L*r*4 + (end-1)*r*4 + 3*r + s_R, tmp);
    }
                                    ...
                                    int B,   int L,  int m, int p,   int r)
{
    ...
    result1[s_B] = u + cc[b_idx*(p)*r*4 + (s_B)*r*4 + s_R];
    result2[s_B] = u + cc[b_idx*(p)*r*4 + (s_B)*r*4 + 1*r + s_R];
    result3[s_B] = u + cc[b_idx*(p)*r*4 + (s_B)*r*4 + 2*r + s_R];
    result4[s_B] = u + cc[b_idx*(p)*r*4 + (s_B)*r*4 + 3*r + s_R];
    __syncthreads();
    if(s_B==0){
                                    ...
                                    int B,   int L,  int m, int p,   int r)
{
    ...
    float tmp = exp(u + cc[b_idx*(p)*r*4 + (s_B)*r*4 + s_R] - parent_inside) * parent_grd;
    atomicAdd(cc_grd + b_idx*(p)*r*4 + (s_B)*r*4 + s_R, tmp);
    u_grd += tmp;
    ...
    tmp = exp(u + cc[b_idx*(p)*r*4 + (s_B)*r*4 + 1*r + s_R]-parent_inside) * parent_grd;
    atomicAdd(cc_grd + b_idx*(p)*r*4 + (s_B)*r*4 + 1*r + s_R, tmp);
    u_grd += tmp;
    ...
    tmp = exp(u + cc[b_idx*(p)*r*4 + (s_B)*r*4 + 2*r + s_R] - parent_inside)*parent_grd;
    atomicAdd(cc_grd + b_idx*(p)*r*4 + (s_B)*r*4 + 2*r + s_R, tmp);
    u_grd += tmp;
    ...
    tmp = exp(u + cc[b_idx*(p)*r*4 + (s_B)*r*4 + 3*r + s_R] - parent_inside)*parent_grd;
    atomicAdd(cc_grd + b_idx*(p)*r*4 + (s_B)*r*4 + 3*r + s_R, tmp);
    u_grd += tmp;
