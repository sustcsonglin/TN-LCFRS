                               ...
                               const int B, const int L, const int width, const int m, const int p, const int d)
{
    ...
    if(gap_start > start+1){
        ...
        right[s_B] =  s_d[(start+1)*L*L*L*d + gap_start*L*L*d+  gap_end*L*d + end*d+s_B];
        __syncthreads();
        for(int sc=0; sc<p; sc++)
        {
            tmp_result = logsumexp(tmp_result, left[sc]+right[s_B]+ binary[s_A*pd*4 + s_B*p*4 + (sc)*4 +0]);
        }
        __syncthreads();
        ...
        left[s_B] = s_d[start*L3 + (gap_start-1)*L2 + gap_end*L1 + end*d + s_B];
        __syncthreads();

        for(int sc=0; sc<p; sc++)
        {
            tmp_result = logsumexp(tmp_result, right[sc]+left[s_B]+ binary[s_A*pd*4 + s_B*p*4 + (sc)*4 + 1]);
        }
        __syncthreads();
     }

    if(gap_end < end-1){
        left[s_B] = s_d[start*L3 + gap_start*L2 + (gap_end+1)*L1 + end*d +s_B];
        for(int ss=s_B; ss<p; ss+=d)
        {
            right[ss] = unary[ b_idx*(L-1)*p+ (gap_end)*p + ss];
        }
        __syncthreads();

        for(int sc=0;sc<p;sc++){
            tmp_result = logsumexp(tmp_result, left[s_B] + right[sc] +  binary[s_A*pd*4 + s_B*p*4 + (sc)*4 + 2]);
        }
        __syncthreads();

        left[s_B] = s_d[start*L3 + gap_start*L2 + gap_end*L1 + (end-1)*d +s_B];
        __syncthreads();
        for(int ss=s_B; ss<p; ss+=d)
        {
            right[ss] = unary[ b_idx*(L-1)*p+ (end-1)*p + ss];
        ...
        __syncthreads();
        for(int sc=0;sc<p;sc++){
            tmp_result = logsumexp(tmp_result, left[s_B] + right[sc] +  binary[s_A*pd*4 + s_B*p*4 + (sc)*4+ 3]);
        }
     }
                               ...
                               const int B, const int L, const int width, const int m, const int p, const int d)
{
    ...
    __syncthreads();
        ...
        for(int sc=0; sc<p; sc++)
        {
            tmp = left[sc]+right[s_B]+ binary[s_A*pd*4 + s_B*p*4 + (sc)*4 +0];
            if(tmp > tmp_result){
                tmp_result = tmp;
                tmp_idx = s_B*p+4 + sc*4 + 0;
            }
        }
        __syncthreads();
        ...
        for(int sc=0; sc<p; sc++)
        {
            tmp = right[sc]+left[s_B]+ binary[s_A*pd*4 + s_B*p*4 + (sc)*4 + 1];
            if(tmp > tmp_result){
                tmp_result = tmp;
                tmp_idx = s_B*p*4 + (sc)*4 + 1;
            }
        }
        __syncthreads();
     }
    ...
    if(gap_end < end-1){
        ...
        for(int sc=0;sc<p;sc++){
            tmp = left[s_B] + right[sc] +  binary[s_A*pd*4 + s_B*p*4 + (sc)*4 + 2];
            if(tmp > tmp_result){
                tmp_result = tmp;
                tmp_idx = s_B*p*4 + (sc)*4 + 2;
            }
        }
        __syncthreads();
        ...
        __syncthreads();
        for(int sc=0;sc<p;sc++){
             tmp = left[s_B] + right[sc] +  binary[s_A*pd*4 + s_B*p*4 + (sc)*4+ 3];
             if(tmp > tmp_result){
                tmp_result = tmp;
                tmp_idx = s_B*p*4 + (sc)*4+ 3;
             }
        }
     }
                               ...
                               const int B, const int L, const int width, const int m, const int p, const int d)
{
    ...
    if(gap_start > start+1){
        ...
        for(int sc=0; sc<p; sc++)
        {
            float tmp = exp(left[sc]+right[s_B]+ binary[s_A*pd*4 + s_B*p*4 + (sc)*4 +0] - parent_inside)*parent_grd;
            atomicAdd(unary_grd + b_idx*(L-1)*p + start * p + sc, tmp);
            atomicAdd(s_d + gap_start*L*L*L*d + (start+1)*L*L*d+  gap_end*L*d + end*d+s_B, tmp);
            atomicAdd(binary_grd + s_B*p*4 + (sc)*4 +0, tmp);
        }
        __syncthreads();
        ...
        for(int sc=0; sc<p; sc++)
        {
            float tmp = exp(right[sc]+left[s_B]+ binary[s_A*pd*4 + s_B*p*4 + (sc)*4 + 1] - parent_inside)*parent_grd;
            atomicAdd(binary_grd + s_B*mp*4 + (sc)*4 + 1, tmp);
            atomicAdd( s_d +  (gap_start-1)*L3 + start*L2 + gap_end*L1 + end*d +s_B, tmp);
    ...
    if(gap_end < end-1){
        ...
        for(int sc=0;sc<p;sc++){
            float tmp =  exp(left[s_B] + right[sc] +  binary[s_A*pd*4 + s_B*p*4 + (sc)*4 + 2] - parent_inside)*parent_grd;
            atomicAdd(binary_grd + s_B*mp*4 + (sc)*4 + 2, tmp);
            atomicAdd(s_d+ gap_start*L3 + start*L2 + (gap_end+1)*L1 + end*d +s_B, tmp);
            atomicAdd(unary_grd + b_idx*(L-1)*p+ gap_end*p + sc, tmp);
        }
        __syncthreads();
        ...
        __syncthreads();
        for(int sc=0;sc<p;sc++){
            float tmp = exp(left[s_B] + right[sc] +  binary[s_A*pd*4 + s_B*p*4 + (sc)*4+ 3] -parent_inside)*parent_grd;
            atomicAdd(binary_grd + s_B*mp*4 + (sc)*4+ 3, tmp);
            atomicAdd(s_d + gap_start*L3 + start*L2 + gap_end*L1 + (end-1)*d +s_B, tmp);
            atomicAdd(unary_grd + b_idx*(L-1)*p+ (end-1)*p + sc, tmp);
        }
     }
