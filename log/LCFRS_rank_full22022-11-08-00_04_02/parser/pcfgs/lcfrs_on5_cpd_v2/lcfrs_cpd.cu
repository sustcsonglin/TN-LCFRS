                                ...
                                int B,  int L,  int width,   int d,   int r)
{
    ...
    F *__restrict__   s = alpha_dc + b_idx * L * L * r ;
    ...
    {
        tmp_result = logsumexp(tmp_result, s_d[split*L3 + gap_start*L2 + gap_end*L1 + end*r*4 + s_R] + rule_score + s[start*L*r + split*r + s_R]);
        tmp_result = logsumexp(tmp_result, s_d[start*L3 + split*L2 + gap_end*L1 + end*r*4 + r + s_R] + rule_score + s[split*L*r + gap_start*r + s_R]);
    }

    for(int split = gap_end+1; split <end; split++){
        tmp_result = logsumexp(tmp_result, s_d[start*L3 + gap_start*L2 + split*L1 + end*r*4 + 2*r + s_R] + rule_score + s[gap_end*L*r + split*r + s_R]);
        tmp_result = logsumexp(tmp_result, s_d[start*L3 + gap_start*L2 + gap_end*L1 + split*r*4 + 3*r + s_R] + rule_score + s[split*L*r + end*r + s_R]);
    }
                                ...
                                const int B, const int L, const int width, const int d, const int r)
{
    ...
    for(int split=start+1; split< gap_start; split++)
    {
        float tmp = exp(s_d[split*L3 + gap_start*L2 + gap_end*L1 + end*r*4 + s_R] + rule_score + s[start*L*r + split*r + s_R] - parent_inside) * parent_grd;
        rule_grd += tmp;
        atomicAdd(s + split*L*r + start*r + s_R, tmp);
        atomicAdd(s_d + gap_start*L3 + split*L2 + gap_end*L1 + end*r*4 + s_R, tmp);

        tmp = exp(s_d[start*L3 + split*L2 + gap_end*L1 + end*r*4 + r + s_R] + rule_score + s[split*L*r + gap_start*r + s_R] - parent_inside) * parent_grd;
        rule_grd += tmp;
        atomicAdd(s + gap_start*L*r + split*r + s_R , tmp);
        atomicAdd(s_d +  split*L3 + start*L2 + gap_end*L1 + end*r*4 + r + s_R, tmp);
    }

    for(int split=gap_end+1; split <end; split++){
        float tmp = exp(s_d[start*L3 + gap_start*L2 + split*L1 + end*r*4 + 2*r + s_R] + rule_score + s[gap_end*L*r + split*r + s_R] - parent_inside) * parent_grd;
        rule_grd += tmp;
        atomicAdd(s_d + gap_start*L3 + start*L2 + split*L1 + end*r*4 + 2*r + s_R, tmp);
        atomicAdd(s + split*L*r + gap_end*r + s_R , tmp);

        tmp = exp(s_d[start*L3 + gap_start*L2 + gap_end*L1 + split*r*4 + 3*r + s_R] + rule_score + s[split*L*r + end*r + s_R] - parent_inside) * parent_grd;
        rule_grd += tmp;
        atomicAdd(s_d + gap_start*L3 + start*L2 + gap_end*L1 + split*r*4 + 3*r + s_R, tmp);
        atomicAdd(s + end*L*r + split*r + s_R, tmp);
    }
                   ...
                   int B, int L, int m, int p, int d, int r1, int r2, int r3, int r4)
{
     for(int w=L-1; w>1; w--){
        ...
        dim3 gridDim9(B, L-w, r4);
        dim3 blockDim9(m);
        kernel_backward_cmr<<<gridDim9, blockDim9>>>(dc, dc_grd, alpha, alpha_dc, B, L, w, m, p, r4);
