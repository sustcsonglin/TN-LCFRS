                                ...
                                int B,   int L,   int width,   int d,   int r)
{
     ...
     float tmp = (s_l[start*L*r + gap_start*r + s_R] + s_r[gap_end*L*r + end*r + s_R]);
     float tmp_grd =  exp(tmp + head_cd[b_idx*d*r+s_R*d+s_A] - parent_inside);
     if(tmp_grd > 1){
        printf("????? %f %f %f %f \n", s_l[start*L*r + gap_start*r + s_R], s_r[gap_end*L*r + end*r + s_R], parent_inside, head_cd[b_idx*d*r+s_R*d+s_A]);
     }
     tmp_grd *=parent_grd;
                                   ...
                                   )
{
     ...
     alpha_c[b_idx*L*L + end*L + start] = float(tmp_idx);
}
                                   ...
                                   )
{
    ...
    alpha_d[b_idx*L4+gap_start*L3+start*L2+gap_end*L+end] = float(tmp_idx);
}

template <typename F>
__global__ void kernel_argmax_d_v2(const F *__restrict__ const s_span_d,
                                    F *__restrict__ alpha_c,
                                    F *__restrict__ alpha_d,
                                    int B,  int L,  int width
                                   )
...
void cuda_argmax_v2(float *s_span_c, float *s_span_d, float *alpha_c, float *alpha_d, int B, int L)
{
    for(int w=2; w<L; w++){
        dim3 gridDim(B);
        dim3 blockDim(L-w);
        ...
        if(w<L-1){
            dim3 gridDim3(B,(w-1), (L-w));
            dim3 blockDim3(L-w);
            kernel_argmax_d_v2<<<gridDim3, blockDim3>>>(s_span_d, alpha_c, alpha_d, B, L, w);
        }
    }
}
