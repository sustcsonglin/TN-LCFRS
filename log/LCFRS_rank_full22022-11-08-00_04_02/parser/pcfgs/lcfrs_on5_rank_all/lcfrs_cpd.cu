                                   ...
                                   int B,  int L,  int width,   int r,   int r1, int r2)
{
    ...
    if(s_A < r2){
        result2[s_R] = tmp + head_dd[b_idx*r*r2*4 + s_A*r*4 + 0*r + s_R];
        result3[s_R] = tmp + head_dd[b_idx*r*r2*4 + s_A*r*4 + 1*r + s_R];
        result4[s_R] = tmp + head_dd[b_idx*r*r2*4 + s_A*r*4 + 2*r + s_R];
        result5[s_R] = tmp + head_dd[b_idx*r*r2*4 + s_A*r*4 + 3*r + s_R];
    }
    ...
    for(unsigned int s= i/2; s>0; s>>=1){
        if((s_R < s) & (s_R+ s < r)){
           ...
           if(s_A < r2){
                ...
                result5[s_R] = logsumexpf(result5[s_R], result5[s_R+s]);
           }
        }
        __syncthreads();
    }
    ...
    if(s_R==0){
        ...
        if(s_A < r2){
            alpha_dd[b_idx*L*L*L*L*r2*4 + start*L*L*L*r2*4 + gap_start*L*L*r2*4 + gap_end*L*r2*4 + end*r2*4 + 0*r2 + s_A] = result2[0];
            alpha_dd[b_idx*L*L*L*L*r2*4 + start*L*L*L*r2*4 + gap_start*L*L*r2*4 + gap_end*L*r2*4 + end*r2*4 + 1*r2 + s_A] = result3[0];
            alpha_dd[b_idx*L*L*L*L*r2*4 + start*L*L*L*r2*4 + gap_start*L*L*r2*4 + gap_end*L*r2*4 + end*r2*4 + 2*r2 + s_A] = result4[0];
            alpha_dd[b_idx*L*L*L*L*r2*4 + start*L*L*L*r2*4 + gap_start*L*L*r2*4 + gap_end*L*r2*4 + end*r2*4 + 3*r2 + s_A] = result5[0];
        }
    }
}
                                ...
                                int B,   int L,   int width,   int r,  int r1, int r2)
{
     ...
     if(s_A < r2){
        parent_inside = alpha_dd[b_idx*L*L*L*L*r2*4 + start*L*L*L*r2*4 + gap_start*L*L*r2*4 + gap_end*L*r2*4 + end*r2*4 + 0*r2 + s_A];
        parent_grd = alpha_dd[b_idx*L*L*L*L*r2*4 + gap_start*L*L*L*r2*4 + start*L*L*r2*4 + gap_end*L*r2*4 + end*r2*4 + 0*r2 + s_A];
        float tmp_grd2 = exp(tmp + head_dd[b_idx*r*r2*4 +s_A*r*4 + 0*r + s_R] - parent_inside) * parent_grd;
        tmp_grd += tmp_grd2;
        atomicAdd(head_dd_grd + b_idx*r*r2*4 + s_A*r*4 + 0*r + s_R, tmp_grd2);

        parent_inside = alpha_dd[b_idx*L*L*L*L*r2*4 + start*L*L*L*r2*4 + gap_start*L*L*r2*4 + gap_end*L*r2*4 + end*r2*4 + 1*r2 + s_A];
        parent_grd = alpha_dd[b_idx*L*L*L*L*r2*4 + gap_start*L*L*L*r2*4 + start*L*L*r2*4 + gap_end*L*r2*4 + end*r2*4 + 1*r2 + s_A];
        tmp_grd2 = exp(tmp + head_dd[b_idx*r*r2*4 +s_A*r*4 + 1*r + s_R] - parent_inside) * parent_grd;
        tmp_grd += tmp_grd2;
        atomicAdd(head_dd_grd + b_idx*r*r2*4 + s_A*r*4 + 1*r + s_R, tmp_grd2);

        parent_inside = alpha_dd[b_idx*L*L*L*L*r2*4 + start*L*L*L*r2*4 + gap_start*L*L*r2*4 + gap_end*L*r2*4 + end*r2*4 + 2*r2 + s_A];
        parent_grd = alpha_dd[b_idx*L*L*L*L*r2*4 + gap_start*L*L*L*r2*4 + start*L*L*r2*4 + gap_end*L*r2*4 + end*r2*4 + 2*r2 + s_A];
        tmp_grd2 = exp(tmp + head_dd[b_idx*r*r2*4 +s_A*r*4 + 2*r + s_R] - parent_inside) * parent_grd;
        tmp_grd += tmp_grd2;
        atomicAdd(head_dd_grd + b_idx*r*r2*4 + s_A*r*4 + 2*r + s_R, tmp_grd2);

        parent_inside = alpha_dd[b_idx*L*L*L*L*r2*4 + start*L*L*L*r2*4 + gap_start*L*L*r2*4 + gap_end*L*r2*4 + end*r2*4 + 3*r2 + s_A];
        parent_grd = alpha_dd[b_idx*L*L*L*L*r2*4 + gap_start*L*L*L*r2*4 + start*L*L*r2*4 + gap_end*L*r2*4 + end*r2*4 + 3*r2 + s_A];
        tmp_grd2 = exp(tmp + head_dd[b_idx*r*r2*4 +s_A*r*4 + 3*r + s_R] - parent_inside) * parent_grd;
        tmp_grd += tmp_grd2;
        atomicAdd(head_dd_grd + b_idx*r*r2*4 + s_A*r*4 + 3*r + s_R, tmp_grd2);
     }
                                ...
                                int B,  int L,  int width,   int r)
{
     ...
     float *  s_d = alpha_dd + b_idx * L * L * L * L * r * 4;
     int L3 = L * L * L * r * 4;
     int L2 = L * L * r * 4;
     int L1 = L * r * 4;

     float tmp_result1 = -999999;
     float tmp_result2 = -999999;
     float tmp_result3 = -999999;
     float tmp_result4 = -999999;
     ...
     {
        tmp_result1 = logsumexpf(tmp_result1, s_d[split*L3 + gap_start*L2 + gap_end*L1 + end*r*4 + 0 *r + s_R] + s[start*L*r*4 + split*r*4 + 0*r + s_R]);



        tmp_result2 = logsumexpf(tmp_result2, s_d[start*L3 + split*L2 + gap_end*L1 + end*r*4 + 1*r + s_R] + s[split*L*r*4 + gap_start*r*4 + 1*r + s_R]);
     }

     for(int split = gap_end+1; split <end; split++){
        tmp_result3 = logsumexpf(tmp_result3, s_d[start*L3 + gap_start*L2 + split*L1 + end*r*4 + 2*r + s_R] + s[gap_end*L*r*4 + split*r*4 + 2*r + s_R]);
        tmp_result4 = logsumexpf(tmp_result4, s_d[start*L3 + gap_start*L2 + gap_end*L1 + split*r*4 + 3*r + s_R] + s[split*L*r*4 + end*r*4 + 3*r + s_R]);
     }
     ...
     tmp1 = tmp_result1 + head_dd[b_idx*r*r*4*4 + s_A*r*4*4 + 0*r*4 + 0*r + s_R];
     tmp1 = logsumexpf(tmp1, tmp_result2 + head_dd[b_idx*r*r*4*4 + s_A*r*4*4 + 0*r*4 + 1*r + s_R]);
     tmp1 = logsumexpf(tmp1, tmp_result3 + head_dd[b_idx*r*r*4*4 + s_A*r*4*4 + 0*r*4 + 2*r + s_R]);
     tmp1 = logsumexpf(tmp1, tmp_result4 + head_dd[b_idx*r*r*4*4 + s_A*r*4*4 + 0*r*4 + 3*r + s_R]);
     result2[s_R] = tmp1;

     tmp1 = tmp_result1 + head_dd[b_idx*r*r*4*4 + s_A*r*4*4 + 1*r*4 + 0*r + s_R];
     tmp1 = logsumexpf(tmp1, tmp_result2 + head_dd[b_idx*r*r*4*4 + s_A*r*4*4 + 1*r*4 + 1*r + s_R]);
     tmp1 = logsumexpf(tmp1, tmp_result3 + head_dd[b_idx*r*r*4*4 + s_A*r*4*4 + 1*r*4 + 2*r + s_R]);
     tmp1 = logsumexpf(tmp1, tmp_result4 + head_dd[b_idx*r*r*4*4 + s_A*r*4*4 + 1*r*4 + 3*r + s_R]);

     result3[s_R] = tmp1;

     tmp1 = tmp_result1 + head_dd[b_idx*r*r*4*4 + s_A*r*4*4 + 2*r*4 + 0*r + s_R];
     tmp1 = logsumexpf(tmp1, tmp_result2 + head_dd[b_idx*r*r*4*4 + s_A*r*4*4 + 2*r*4 + 1*r + s_R]);
     tmp1 = logsumexpf(tmp1, tmp_result3 + head_dd[b_idx*r*r*4*4 + s_A*r*4*4 + 2*r*4 + 2*r + s_R]);
     tmp1 = logsumexpf(tmp1, tmp_result4 + head_dd[b_idx*r*r*4*4 + s_A*r*4*4 + 2*r*4 + 3*r + s_R]);
     result4[s_R] = tmp1;

     tmp1 = tmp_result1 + head_dd[b_idx*r*r*4*4 + s_A*r*4*4 + 3*r*4 + 0*r + s_R];
     tmp1 = logsumexpf(tmp1, tmp_result2 + head_dd[b_idx*r*r*4*4 + s_A*r*4*4 + 3*r*4 + 1*r + s_R]);
     tmp1 = logsumexpf(tmp1, tmp_result3 + head_dd[b_idx*r*r*4*4 + s_A*r*4*4 + 3*r*4 + 2*r + s_R]);
     tmp1 = logsumexpf(tmp1, tmp_result4 + head_dd[b_idx*r*r*4*4 + s_A*r*4*4 + 3*r*4 + 3*r + s_R]);
     ...
     if(s_R==0){
        ...
        for(int i=0;i<r;i++){
            ...
            final_result3 = logsumexpf(final_result3, result3[i]);
            final_result4 = logsumexpf(final_result4, result4[i]);
            final_result5 = logsumexpf(final_result5, result5[i]);
        }
        ...
        alpha_dd[b_idx*L*L*L*L*r*4 + start*L*L*L*r*4 + gap_start*L*L*r*4 + gap_end*L*r*4 + end*r*4 + 0*r + s_A] = logsumexpf(alpha_dd[b_idx*L*L*L*L*r*4 + start*L*L*L*r*4 + gap_start*L*L*r*4 + gap_end*L*r*4 + end*r*4 + 0*r + s_A], final_result2);
        alpha_dd[b_idx*L*L*L*L*r*4 + start*L*L*L*r*4 + gap_start*L*L*r*4 + gap_end*L*r*4 + end*r*4 + 1*r + s_A] = logsumexpf(alpha_dd[b_idx*L*L*L*L*r*4 + start*L*L*L*r*4 + gap_start*L*L*r*4 + gap_end*L*r*4 + end*r*4 + 1*r + s_A], final_result3);
        alpha_dd[b_idx*L*L*L*L*r*4 + start*L*L*L*r*4 + gap_start*L*L*r*4 + gap_end*L*r*4 + end*r*4 + 2*r + s_A] = logsumexpf(alpha_dd[b_idx*L*L*L*L*r*4 + start*L*L*L*r*4 + gap_start*L*L*r*4 + gap_end*L*r*4 + end*r*4 + 2*r + s_A], final_result4);
        alpha_dd[b_idx*L*L*L*L*r*4 + start*L*L*L*r*4 + gap_start*L*L*r*4 + gap_end*L*r*4 + end*r*4 + 3*r + s_A] = logsumexpf(alpha_dd[b_idx*L*L*L*L*r*4 + start*L*L*L*r*4 + gap_start*L*L*r*4 + gap_end*L*r*4 + end*r*4 + 3*r + s_A], final_result5);
    }
}
                                ...
                                int B,  int L,  int width, int r)
{
    ...
    float rule_score21 = head_dd[b_idx*r*r*4*4 + s_A*r*4*4 + 0*r*4 + 0*r + s_R];
    float rule_score22 = head_dd[b_idx*r*r*4*4 + s_A*r*4*4 + 0*r*4 + 1*r + s_R];
    float rule_score23 = head_dd[b_idx*r*r*4*4 + s_A*r*4*4 + 0*r*4 + 2*r + s_R];
    float rule_score24 = head_dd[b_idx*r*r*4*4 + s_A*r*4*4 + 0*r*4 + 3*r + s_R];

    float rule_score31 = head_dd[b_idx*r*r*4*4 + s_A*r*4*4 + 1*r*4 + 0*r + s_R];
    float rule_score32 = head_dd[b_idx*r*r*4*4 + s_A*r*4*4 + 1*r*4 + 1*r + s_R];
    float rule_score33 = head_dd[b_idx*r*r*4*4 + s_A*r*4*4 + 1*r*4 + 2*r + s_R];
    float rule_score34 = head_dd[b_idx*r*r*4*4 + s_A*r*4*4 + 1*r*4 + 3*r + s_R];

    float rule_score41 = head_dd[b_idx*r*r*4*4 + s_A*r*4*4 + 2*r*4 + 0*r + s_R];
    float rule_score42 = head_dd[b_idx*r*r*4*4 + s_A*r*4*4 + 2*r*4 + 1*r + s_R];
    float rule_score43 = head_dd[b_idx*r*r*4*4 + s_A*r*4*4 + 2*r*4 + 2*r + s_R];
    float rule_score44 = head_dd[b_idx*r*r*4*4 + s_A*r*4*4 + 2*r*4 + 3*r + s_R];

    float rule_score51 = head_dd[b_idx*r*r*4*4 + s_A*r*4*4 + 3*r*4 + 0*r + s_R];
    float rule_score52 = head_dd[b_idx*r*r*4*4 + s_A*r*4*4 + 3*r*4 + 1*r + s_R];
    float rule_score53 = head_dd[b_idx*r*r*4*4 + s_A*r*4*4 + 3*r*4 + 2*r + s_R];
    float rule_score54 = head_dd[b_idx*r*r*4*4 + s_A*r*4*4 + 3*r*4 + 3*r + s_R];
    ...
    float parent_inside21 = alpha_dd[b_idx*L*L*L*L*r*4 + start*L3*4 + gap_start*L2*4 + gap_end*L1*4 + end*r*4 + 0*r + s_A];
    float parent_grd21 = alpha_dd[b_idx*L*L*L*L*r*4 + gap_start*L3*4 + start*L2*4 + gap_end*L1*4 + end*r*4 + 0*r + s_A];

    float parent_inside22 = alpha_dd[b_idx*L*L*L*L*r*4 + start*L3*4 + gap_start*L2*4 + gap_end*L1*4 + end*r*4 + 1*r + s_A];
    float parent_grd22 = alpha_dd[b_idx*L*L*L*L*r*4 + gap_start*L3*4 + start*L2*4 + gap_end*L1*4 + end*r*4 + 1*r + s_A];

    float parent_inside23 = alpha_dd[b_idx*L*L*L*L*r*4 + start*L3*4 + gap_start*L2*4 + gap_end*L1*4 + end*r*4 + 2*r + s_A];
    float parent_grd23 = alpha_dd[b_idx*L*L*L*L*r*4 + gap_start*L3*4 + start*L2*4 + gap_end*L1*4 + end*r*4 + 2*r + s_A];

    float parent_inside24 = alpha_dd[b_idx*L*L*L*L*r*4 + start*L3*4 + gap_start*L2*4 + gap_end*L1*4 + end*r*4 + 3*r + s_A];
    float parent_grd24 = alpha_dd[b_idx*L*L*L*L*r*4 + gap_start*L3*4 + start*L2*4 + gap_end*L1*4 + end*r*4 + 3*r + s_A];
    ...
    for(int split=start+1; split< gap_start; split++)
    {
        float tmp = s_d[split*L3 + gap_start*L2 + gap_end*L1 + end*r*4 + 0*r + s_R] + s[start*L*r*4 + split*r*4 + 0*r + s_R];

        float grd1 = exp(tmp + rule_score11 - parent_inside) * parent_grd;
        rule_grd11 += grd1;
        float grd2 = exp(tmp + rule_score21 - parent_inside21) * parent_grd21;
        rule_grd21 += grd2;
        float grd3 = exp(tmp + rule_score31 - parent_inside22) * parent_grd22;
        rule_grd31 += grd3;
        float grd4 = exp(tmp + rule_score41 - parent_inside23) * parent_grd23;
        rule_grd41 += grd4;
        float grd5 =  exp(tmp + rule_score51 - parent_inside24) * parent_grd24;
        rule_grd51 += grd5;
        atomicAdd(s_d + gap_start*L3 + split*L2 + gap_end*L1 + end*r*4 + 0*r + s_R, grd1+grd2+grd3+grd4+grd5);
        atomicAdd(s + split*L*r*4 + start*r*4 + 0*r + s_R, grd1+grd2+grd3+grd4+grd5);

        tmp = s_d[start*L3 + split*L2 + gap_end*L1 + end*r*4 + 1*r + s_R]  + s[split*L*r*4 + gap_start*r*4 + 1*r + s_R];
        grd1 = exp(tmp + rule_score12 - parent_inside) * parent_grd;
        rule_grd12 += grd1;
        grd2 = exp(tmp + rule_score22 - parent_inside21) * parent_grd21;
        rule_grd22 += grd2;
        grd3 = exp(tmp + rule_score32 - parent_inside22) * parent_grd22;
        rule_grd32 += grd3;
        grd4 = exp(tmp + rule_score42 - parent_inside23) * parent_grd23;
        rule_grd42 += grd4;
        grd5 =  exp(tmp + rule_score52 - parent_inside24) * parent_grd24;
        rule_grd52 += grd5;
        atomicAdd(s_d + split*L3 + start*L2 + gap_end*L1 + end*r*4 + 1*r + s_R, grd1+grd2+grd3+grd4+grd5);
        atomicAdd(s + gap_start*L*r*4 + split*r*4 + 1*r + s_R, grd1+grd2+grd3+grd4+grd5);
    }

    for(int split=gap_end+1; split <end; split++){
        float tmp = s_d[start*L3 + gap_start*L2 + split*L1 + end*r*4 + 2*r + s_R] + s[gap_end*L*r*4 + split*r*4 + 2*r + s_R];
        float grd1 = exp(tmp + rule_score13 - parent_inside) * parent_grd;
        rule_grd13 += grd1;
        float grd2 = exp(tmp + rule_score23 - parent_inside21) * parent_grd21;
        rule_grd23 += grd2;
        float grd3 = exp(tmp + rule_score33 - parent_inside22) * parent_grd22;
        rule_grd33 += grd3;
        float grd4 = exp(tmp + rule_score43 - parent_inside23) * parent_grd23;
        rule_grd43 += grd4;
        float grd5 =  exp(tmp + rule_score53 - parent_inside24) * parent_grd24;
        rule_grd53 += grd5;
        atomicAdd(s_d + gap_start*L3 + start*L2 + split*L1 + end*r*4 + 2*r + s_R, grd1+grd2+grd3+grd4+grd5);
        atomicAdd(s + split*L*r*4 + gap_end*r*4 + 2*r + s_R, grd1+grd2+grd3+grd4+grd5);

        tmp = s_d[start*L3 + gap_start*L2 + gap_end*L1 + split*r*4 + 3*r + s_R] + s[split*L*r*4 + end*r*4 + 3*r + s_R];
        grd1 = exp(tmp + rule_score14 - parent_inside) * parent_grd;
        rule_grd14 += grd1;
        grd2 = exp(tmp + rule_score24 - parent_inside21) * parent_grd21;
        rule_grd24 += grd2;
        grd3 = exp(tmp + rule_score34 - parent_inside22) * parent_grd22;
        rule_grd34 += grd3;
        grd4 = exp(tmp + rule_score44 - parent_inside23) * parent_grd23;
        rule_grd44 += grd4;
        grd5 =  exp(tmp + rule_score54 - parent_inside24) * parent_grd24;
        rule_grd54 += grd5;

        atomicAdd(s_d + gap_start*L3 + start*L2 + gap_end*L1 + split*r*4 + 3*r + s_R, grd1+grd2+grd3+grd4+grd5);
        atomicAdd(s + end*L*r*4 + split*r*4 + 3*r + s_R, grd1+grd2+grd3+grd4+grd5);
    }

    atomicAdd(head_cd_grd + b_idx*r*r*4 + s_A*r*4 + 0*r + s_R, rule_grd11);
    atomicAdd(head_cd_grd + b_idx*r*r*4 + s_A*r*4 + 1*r + s_R , rule_grd12);
    atomicAdd(head_cd_grd + b_idx*r*r*4 + s_A*r*4 + 2*r + s_R , rule_grd13);
    atomicAdd(head_cd_grd + b_idx*r*r*4 + s_A*r*4 + 3*r + s_R , rule_grd14);

    atomicAdd(head_dd_grd + b_idx*r*r*4*4 + s_A*r*4*4 + 0*r*4 + 0*r + s_R, rule_grd21);
    atomicAdd(head_dd_grd + b_idx*r*r*4*4 + s_A*r*4*4 + 0*r*4 + 1*r + s_R, rule_grd22);
    atomicAdd(head_dd_grd + b_idx*r*r*4*4 + s_A*r*4*4 + 0*r*4 + 2*r + s_R, rule_grd23);
    atomicAdd(head_dd_grd + b_idx*r*r*4*4 + s_A*r*4*4 + 0*r*4 + 3*r + s_R, rule_grd24);

    atomicAdd(head_dd_grd + b_idx*r*r*4*4 + s_A*r*4*4 + 1*r*4 + 0*r + s_R, rule_grd31);
    atomicAdd(head_dd_grd + b_idx*r*r*4*4 + s_A*r*4*4 + 1*r*4 + 1*r + s_R, rule_grd32);
    atomicAdd(head_dd_grd + b_idx*r*r*4*4 + s_A*r*4*4 + 1*r*4 + 2*r + s_R, rule_grd33);
    atomicAdd(head_dd_grd + b_idx*r*r*4*4 + s_A*r*4*4 + 1*r*4 + 3*r + s_R, rule_grd34);

    atomicAdd(head_dd_grd + b_idx*r*r*4*4 + s_A*r*4*4 + 2*r*4 + 0*r + s_R, rule_grd41);
    atomicAdd(head_dd_grd + b_idx*r*r*4*4 + s_A*r*4*4 + 2*r*4 + 1*r + s_R, rule_grd42);
    atomicAdd(head_dd_grd + b_idx*r*r*4*4 + s_A*r*4*4 + 2*r*4 + 2*r + s_R, rule_grd43);
    atomicAdd(head_dd_grd + b_idx*r*r*4*4 + s_A*r*4*4 + 2*r*4 + 3*r + s_R, rule_grd44);

    atomicAdd(head_dd_grd + b_idx*r*r*4*4 + s_A*r*4*4 + 3*r*4 + 0*r + s_R, rule_grd51);
    atomicAdd(head_dd_grd + b_idx*r*r*4*4 + s_A*r*4*4 + 3*r*4 + 1*r + s_R, rule_grd52);
    atomicAdd(head_dd_grd + b_idx*r*r*4*4 + s_A*r*4*4 + 3*r*4 + 2*r + s_R, rule_grd53);
    atomicAdd(head_dd_grd + b_idx*r*r*4*4 + s_A*r*4*4 + 3*r*4 + 3*r + s_R, rule_grd54);
   }
