                                   ...
                                   int B,  int L,   int width,   int r,   int r1, int r2, int r3)
{
    ...
    if (s_A < r1){
        result[s_R] = tmp + head_cd[b_idx * r_all * r1 + s_A*r_all + s_R];
    }
    if(s_A < r2){
        result2[s_R] = tmp + head_dd[b_idx * r_all * r2 + s_A*r_all + s_R];
    }

    if (s_A < r3){
        result3[s_R] = tmp + head_ill_in[b_idx * r_all* r3 + s_A * r_all + s_R];
        result4[s_R] = tmp + head_ill_out[b_idx* r_all* r3 + s_A * r_all + s_R];
    ...
    if(s_R==0){
        ...
        if(s_A < r3){
            alpha_ill_in[b_idx*L*L*L*L*r3 + start*L*L*L*r3 + gap_start*L*L*r3 + gap_end*L*r3 + end*r3 + s_A] = result3[0];
            alpha_ill_out[b_idx*L*L*L*L*r3 + start*L*L*L*r3 + gap_start*L*L*r3 + gap_end*L*r3 + end*r3 + s_A] = result4[0];
        }
    }
}
                                     ...
                                     int B,  int L,   int width,   int r,  int r1, int r2, int r3)
{
     ...
     float tmp = (s_l[start*L*r + gap_start*r + s_R] + s_r[gap_end*L*r + end*r + s_R]);
      if (s_A < r1){
        ...
        tmp_grd =  exp(tmp + head_cd[b_idx*r_all*r1 + s_A*r_all + s_R] - parent_inside) * parent_grd;
        atomicAdd(head_cd_grd + b_idx*r_all*r1 + s_A * r_all + s_R, tmp_grd);
     }
     ...
     if(s_A < r2){
        ...
        float tmp_grd2 = exp(tmp + head_dd[b_idx*r_all*r2 +s_A*r_all + s_R] - parent_inside) * parent_grd;
        tmp_grd += tmp_grd2;
        atomicAdd(head_dd_grd + b_idx*r_all*r2 + s_A*r_all + s_R, tmp_grd2);
     }

     if (s_A < r3){
        float parent_inside = alpha_ill_in[b_idx*L*L*L*L*r3 + start*L*L*L*r3 + gap_start*L*L*r3 + gap_end*L*r3 + end*r3 + s_A];
        float parent_grd = alpha_ill_in[b_idx*L*L*L*L*r3 + gap_start*L*L*L*r3 + start*L*L*r3 + gap_end*L*r3 + end*r3 + s_A];
        float tmp_grd2 = exp(tmp + head_ill_in[b_idx*r_all*r3 +s_A*r_all+ s_R] - parent_inside) * parent_grd;
        tmp_grd += tmp_grd2;
        atomicAdd(head_ill_in_grd + b_idx*r_all*r3 + s_A*r_all + s_R, tmp_grd2);
     }

      if (s_A < r3){
        float parent_inside = alpha_ill_out[b_idx*L*L*L*L*r3 + start*L*L*L*r3 + gap_start*L*L*r3 + gap_end*L*r3 + end*r3 + s_A];
        float parent_grd = alpha_ill_out[b_idx*L*L*L*L*r3 + gap_start*L*L*L*r3 + start*L*L*r3 + gap_end*L*r3 + end*r3 + s_A];
        float tmp_grd2 = exp(tmp + head_ill_out[b_idx*r_all*r3 +s_A*r_all + s_R] - parent_inside) * parent_grd;
        tmp_grd += tmp_grd2;
        atomicAdd(head_ill_out_grd + b_idx*r_all*r3 + s_A*r_all + s_R, tmp_grd2);
     }
...
__global__ void kernel_forward_d3_step1(
                                float *alpha_in, float *alpha_out,
                                int B,  int L,  int width,   int r)
{
      int b_idx = blockIdx.x / (L-width);
      int start = blockIdx.x % (L-width);
     ...
     float *   s_in = alpha_in + b_idx * L * L * L * L * r;
     float *   s_out = alpha_out + b_idx * L * L * L * L * r;
     int L3 = L * L * L * r;
     int L2 = L * L * r;
     int L1 = L * r;
     float tmp_result = logf(0);
     ...
     for(int split=start+1; split< gap_start; split++)
     {
        for (int split2 = gap_end+1; split2 < end; split2++){
           tmp_result = logsumexpf( tmp_result, s_in[split*L3 + gap_start*L2 + gap_end*L1 + split2*r + s_R]  + s_out[start*L3 + split*L2 + split2 *L1 +  end*r +  s_R]);
        }
     }
     s_out[start*L3 + gap_start*L2 + end*L1 + gap_end*r + s_R] = tmp_result;
}

__global__ void kernel_backward_d3_step1(
                                float * alpha_in,
                                float * alpha_out,
                                int B,  int L,  int width,   int r)
{
     ...
     int L2 = L * L * r;
     int L1 = L * r;
     float parent_inside = s_out[start*L3 + gap_start*L2 + end*L1 + gap_end*r + s_R];
     float parent_grd =  s_out[gap_start*L3 + start*L2 + end*L1 + gap_end*r + s_R];

     for(int split=start+1; split< gap_start; split++)
     {
        for (int split2 = gap_end+1; split2 < end; split2++){
           float tmp =  expf(s_in[split*L3 + gap_start*L2 + gap_end*L1 + split2*r + s_R] + s_out[start*L3 + split*L2 + split2 *L1 +  end*r +  s_R] - parent_inside) * parent_grd;
           atomicAdd(s_in + gap_start*L3 + split*L2 + gap_end*L1 + split2*r + s_R, tmp);
           atomicAdd(s_out + split*L3 + start*L2 + split2 *L1 +  end*r + s_R, tmp);
        }
     }
                                ...
                                int B,  int L,  int width, int r, int r1, int r2, int r3)
{
      ...
      if(s_A < r1){
        result[s_R] = score + head_cd[b_idx*r_all*r1 + s_A * r_all + s_R + bias];
      }

      if(s_A < r2){
         result2[s_R] = score + head_dd[b_idx*r_all*r2 + s_A * r_all + s_R + bias];
      }

      if(s_A < r3){
         result3[s_R] = score + head_ill_in[b_idx*r_all*r3 + s_A * r_all + s_R + bias];
         result4[s_R] = score + head_ill_out[b_idx*r_all*r3 + s_A * r_all + s_R + bias];
      }
      ...
      if(s_R==0){
        ...
        if(s_A < r3){
            alpha_ill_in[b_idx*L*L*L*L*r3 + start*L*L*L*r3 + gap_start*L*L*r3 + gap_end*L*r3 + end*r3 + s_A] = logsumexpf(alpha_ill_in[b_idx*L*L*L*L*r3 + start*L*L*L*r3 + gap_start*L*L*r3 + gap_end*L*r3 + end*r3 + s_A], final_result3);
            alpha_ill_out[b_idx*L*L*L*L*r3 + start*L*L*L*r3 + gap_start*L*L*r3 + gap_end*L*r3 + end*r3 + s_A] = logsumexpf(alpha_ill_out[b_idx*L*L*L*L*r3 + start*L*L*L*r3 + gap_start*L*L*r3 + gap_end*L*r3 + end*r3 + s_A], final_result4);
        }
    }
}


__global__ void kernel_forward_d3_step2(
                                float *  head_cd,
                                float *  head_dd,
                                float *  head_ill_in,
                                float *  head_ill_out,
                                ...
                                int B,  int L,  int width, int r, int r1, int r2, int r3)
{
      ...
      int r_all = r1 + r2 + r3;
      int bias = r1 + r2;
      float score = alpha_ill_out[b_idx*L*L*L*L*r + start*L*L*L*r + gap_start*L*L*r + end*L*r + gap_end*r + s_R];
                                ...
                                int B,  int L,  int width, int r, int r1, int r2, int r3)
{
      ...
      if (s_A < r1){
        ...
        tmp_grd = exp(inside + head_cd[b_idx*r_all*r1 + s_A * r_all + s_R + bias] - parent_inside) * parent_grd;
        atomicAdd(head_cd_grd + b_idx*r_all*r1 + s_A * r_all + s_R + bias, tmp_grd);
     }

      if(s_A < r2){
        float parent_inside = alpha_dd[b_idx*L*L*L*L*r2 + start*L*L*L*r2 + gap_start*L*L*r2 + gap_end*L*r2 + end*r2 + s_A];
        float parent_grd = alpha_dd[b_idx*L*L*L*L*r2 + gap_start*L*L*L*r2 + start*L*L*r2 + gap_end*L*r2 + end*r2 + s_A];
        float tmp_grd2 = exp(inside + head_dd[b_idx*r_all*r2 + s_A * r_all + s_R + bias] - parent_inside);
        tmp_grd2 *= parent_grd;
        atomicAdd(head_dd_grd + b_idx*r_all*r2 + s_A * r_all + s_R + bias, tmp_grd2);
        tmp_grd += tmp_grd2;
      }
      ...
      if (s_A < r3){
        float parent_inside = alpha_ill_in[b_idx*L*L*L*L*r3 + start*L*L*L*r3 + gap_start*L*L*r3 + gap_end*L*r3 + end*r3 + s_A];
        float parent_grd = alpha_ill_in[b_idx*L*L*L*L*r3 + gap_start*L*L*L*r3 + start*L*L*r3 + gap_end*L*r3 + end*r3 + s_A];
        float tmp_grd2 = exp(inside + head_ill_in[b_idx*r_all*r3 +s_A*r_all + s_R + bias] - parent_inside) * parent_grd;
        tmp_grd += tmp_grd2;
        atomicAdd(head_ill_in_grd + b_idx*r_all*r3 + s_A*r_all + s_R + bias, tmp_grd2);
     }

      if (s_A < r3){
        float parent_inside = alpha_ill_out[b_idx*L*L*L*L*r3 + start*L*L*L*r3 + gap_start*L*L*r3 +  gap_end*L*r3 + end*r3 + s_A];
        float parent_grd = alpha_ill_out[b_idx*L*L*L*L*r3 + gap_start*L*L*L*r3 + start*L*L*r3 +  gap_end*L*r3 + end*r3 + s_A];
        float tmp_grd2 = exp(inside + head_ill_out[b_idx*r_all*r3 + s_A*r_all + s_R + bias] - parent_inside) * parent_grd;
        tmp_grd += tmp_grd2;
        atomicAdd(head_ill_out_grd + b_idx*r_all*r3 + s_A*r_all + s_R + bias, tmp_grd2);
      }
     atomicAdd(alpha_dd + b_idx*L*L*L*L*r + gap_start*L*L*L*r + start*L*L*r + end*L*r + gap_end*r + s_R, tmp_grd);
}


__global__ void kernel_backward_d3_step2(
                                float *  head_cd,
                                float *  head_cd_grd,
                                float *  head_dd,
                                float *  head_dd_grd,
                                ...
                                int B,  int L,  int width, int r, int r1, int r2, int r3)
{
      ...
      float tmp_grd = 0;
      float inside = alpha_ill_out[b_idx*L*L*L*L*r + start*L*L*L*r + gap_start*L*L*r + end*L*r + gap_end*r + s_R];
      int r_all = r1+r2+r3;
      int bias = r1+r2;
      ...
      if (s_A < r1){
        ...
        tmp_grd = exp(inside + head_cd[b_idx*r_all*r1 + s_A * r_all + s_R + bias] - parent_inside);
      ...
      if (s_A < r3){
        float parent_inside = alpha_ill_in[b_idx*L*L*L*L*r3 + start*L*L*L*r3 + gap_start*L*L*r3 + gap_end*L*r3 + end*r3 + s_A];
        float parent_grd = alpha_ill_in[b_idx*L*L*L*L*r3 + gap_start*L*L*L*r3 + start*L*L*r3 + gap_end*L*r3 + end*r3 + s_A];
        float tmp_grd2 = exp(inside + head_ill_in[b_idx*r_all*r3 +s_A*r_all+ s_R + bias] - parent_inside);
        if(tmp_grd2 > 1){
            printf("%f %f %f  ?? \n", inside, head_ill_in[b_idx*r_all*r3 +s_A*r_all+ s_R + bias], parent_inside);
        }
        tmp_grd2 *=parent_grd;
        tmp_grd += tmp_grd2;
        atomicAdd(head_ill_in_grd + b_idx*r_all*r3 + s_A*r_all + s_R + bias, tmp_grd2);
      ...
      atomicAdd(alpha_ill_out + b_idx*L*L*L*L*r + gap_start*L*L*L*r + start*L*L*r + end*L*r + gap_end*r + s_R, tmp_grd);
}
                   ...
                   int B, int L, int r1, int r2, int r3, int r4, int r5)
{
    for(int w=2; w<L-1; w++){
      ...
      kernel_forward_d1<<<gridDim11, blockDim11>>>(head_cd, head_dd, head_ill_in, head_ill_out,
                                                   alpha_ld, alpha_rd, alpha_cd, alpha_dd,
      ...
      dim3 gridDim13(B*(L-w),  (w-1)*(L-w),  max(max(r2, r4), r5));
      kernel_forward_d2_step2<<<gridDim13, blockDim12>>>(head_cd, head_dd, head_ill_in, head_ill_out,
                                            alpha_dd, alpha_cd, alpha_ill_in, alpha_ill_out,
                                            B, L, w, r4, r2, r4, r5);
      }
      ...
      if(w>3){
        dim3 gridDim12(B*(L-w),  (w-1)*(L-w));
        dim3 blockDim12(r5);
        kernel_forward_d3_step1<<<gridDim12, blockDim12>>>(alpha_ill_in, alpha_ill_out, B, L, w, r5);
        dim3 gridDim13(B*(L-w),  (w-1)*(L-w),  max(max(r2, r4), r5));
        kernel_forward_d3_step2<<<gridDim13, blockDim12>>>(head_cd, head_dd, head_ill_in, head_ill_out,
                                                           alpha_dd, alpha_cd, alpha_ill_in, alpha_ill_out,
                                                           B, L, w, r5, r2, r4, r5);
      }
...

{
     ...
     for(int w=L-2; w>1; w--){
       ...
       if(w>3){
            dim3 gridDim4(B*(L-w),  (w-1)*(L-w), max(max(r2, r4), r5));
            dim3 blockDim4(r5);
            kernel_backward_d3_step2<<<gridDim4, blockDim4>>>(head_cd, head_cd_grd, head_dd, head_dd_grd,
                                                              head_ill_in, head_ill_in_grd, head_ill_out, head_ill_out_grd,
                                                              alpha_dd, alpha_cd, alpha_ill_in, alpha_ill_out,
                                                              B, L, w, r5, r2, r4, r5);

            dim3 gridDim44(B*(L-w),  (w-1)*(L-w));
            kernel_backward_d3_step1<<<gridDim44, blockDim4>>>(alpha_ill_in, alpha_ill_out, B, L, w, r5);
       }

        if(w>2){
            dim3 gridDim4(B*(L-w),  (w-1)*(L-w), max(max(r2, r4), r5));
            dim3 blockDim4(r4);
            kernel_backward_d2_step2<<<gridDim4, blockDim4>>>(head_cd, head_cd_grd, head_dd, head_dd_grd,
                                                              head_ill_in, head_ill_in_grd, head_ill_out, head_ill_out_grd,
                                                              alpha_dd, alpha_cd, alpha_ill_in, alpha_ill_out, B, L, w, r4, r2, r4, r5);
            dim3 gridDim44(B*(L-w),  (w-1)*(L-w));
            kernel_backward_d2_step1<<<gridDim44, blockDim4>>>(alpha_dc, alpha_dd, B, L, w, r4);
       }

       dim3 gridDim3(B*(L-w),  (w-1)*(L-w), max(max(r2, r4), r5));
       dim3 blockDim3(r3);
       kernel_backward_d1<<<gridDim3, blockDim3>>>(head_cd, head_cd_grd,
                                                   head_dd, head_dd_grd,
                                                   head_ill_in, head_ill_in_grd, head_ill_out, head_ill_out_grd,
                                                   alpha_ld, alpha_rd, alpha_cd, alpha_dd, alpha_ill_in, alpha_ill_out,
                                                   B, L, w, r3, r2, r4, r5);
