                                ...
                                const int B, const int L, const int width, const int m, const int p)
{
    ...
    float tmp_result = logf(0);
    float right_unary = s[(start+1) * L * m + end * m + s_B];
    for(int sc=s_B; sc<p; sc+=m){
        shared[sc] = unary[b_idx*(L-1)*p + (start) *p + sc];
    }
    __syncthreads();
    for (int sc=0; sc<p; sc++)
    {
        tmp_result = logsumexp(tmp_result, shared[sc]  + right_unary + binary[s_A*(mp)*(mp) + (m+sc)*mp + s_B]);
    }
    __syncthreads();

    for (int split = start+2; split < start+width-1; split++)
    {
         float left = s[start * L * m + split * m + s_B];
         shared[s_B] = s[split * L * m + end * m + s_B];
         __syncthreads();
         for(int sc=0; sc<m; sc++){
                 tmp_result = logsumexp(tmp_result, left + shared[sc] + binary[s_A*(mp)*(mp) + s_B*mp + sc]);
         }
         __syncthreads();
    }

    float left = s[start * L * m + (start+width-1) * m + s_B];
    for(int sc = s_B; sc < p; sc+=m){
        shared[sc] = unary[b_idx*(L-1)*p + (start+width-1) *p + sc];
    }
    __syncthreads();
    for (int sc=0; sc<p; sc++)
    {
        tmp_result = logsumexp(tmp_result, left +  shared[sc] + binary[s_A*(mp)*(mp) + s_B*mp +m+sc]);
    }
                               ...
                               const int B, const int L, const int width, const int m, const int p, const int d)
{
    ...
    for (int split = start+1; split < start+width-1; split++)
    {
         ...
         {
              float left =  s_d[start * L3 + split * L2 + (split2) * L1 + end*d + s_B];

              if(split2 == split+1){
                    for(int ss=s_B; ss < p; ss+=d){
                        shared[ss] = unary[split * p + ss];
                    }
                    __syncthreads();
                    for(int sc=0; sc<p;sc+=1)
                    {
                        tmp_result = logsumexp(tmp_result, left + shared[sc] + binary[s_B*mp + (m+sc)]);
                    }

              }
              else{
                  for(int ss=s_B; ss < m; ss+=d){
                        shared[ss] = s[split*L*m + split2*m + ss];
                   }
                   __syncthreads();
                   for(int sc=0; sc<m;sc+=1)
                   {
                      tmp_result = logsumexp(tmp_result, left + shared[sc] + binary[s_B*mp + (sc)]);
                   }
              }
            __syncthreads();
         }
                               ...
                               const int B, const int L, const int width, const int m, const int p, const int d)
{
    ...
    const F *__restrict__ const s = alpha + b_idx * L * L * m;
    const F *__restrict__ const s_d = alpha_d + b_idx * L * L * L * L * d;
    const F *__restrict__ const binary = binary_rule_d + b_idx * d * d * mp * 4 + s_A * d * mp * 4;
    const F *__restrict__ const binary_c = binary_rule_dc + b_idx * d * mp * mp + s_A * mp * mp;
    const F *__restrict__ const unary = unary_rule + b_idx*(L-1)*p;
    ...
    if(gap_start==start+1){
       ...
       if(gap_end==end-1){
           for(int ss=s_B; ss<p; ss+=d)
           {
               float right = unary[b_idx*(L-1)*p+ (end-1)*p + ss];
               for(int sc=0;sc<p;sc++){
                  tmp_result = logsumexp(tmp_result, left[ss] + right[sc] + binary_c[(ss+m)*mp+ (sc+m)]);
              }
           }
       }
       else{
          for(int ss=s_B; ss<m; ss+=d){
            float right =  s[gap_end * L*m + end*m + ss];
            for(int sc=0; sc<p; sc++){
                tmp_result = logsumexp(tmp_result, shared[sc] + right +  binary_c[s_A*mp*mp+ (sc+m)*mp+ ss]);
            }
          }
