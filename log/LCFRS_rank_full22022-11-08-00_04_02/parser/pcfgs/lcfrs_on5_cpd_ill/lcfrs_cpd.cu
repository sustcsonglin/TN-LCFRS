...
template <typename F>
__global__ void kernel_forward_ill(  F *__restrict__  head_rule,
                                 F *__restrict__ alpha,
                                 F *__restrict__ alpha_ill,
                                 int B,   int L,   int width,   int m,    int r)
{
      int b_idx = blockIdx.x;
      int start = blockIdx.y;
    ...
    int end = start + width;
    F *__restrict__   s = alpha_ill + b_idx * L * L * L * L * r;
    int L3 = L * L * L * r;
    int L2 = L * L * r;
    int L1 = L * r;
    ...
    for (int split = start+1; split < start+width-1; split++)
    {
         for(int split2 = split+1; split2 < start+width; split2++)
         {
              for(int split3 = split2+1; split3 < start+width; split3++){
                   tmp_result = logsumexp(tmp_result, s[start*L3 + split*L2 + split2*L1 + split3*r + s_R] + s[split*L3+split2*L2+end*L1+split3*r + s_R] + rule_score);
              }
         }
    }
...
template <typename F>
__global__ void kernel_backward_ill(  F *__restrict__  head_rule,
                                      F *__restrict__  head_rule_grd,
                                      F *__restrict__ alpha,
                                      F *__restrict__ alpha_ill,
                                      int B,   int L,   int width,   int m,    int r)
{
    ...
    for (int split = start+1; split < start+width-1; split++)
    {
         ...
         {
              for(int split3 = split2+1; split3 < start+width; split3++){
                   float tmp = exp( s[start*L3 + split*L2 + split2*L1 + split3*r + s_R] + s[split*L3+split2*L2+end*L1+split3*r + s_R] + rule_score - parent_inside) * parent_grd;
                   rule_score_grd += tmp;
                   atomicAdd(s + split * L3 + start * L2 + split2*L1 + split3*r + s_R, tmp);
                   atomicAdd(s + split2 * L3 + split * L2 +  end*L1 + split3*r + s_R, tmp);
              }
         }
    }
    atomicAdd(head_rule_grd + b_idx*m*r + s_A*r + s_R, rule_score_grd);
...
template <typename F>
__global__ void kernel_forward_dr2(  F *__restrict__   dd1,
                                       F *__restrict__   dd2,
                                       F *__restrict__ alpha_d,
                                       F *__restrict__ alpha_dd,
                                       int B,   int L,   int width,   int d,   int r)
{
      ...
      result[s_B] = inside + dd1[b_idx*d*r + s_B*r + s_R];
      result2[s_B] = inside + dd2[b_idx*d*r + s_B*r + s_R];
      __syncthreads();
      ...
      for(int i=0;i<d;i++){
       ...
       alpha_dd[b_idx*L*L*L*L*r + start*L*L*L*r + gap_start*L*L*r + gap_end*L*r + end*r + s_R] = final_result;
       alpha_dd[b_idx*L*L*L*L*r + start*L*L*L*r + gap_start*L*L*r + end*L*r + gap_end*r + s_R] = final_result2;
      }
}
...
template <typename F>
__global__ void kernel_backward_dr2(  F *__restrict__   dd1,
                                      F *__restrict__   dd2,
                                F *__restrict__   dd1_grd,
                                F *__restrict__   dd2_grd,
                                F *__restrict__ alpha_d,
                                F *__restrict__ alpha_dd,
                                int B,   int L,   int width,   int d,   int r)
{
      ...
      float parent_grd = alpha_dd[b_idx*L*L*L*L*r + gap_start*L*L*L*r + start*L*L*r + gap_end*L*r + end*r + s_R];
      float inside = alpha_d[b_idx*L*L*L*L*d + start*L*L*L*d + gap_start*L*L*d + gap_end*L*d + end*d + s_B];
      float tmp = exp(inside + dd1[b_idx*d*r + s_B*r + s_R] - parent_inside)*parent_grd;
      atomicAdd(dd1_grd + b_idx*d*r + s_B*r + s_R, tmp);

      parent_inside = alpha_dd[b_idx*L*L*L*L*r + start*L*L*L*r + gap_start*L*L*r + end*L*r + gap_end*r + s_R];
      parent_grd = alpha_dd[b_idx*L*L*L*L*r + gap_start*L*L*L*r + start*L*L*r + end*L*r + gap_end*r + s_R];
      float tmp2 = exp(inside + dd2[b_idx*d*r + s_B*r + s_R] - parent_inside) * parent_grd;
      atomicAdd(dd2_grd + b_idx*d*r + s_B*r + s_R, tmp2);

      atomicAdd(alpha_d + b_idx*L*L*L*L*d + gap_start*L*L*L*d + start*L*L*d + gap_end*L*d + end*d + s_B, tmp+tmp2);
}
                   ...
                   int B, int L, int m, int p, int d, int r1, int r2, int r3, int r4, int r5)
{
    ...
    for(int w=2; w<L; w++){
      ...
      if (w>=4){
        dim3 gridDim21(B, L-w, m);
        dim3 blockDim21(r5);
        kernel_forward_ill<<<gridDim21, blockDim21>>>(head_c3, alpha, alpha_d_ill, B, L, w, m, r5);
      }
      ...
      if(w<L-1){
        ...
        dim3 gridDim555(B*(L-w),  (w-1)*(L-w), r5);
        dim3 blockDim555(d);
        kernel_forward_dr2<<<gridDim555, blockDim555>>>(left_d_ill, right_d_ill, alpha_d, alpha_d_ill, B, L, w, d, r5);
      }
    }
}
                   ...
                   int B, int L, int m, int p, int d, int r1, int r2, int r3, int r4, int r5)
{
     for(int w=L-1; w>1; w--){
        ...
        dim3 gridDim555(B*(L-w),  (w-1)*(L-w), r5);
        dim3 blockDim555(d);
        kernel_backward_dr2<<<gridDim555, blockDim555>>>(left_d_ill, right_d_ill, left_d_ill_grd, right_d_ill_grd, alpha_d, alpha_d_ill, B, L, w, d, r5);
    ...
    if (w>=4){
        dim3 gridDim2(B, L-w, m);
        dim3 blockDim2(r5);
        kernel_backward_ill<<<gridDim2, blockDim2>>>(head_c3, head_c3_grd, alpha, alpha_d_ill, B, L, w, m, r5);
    }
                                   ...
                                   )
{
     ...
     for(int split = start + 1; split < end; split+=1){
        ...
        for(int split2 =split+1;split2<end;split2+=1){
            ...
            for(int split3 = split2+1; split3 < end; split3+=1){
                tmp = s_d[start*L3+split*L2+split2*L+split3] + s_d[split*L3+split2*L2+split3*L+end];
                if(tmp > tmp_max){
                    tmp_max = tmp;
                    tmp_idx = split*L2+split2*L+split3;
                }
            }
        }
     }
