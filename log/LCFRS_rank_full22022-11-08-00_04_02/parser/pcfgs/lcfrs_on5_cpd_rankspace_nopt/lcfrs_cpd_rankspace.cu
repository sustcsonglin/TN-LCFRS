                                ...
                                int B,  int L,  int width,   int d,   int r)
{
 ...
 __syncthreads();
    ...
    {
        tmp_result = logsumexp(tmp_result, s_d[split*L3 + gap_start*L2 + gap_end*L1 + end*r + s_R] + s[start*L*r*4 + split*r*4 +  0*r + s_R]);
        tmp_result = logsumexp(tmp_result, s_d[start*L3 + split*L2 + gap_end*L1 + end*r + s_R] + s[split*L*r*4 + gap_start*r*4 + 1*r + s_R]);
    }

    for(int split = gap_end+1; split <end; split++){
        tmp_result = logsumexp(tmp_result, s_d[start*L3 + gap_start*L2 + split*L1 + end*r + s_R] + s[gap_end*L*r*4 + split*r*4 + 2*r + s_R]);
        tmp_result = logsumexp(tmp_result, s_d[start*L3 + gap_start*L2 + gap_end*L1 + split*r + s_R] + s[split*L*r*4 + end*r*4 + 3*r + s_R]);
    }
    ...
    alpha_dd[b_idx*L*L*L*L*d + start*L*L*L*d + gap_start*L*L*d + gap_end*L*d + end*d + s_A] = logsumexp( alpha_dd[b_idx*L*L*L*L*d + start*L*L*L*d + gap_start*L*L*d + gap_end*L*d + end*d + s_A],final_result2);
    }
}
                                      ...
                                      int B,   int L,   int width,   int m,   int p,   int r)
{
    ...
    result[s_B] = inside + cc[b_idx*(m)*r + s_B*r + s_R];
    __syncthreads();
                                      ...
                                      int B,   int L,   int width,   int m,   int p,   int r)
{
    ...
    float tmp = exp(alpha[b_idx*L*L*m + start*L*m + end*m + s_B] + cc[b_idx*(m)*r + s_B*r + s_R] - parent_inside);

    tmp = tmp * parent_grd;
    atomicAdd(alpha + b_idx*L*L*m + end*L*m + start*m + s_B, tmp);
    atomicAdd(cc_grd + b_idx*(m)*r + s_B*r + s_R, tmp);
}
                                      ...
                                      int B,   int L, int m,    int p,   int r)
{
    ...
    __shared__ float result[1000];
    result[s_B] = unary[b_idx*m*(L-1) + start*m+ s_B] + cc[b_idx*(m)*r + (s_B)*r + s_R];
    __syncthreads();
    if(s_B==0){
    float final_result = logf(0);
        for(int i=0;i<m;i++){
                                      ...
                                      int B,   int L, int m,    int p,   int r)
{
    ...
    float parent_inside = alpha_cc[b_idx*L*L*r + start*L*r + end*r + s_R];
    float parent_grd = alpha_cc[b_idx*L*L*r + end*L*r + start*r + s_R];
    float tmp = exp(unary[b_idx*m*(L-1) + start*m+ s_B] + cc[b_idx*(m)*r + (s_B)*r + s_R] - parent_inside);
    tmp = tmp * parent_grd;
    atomicAdd(unary_grd + b_idx*m*(L-1) + start*m+ s_B, tmp);
    atomicAdd(cc_grd +  b_idx*(m)*r + (s_B)*r + s_R, tmp);
}
                                    ...
                                    int B,   int L,  int m, int p, int r)
{
    ...
    float u = unary[b_idx*m*(L-1) + start*m + s_B];
    result1[s_B] = u + cc[b_idx*(m)*r*4 + (s_B)*r*4 + s_R];
    result2[s_B] = u + cc[b_idx*(m)*r*4 + (s_B)*r*4 + 1*r + s_R];
    result3[s_B] = u + cc[b_idx*(m)*r*4 + (s_B)*r*4 + 2*r + s_R];
    result4[s_B] = u + cc[b_idx*(m)*r*4 + (s_B)*r*4 + 3*r + s_R];
    __syncthreads();
                                    ...
                                    int B,   int L,  int m, int p,  int r)
{
    ...
    float tmp = exp(u + cc[b_idx*(m)*r*4 + (s_B)*r*4 + s_R] - parent_inside) * parent_grd;
    atomicAdd(cc_grd + b_idx*(m)*r*4 + (s_B)*r*4 + s_R, tmp);
    u_grd += tmp;
    ...
    tmp = exp(u + cc[b_idx*(m)*r*4 + (s_B)*r*4 + 1*r + s_R]-parent_inside) * parent_grd;
    atomicAdd(cc_grd + b_idx*(m)*r*4 + (s_B)*r*4 + 1*r + s_R, tmp);
    u_grd += tmp;
    ...
    tmp = exp(u + cc[b_idx*(m)*r*4 + (s_B)*r*4 + 2*r + s_R] - parent_inside)*parent_grd;
    atomicAdd(cc_grd + b_idx*(m)*r*4 + (s_B)*r*4 + 2*r + s_R, tmp);
    u_grd += tmp;
    ...
    tmp = exp(u + cc[b_idx*(m)*r*4 + (s_B)*r*4 + 3*r + s_R] - parent_inside)*parent_grd;
    atomicAdd(cc_grd + b_idx*(m)*r*4 + (s_B)*r*4 + 3*r + s_R, tmp);
    u_grd += tmp;
    atomicAdd(unary_grd + b_idx*m*(L-1) + start*m + s_B, u_grd);
}
...
template <typename F>
__global__ void kernel_argmax_d_on6wn(const F *__restrict__ const s_span_d,
                                    F *__restrict__ alpha_c,
                                    F *__restrict__ alpha_d,
                                    int B,  int L,  int width
                               )
