...
void cuda_forward(const float *binary, const float *binary_close,  const float *binary_dc,  const float *binary_d, const float *binary_ill, const float *unary, float *alpha,  float *alpha_d, int B, int L, int m, int p, int d)
{
...
for(int w=3; w<L; w++){
  ...
  dim3 blockDim2(d);
  kernel_forward_close<<<gridDim2, blockDim2>>>(binary_close, binary_ill, unary, alpha, alpha_d, B, L, w, m, p, d);
  if(w<L-1){
...
void cuda_forward_argmax(const float *binary, const float *binary_close,  const float *binary_dc,  const float *binary_d, const float *binary_ill, const float *unary, float *alpha,  float *alpha_d, int B, int L, int m, int p, int d)
{
    dim3 gridDim(B, L-2+1, m);
    dim3 blockDim(p);
...
for(int w=3; w<L; w++){
  ...
  dim3 gridDim2(B, L-w, m);
  dim3 blockDim2(d);
  kernel_forward_close_argmax<<<gridDim2, blockDim2>>>(binary_close, binary_ill, unary, alpha, alpha_d, B, L, w, m, p, d);
  if(w<L-1){
    dim3 gridDim3(B*(L-w),  (w-1)*(L-w), d);
    dim3 blockDim3(d);
