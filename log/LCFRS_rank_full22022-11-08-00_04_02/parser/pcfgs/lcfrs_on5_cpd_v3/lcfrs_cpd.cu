                                ...
                                int B,  int L,  int width,   int d,   int r)
{
    ...
    F *__restrict__   s_d = alpha_dd + b_idx * L * L * L * L * r * 4;
    int L3 = L * L * L * r * 4;
    int L2 = L * L * r * 4;
    int L1 = L * r * 4;
    float tmp_result = logf(0);
    ...
    {
        tmp_result = logsumexp(tmp_result, s_d[split*L3 + gap_start*L2 + gap_end*L1 + end*r*4 + s_R] + head[b_idx*d*r*4 + s_A*r*4 + s_R] + s[start*L*r*4 + split*r*4 + s_R]);
        tmp_result = logsumexp(tmp_result, s_d[start*L3 + split*L2 + gap_end*L1 + end*r*4 + r + s_R] + head[b_idx*d*r*4 + s_A*r*4 + r + s_R] + s[split*L*r*4 + gap_start*r*4 + r + s_R]);
    }

    for(int split = gap_end+1; split <end; split++){
        tmp_result = logsumexp(tmp_result, s_d[start*L3 + gap_start*L2 + split*L1 + end*r*4 + 2*r + s_R] + head[b_idx*d*r*4 + s_A*r*4 + 2*r + s_R] + s[gap_end*L*r*4 + split*r*4 + 2*r + s_R]);
        tmp_result = logsumexp(tmp_result, s_d[start*L3 + gap_start*L2 + gap_end*L1 + split*r*4 + 3*r + s_R] + head[b_idx*d*r*4 + s_A*r*4 + 3*r + s_R] + s[split*L*r*4 + end*r*4 + 3*r + s_R]);
    }
                                ...
                                const int B, const int L, const int width, const int d, const int r)
{
    ...
    for(int split=start+1; split< gap_start; split++)
    {
        float tmp = exp(s_d[split*L3 + gap_start*L2 + gap_end*L1 + end*r*4 + s_R] + head_rule[b_idx*d*r*4 + s_A*r*4 + s_R] + s[start*L*r*4 + split*r*4 + s_R] - parent_inside) * parent_grd;
        atomicAdd(s + split*L*r*4 + start*r*4 + s_R, tmp);
        atomicAdd(s_d + gap_start*L3 + split*L2 + gap_end*L1 + end*r*4 + s_R, tmp);
        atomicAdd(head_rule_grd + b_idx*d*r*4 + s_A*r*4 + s_R, tmp);

        tmp = exp(s_d[start*L3 + split*L2 + gap_end*L1 + end*r*4 + r + s_R] + head_rule[b_idx*d*r*4 + s_A*r*4 + r + s_R] + s[split*L*r*4 + gap_start*r*4 + r + s_R] - parent_inside) * parent_grd;
        atomicAdd(s + gap_start*L*r*4 + split*r*4 + r + s_R , tmp);
        atomicAdd(s_d +  split*L3 + start*L2 + gap_end*L1 + end*r*4 + r + s_R, tmp);
        atomicAdd(head_rule_grd + b_idx*d*r*4 + s_A*r*4 + r + s_R, tmp);
    }

    for(int split=gap_end+1; split <end; split++){
        float tmp = exp(s_d[start*L3 + gap_start*L2 + split*L1 + end*r*4 + 2*r + s_R] + head_rule[b_idx*d*r*4 + s_A*r*4 + 2*r + s_R] + s[gap_end*L*r*4 + split*r*4 + 2*r +s_R] - parent_inside) * parent_grd;
        atomicAdd(s_d + gap_start*L3 + start*L2 + split*L1 + end*r*4 + 2*r + s_R, tmp);
        atomicAdd(s + split*L*r*4 + gap_end*r*4 + 2*r + s_R , tmp);
        atomicAdd(head_rule_grd + b_idx*d*r*4 + s_A*r*4 + 2*r + s_R, tmp);

        tmp = exp(s_d[start*L3 + gap_start*L2 + gap_end*L1 + split*r*4 + 3*r + s_R] +  head_rule[b_idx*d*r*4 + s_A*r*4 + 3*r + s_R] + s[split*L*r*4 + end*r*4 + 3*r + s_R] - parent_inside) * parent_grd;
        atomicAdd(s_d + gap_start*L3 + start*L2 + gap_end*L1 + split*r*4 + 3*r + s_R, tmp);
        atomicAdd(s + end*L*r*4 + split*r*4 + 3*r + s_R, tmp);
        atomicAdd(head_rule_grd + b_idx*d*r*4 + s_A*r*4 + 3*r + s_R, tmp);
    }
...
template <typename F>
__global__ void kernel_forward_dr2(  F *__restrict__   dd,
                                    F *__restrict__ alpha_d,
                                    F *__restrict__ alpha_dd,
                                    int B,   int L,   int width,   int d,   int r)
{
      ...
      float tmp = alpha_d[b_idx*L*L*L*L*d + start*L*L*L*d + gap_start*L*L*d + gap_end*L*d + end*d + s_B];

      result[s_B] = tmp + dd[b_idx*d*r*4 + s_B*r*4 + s_R];
      result2[s_B] = tmp + dd[b_idx*d*r*4 + s_B*r*4 + r + s_R];
      result3[s_B] = tmp + dd[b_idx*d*r*4 + s_B*r*4 + 2*r + s_R];
      result4[s_B] = tmp + dd[b_idx*d*r*4 + s_B*r*4 + 3*r + s_R];
      __syncthreads();
      ...
      alpha_dd[b_idx*L*L*L*L*r*4 + start*L*L*L*r*4 + gap_start*L*L*r*4 + gap_end*L*r*4 + end*r*4 + s_R] = final_result;
      alpha_dd[b_idx*L*L*L*L*r*4 + start*L*L*L*r*4 + gap_start*L*L*r*4 + gap_end*L*r*4 + end*r*4 + r + s_R] = final_result2;
      alpha_dd[b_idx*L*L*L*L*r*4 + start*L*L*L*r*4 + gap_start*L*L*r*4 + gap_end*L*r*4 + end*r*4 + 2*r + s_R] = final_result3;
      alpha_dd[b_idx*L*L*L*L*r*4 + start*L*L*L*r*4 + gap_start*L*L*r*4 + gap_end*L*r*4 + end*r*4 + 3*r + s_R] = final_result4;
      }
}

template <typename F>
__global__ void kernel_backward_dr2(  F *__restrict__   dd,
                                    ...
                                    int B,   int L,   int width,   int d,   int r)
{
      ...
      float inside = alpha_d[b_idx*L*L*L*L*d + start*L*L*L*d + gap_start*L*L*d + gap_end*L*d + end*d + s_B];
      float parent_inside = alpha_dd[b_idx*L*L*L*L*r*4 + start*L*L*L*r*4 + gap_start*L*L*r*4 + gap_end*L*r*4 + end*r*4 + s_R];
      float parent_grd = alpha_dd[b_idx*L*L*L*L*r*4 + gap_start*L*L*L*r*4 + start*L*L*r*4 + gap_end*L*r*4 + end*r*4 + s_R];
      float tmp = exp(inside + dd[b_idx*d*r*4 + s_B*r*4 + s_R] - parent_inside) * parent_grd;
      tmp_grd+=tmp;
      atomicAdd(dd_grd + b_idx*d*r*4 + s_B*r*4 + s_R, tmp);

      parent_inside = alpha_dd[b_idx*L*L*L*L*r*4 + start*L*L*L*r*4 + gap_start*L*L*r*4 + gap_end*L*r*4 + end*r*4 + r + s_R];
      parent_grd = alpha_dd[b_idx*L*L*L*L*r*4 + gap_start*L*L*L*r*4 + start*L*L*r*4 + gap_end*L*r*4 + end*r*4 + r + s_R];
      tmp =  exp(inside + dd[b_idx*d*r*4 + s_B*r*4 + r + s_R] - parent_inside)*parent_grd;
      tmp_grd+=tmp;
      atomicAdd(dd_grd + b_idx*d*r*4 + s_B*r*4 + r +s_R, tmp);

      parent_inside = alpha_dd[b_idx*L*L*L*L*r*4 + start*L*L*L*r*4 + gap_start*L*L*r*4 + gap_end*L*r*4 + end*r*4 + 2*r + s_R];
      parent_grd = alpha_dd[b_idx*L*L*L*L*r*4 + gap_start*L*L*L*r*4 + start*L*L*r*4 + gap_end*L*r*4 + end*r*4 + 2*r + s_R];
      tmp =  exp(inside + dd[b_idx*d*r*4 + s_B*r*4 + 2*r + s_R] - parent_inside)*parent_grd;
      tmp_grd+=tmp;
      atomicAdd(dd_grd + b_idx*d*r*4 + s_B*r*4 + 2*r +s_R, tmp);

      parent_inside = alpha_dd[b_idx*L*L*L*L*r*4 + start*L*L*L*r*4 + gap_start*L*L*r*4 + gap_end*L*r*4 + end*r*4 + 3*r + s_R];
      parent_grd = alpha_dd[b_idx*L*L*L*L*r*4 + gap_start*L*L*L*r*4 + start*L*L*r*4 + gap_end*L*r*4 + end*r*4 + 3*r + s_R];
      tmp =  exp(inside + dd[b_idx*d*r*4 + s_B*r*4 + 3*r + s_R] - parent_inside)*parent_grd;
      tmp_grd+=tmp;
      atomicAdd(dd_grd + b_idx*d*r*4 + s_B*r*4 + 3*r +s_R, tmp);

      atomicAdd(alpha_d + b_idx*L*L*L*L*d + gap_start*L*L*L*d + start*L*L*d + gap_end*L*d + end*d + s_B, tmp_grd);
                   ...
                   int B, int L, int m, int p, int d, int r1, int r2, int r3, int r4)
{
    ...
    for(int w=2; w<L; w++){
      ...
      if(w<L-1){
        ...
        dim3 blockDim9(m);
        kernel_forward_dmr<<<gridDim9, blockDim9>>>(dc, alpha, alpha_dc, B, L, w, m, p, r4);
        ...
        dim3 blockDim5(d);
        kernel_forward_dr2<<<gridDim5, blockDim5>>>(dd, alpha_d, alpha_dd, B, L, w, d, r4);
                   ...
                   int B, int L, int m, int p, int d, int r1, int r2, int r3, int r4)
{
     for(int w=L-1; w>1; w--){
        ...
        dim3 gridDim5(B*(L-w), (w-1)*(L-w), r4);
        dim3 blockDim5(d);
        kernel_backward_dr2<<<gridDim5, blockDim5>>>(dd, dd_grd, alpha_d, alpha_dd, B, L, w, d, r4);
        ...
        dim3 gridDim9(B, L-w, r4);
        dim3 blockDim9(m);
        kernel_backward_dmr<<<gridDim9, blockDim9>>>(dc, dc_grd, alpha, alpha_dc, B, L, w, m, p, r4);
