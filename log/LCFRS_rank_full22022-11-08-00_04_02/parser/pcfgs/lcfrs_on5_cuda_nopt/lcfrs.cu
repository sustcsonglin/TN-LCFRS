#include <stdio.h>
#include <cuda_runtime.h>


template <typename scalar_t>
__device__ scalar_t logsumexp(scalar_t a, scalar_t b) {
  const scalar_t m = max(a, b);
  return log(exp(a - m) + exp(b - m)) + m;
}



template <typename F>
__global__ void kernel_forward(const F *__restrict__ const binary_rule,
                                const F *__restrict__ const unary,
                                F *__restrict__ alpha,
                                const int B, const int L, const int width, const int m, const int p)
{
    const int b_idx = blockIdx.x;
    const int start = blockIdx.y;
    const int s_A = blockIdx.z;
    const int s_B = threadIdx.x;
    const int mp = m+p;

    if(start+width >= L){
        return;
    }

    __shared__ float result[200];
    __shared__ float left[200];
    __shared__ float right[200];

    int end = start + width;
    const F *__restrict__ const s = alpha + b_idx * L * L * m;
    const F *__restrict__ const binary = binary_rule + b_idx * m * mp * mp;

    float tmp_result = logf(0);

    right[s_B] = s[(start+1) * L * m + end * m + s_B];
    __syncthreads();


    for (int split = start+1; split < start+width; split++)
    {
         left[s_B] =  s[start * L * m + split * m + s_B];
         right[s_B] = s[split * L * m + end * m + s_B];
         __syncthreads();
         for(int sc=0; sc<m; sc++){
              tmp_result = logsumexp(tmp_result, left[s_B] + right[sc] + binary[s_A*(m)*(m) + s_B*m + sc]);
         }
         __syncthreads();
    }

    if(s_B==0){
    float final_result = logf(0);
    for(int i=0;i<m;i++){
        final_result = logsumexp(final_result, result[i]);
    }
    alpha[b_idx*L*L*m+start*L*m+end*m+s_A] = final_result;
  }
}


template <typename F>
__global__ void kernel_forward_argmax(const F *__restrict__ const binary_rule,
                                const F *__restrict__ const unary,
                                F *__restrict__ alpha,
                                const int B, const int L, const int width, const int m, const int p)
{
    const int b_idx = blockIdx.x;
    const int start = blockIdx.y;
    const int s_A = blockIdx.z;
    const int s_B = threadIdx.x;

    if(start+width >= L){
        return;
    }

    __shared__ float result[200];
    __shared__ int result_idx[200];
    __shared__ float left[200];
    __shared__ float right[200];

    int end = start + width;
    const F *__restrict__ const s = alpha + b_idx * L * L * m;
    const F *__restrict__ const binary = binary_rule + b_idx * m * m * m;

    float tmp_result = logf(0);
    float tmp = 0;
    int tmp_idx = -1;

    for (int split = start+1; split < start+width; split++)
    {
         left[s_B] =  s[start * L * m + split * m + s_B];
         right[s_B] = s[split * L * m + end * m + s_B];
         __syncthreads();
         for(int sc=0; sc<m; sc++){
                 tmp = left[s_B] + right[sc] + binary[s_A*(m)*(m) + s_B*m + sc];
                 if(tmp > tmp_result){
                    tmp_result = tmp;
                    tmp_idx = (split)*mp*mp + s_B*mp + sc;
                 }
         }
         __syncthreads();
    }

    result[s_B] = tmp_result;
    result_idx[s_B] = tmp_idx;
    __syncthreads();

    if(s_B==0){
    float final_result = logf(0);
    float final_idx = -1;
    for(int i=0;i<m;i++){
        if(result[i] > final_result){
            final_result = result[i];
            final_idx = result_idx[i];
        }
    }

    alpha[b_idx*L*L*m+start*L*m+end*m+s_A] = final_result;
    alpha[b_idx*L*L*m+end*L*m +start*m + s_A] = final_idx;
  }
}




template <typename F>
__global__ void kernel_forward_close(const F *__restrict__ const binary_rule,
                                    const F *__restrict__ const binary_rule_ill,
                                 const F *__restrict__ const unary_rule,
                                F *__restrict__ alpha,
                                F *__restrict__ alpha_d,
                               const int B, const int L, const int width, const int m, const int p, const int d)
{

    const int b_idx = blockIdx.x;
    const int start = blockIdx.y;
    const int s_A = blockIdx.z;
    const int s_B = threadIdx.x;
    const int mp = m+p;

    if(start+width >= L){
        return;
    }

    __shared__ float result[200];
    __shared__ float left[200];
    __shared__ float right[200];

    int end = start + width;
    const F *__restrict__ const s = alpha + b_idx * L * L * m;
    const F *__restrict__ const s_d = alpha_d + b_idx * L * L * L * L * d;
    const F *__restrict__ const binary = binary_rule + b_idx * m * m * m + s_A * m * m;
    const F *__restrict__ const binary_ill = binary_rule_ill + b_idx * m * d * d + s_A * d * d;
    const F *__restrict__ const unary = unary_rule + b_idx * (L-1) * p;

    const int L3 = L * L * L * d;
    const int L2 = L * L * d;
    const int L1 = L * d;

    float tmp_result = logf(0);

    for (int split = start+1; split < start+width-1; split++)
    {
         for(int split2 = split+1; split2 < start+width; split2++)
         {

              left[s_B] = s_d[start * L3 + split * L2 + (split2) * L1 + end*d + s_B];
              __syncthreads();
              if(split2 == split+1){
                    for(int ss=s_B; ss < p; ss+=d){
                        right[ss] = unary[split * p + ss];
                    }
                    __syncthreads();
                    for(int sc=0; sc<p;sc+=1)
                    {
                        tmp_result = logsumexp(tmp_result, left[s_B] + right[sc] + binary[s_B*mp + (m+sc)]);
                    }

              }
              else{
                    for(int ss=s_B; ss<m; ss+=d){
                        right[ss] = s[split*L*m + split2*m + ss];
                    }
                    __syncthreads();
                    for(int sc=0; sc<m;sc+=1)
                    {
                        tmp_result = logsumexp(tmp_result, left[s_B] + right[sc] + binary[s_B*mp + (sc)]);
                    }
              }
            __syncthreads();

            for(int split3 = split2+1; split3 < start+width; split3+=1){
                left[s_B] = s_d[start*L3 + split*L2 + split2*L1 + split3*d + s_B];
                right[s_B] = s_d[split*L3 + split2*L2 + split3*L1 + end*d + s_B];
                __syncthreads();
                for(int sd =0; sd<d; sd++){
                    tmp_result = logsumexp(tmp_result, left[s_B] + right[sd] + binary_ill[ s_B*d + sd]);
                }
                __syncthreads();
            }
         }
    }

    result[s_B] = tmp_result;
    __syncthreads();

    if(s_B==0){
        float final_result = logf(0);
        for(int i=0;i<d;i++){
            final_result = logsumexp(final_result, result[i]);
        }
        alpha[b_idx*L*L*m+start*L*m+(end)*m+s_A] = logsumexp(alpha[b_idx*L*L*m+start*L*m+(end)*m+s_A], final_result);
    }
}





template <typename F>
__global__ void kernel_forward_close_argmax(const F *__restrict__ const binary_rule,
                                 const F *__restrict__ const binary_rule_ill,
                                 const F *__restrict__ const unary_rule,
                                F *__restrict__ alpha,
                                F *__restrict__ alpha_d,
                               const int B, const int L, const int width, const int m, const int p, const int d)
{

    const int b_idx = blockIdx.x;
    const int start = blockIdx.y;
    const int s_A = blockIdx.z;
    const int s_B = threadIdx.x;
    const int mp = m+p;

    if(start+width >= L){
        return;
    }

    __shared__ float result[200];
    __shared__ float result_idx[200];
    __shared__ float left[200];
    __shared__ float right[200];

    int end = start + width;
    const F *__restrict__ const s = alpha + b_idx * L * L * m;
    const F *__restrict__ const s_d = alpha_d + b_idx * L * L * L * L * d;
    const F *__restrict__ const binary = binary_rule + b_idx * m * d * mp + s_A * d *mp;
    const F *__restrict__ const binary_ill = binary_rule + b_idx * m * d * d + s_A * d * d;
    const F *__restrict__ const unary = unary_rule + b_idx * (L-1) * p;

    const int L3 = L * L * L * d;
    const int L2 = L * L * d;
    const int L1 = L * d;

    float tmp_result = logf(0);
    float tmp = 0;
    float tmp_idx = -1;

    for (int split = start+1; split < start+width-1; split++)
    {
         for(int split2 = split+1; split2 < start+width; split2++)
         {
              left[s_B] = s_d[start * L3 + split * L2 + (split2) * L1 + end*d + s_B];
              __syncthreads();
              if(split2 == split+1){
                    for(int ss=s_B; ss < p; ss+=d){
                        right[ss] = unary[split * p + ss];
                    }
                    __syncthreads();
                    for(int sc=0; sc<p;sc+=1)
                    {
                        tmp = left[s_B] + right[sc] + binary[s_B*mp + (m+sc)];
                        if(tmp > tmp_result){
                            tmp_result=tmp;
                            tmp_idx = -(split*L1*mp + split2*d*mp + s_B*mp + (sc+m));
                        }
                    }
              }
              else{
                    for(int ss=s_B; ss<m; ss+=d){
                        right[ss] = s[split*L*m + split2*m + ss];
                    }
                    __syncthreads();
                    for(int sc=0; sc<m;sc+=1)
                    {
                        tmp =  left[s_B] + right[sc] + binary[s_B*mp + (sc)];
                        if(tmp > tmp_result){
                            tmp_result=tmp;
                            tmp_idx = -(split*L1*mp +split2*d*mp + s_B*mp + (sc));
                        }

                    }
              }
              __syncthreads();

              for(int split3 = split2+1; split3 < start+width; split3+=1){
                left[s_B] = s_d[start*L3 + split*L2 + split2*L1 + split3*d + s_B];
                right[s_B] = s_d[split*L3 + split2*L2 + split3*L1 + end*d + s_B];
                __syncthreads();
                for(int sd =0; sd<d; sd++){
                    tmp =  left[s_B] + right[sd] + binary_ill[ s_B*d + sd];
                    if(tmp > tmp_result){
                        tmp_result = tmp;
                        tmp_idx = 10000000 + (split*L2 *d +split2*L1 *d + split3*d*d + s_B*d + sd);
                    }
                }
                __syncthreads();
            }


         }
    }

    result[s_B] = tmp_result;
    result_idx[s_B] = tmp_idx;
    __syncthreads();

    if(s_B==0){
        float final_result = logf(0);
        float final_idx = -1;
        for(int i=0;i<d;i++){
            if(result[i] > final_result){
                final_result = result[i];
                final_idx = result_idx[i];
            }
        }

        float prev_best = alpha[b_idx*L*L*m+start*L*m+(end)*m+s_A];
        if(final_result > prev_best){
            alpha[b_idx*L*L*m+start*L*m+(end)*m+s_A] = final_result;
            alpha[b_idx*L*L*m+end*L*m+(start)*m+s_A] = final_idx;
        }
    }
}


template <typename F>
__global__ void kernel_backward_close(const F *__restrict__ const binary_rule,
                                const F *__restrict__ const binary_rule_ill,
                                 const F *__restrict__ const unary_rule,
                                 F *__restrict__ binary_rule_grd,
                                 F *__restrict__ binary_rule_ill_grd,
                                 F *__restrict__ unary_rule_grd,
                                 F *__restrict__ alpha,
                                 F *__restrict__ alpha_d,
                                const int B, const int L, const int width, const int m, const int p, const int d)
{
    const int b_idx = blockIdx.x;
    const int start = blockIdx.y;
    const int s_A = blockIdx.z;
    const int s_B = threadIdx.x;
    const int mp = m+p;

    if(start+width >= L){
        return;
    }

    __shared__ float result[200];
    __shared__ float left[200];
    __shared__ float right[200];

    int end = start + width;
    F *__restrict__ s = alpha + b_idx * L * L * m;
    F *__restrict__ s_d = alpha_d + b_idx * L * L * L * L * d;
    const F *__restrict__ const binary = binary_rule + b_idx * m * d * mp + s_A * d *mp;
    const F *__restrict__ const binary_ill = binary_rule_ill + b_idx * m * d * d + s_A * d * d;
    const F *__restrict__ const unary = unary_rule + b_idx * (L-1) * p;
    F *__restrict__ binary_grd = binary_rule_grd + b_idx * m * d * mp + s_A * d *mp;
    F *__restrict__ binary_ill_grd = binary_rule_grd + b_idx * m * d * mp + s_A * d *mp;
    F *__restrict__ unary_grd = unary_rule_grd + b_idx * (L-1) * p;

    const int L3 = L * L * L * d;
    const int L2 = L * L * d;
    const int L1 = L * d;

    float tmp_result = logf(0);
    float tmp = 0;
    float parent_inside = s[start*L*m + end*m + s_A];
    float parent_grd = s[end*L*m + start*m + s_A];

    for (int split = start+1; split < start+width-1; split++)
    {
         for(int split2 = split+1; split2 < start+width; split2++)
         {
              left[s_B] = s_d[start * L3 + split * L2 + (split2) * L1 + end*d + s_B];
              __syncthreads();
              if(split2 == split+1){
                    for(int ss=s_B; ss < p; ss+=d){
                        right[ss] = unary[split * p + ss];
                    }
                    __syncthreads();
                    for(int sc=0; sc<p;sc+=1)
                    {
                        tmp = exp(left[s_B] + right[sc] + binary[s_B*mp + (m+sc)] - parent_inside) * parent_grd;
                        atomicAdd(binary_grd + s_B*mp + (m+sc), tmp);
                        atomicAdd( s_d + split * L3 + start * L2 + (split2) * L1 + end*d + s_B, tmp);
                        atomicAdd(unary_grd + split * p + sc, tmp);
                    }

              }
              else{
                    for(int ss=s_B; ss<m; ss+=d){
                        right[ss] = s[split*L*m + split2*m + ss];
                    }
                    __syncthreads();
                    for(int sc=0; sc<m;sc+=1)
                    {
                        tmp = exp(left[s_B] + right[sc] + binary[s_B*mp + (sc)] - parent_inside) * parent_grd;
                        atomicAdd(binary_grd +  s_B*mp + sc, tmp);
                        atomicAdd(s_d + split * L3 + start * L2 + (split2) * L1 + end*d + s_B, tmp);
                        atomicAdd(s + split2*L*m + split*m + sc, tmp);
                   }
              }
              __syncthreads();
              for(int split3 = split2+1; split3 < start+width; split3+=1){
                left[s_B] = s_d[start*L3 + split*L2 + split2*L1 + split3*d + s_B];
                right[s_B] = s_d[split*L3 + split2*L2 + split3*L1 + end*d + s_B];
                __syncthreads();
                for(int sd =0; sd<d; sd++){
                    tmp = exp(left[s_B] + right[sd] + binary_ill[ s_B*d + sd] -parent_inside) * parent_grd;
                    atomicAdd(s_d + split * L3 + start * L2 + split2*L1 + split3*d + s_B, tmp);
                    atomicAdd(s_d + split2 * L3 + split * L2 + split3*L1 + end*d + sd, tmp);
                    atomicAdd(binary_ill_grd + s_B*d + sd, tmp);
                }
                __syncthreads();
            }

         }
    }
}



template <typename F>
__global__ void kernel_forward_d(const F *__restrict__ const binary_rule_d,
                                    const F *__restrict__ const binary_rule_dc,
                                    const F *__restrict__ const binary_rule_cd,
                                 const F *__restrict__ const unary,
                                F *__restrict__ alpha,
                              F *__restrict__ alpha_d,
                               const int B, const int L, const int width, const int m, const int p, const int d)
{
    const int b_idx = blockIdx.x / (L-width);
    const int start = blockIdx.x % (L-width);
    const int gap_start_minus_start = (blockIdx.y) / (L-width);
    const int gap_width =  ((blockIdx.y) % ( L-width)) + 1;
    const int end = start + width + gap_width;
    if(end>=L){
        return;
    }
    const int s_A = (blockIdx.z);
    const int s_B = threadIdx.x;
    const int mp = m+p;
    const int mpd = d* (m+p);
    const int gap_start = start + gap_start_minus_start + 1;
    const int gap_end = gap_start + gap_width;
    if(gap_start - start > width-1)
    {
        return;
    }

    __shared__ float result[200];
    __shared__ float left[200];
    __shared__ float right[200];

    const F *__restrict__ const s = alpha + b_idx * L * L * m;
    const F *__restrict__ const s_d = alpha_d + b_idx * L * L * L * L * d;
    const F *__restrict__ const binary = binary_rule_d + b_idx * d * d * mp * 4;
    const F *__restrict__ const binary_c = binary_rule_dc + b_idx * d * mp * mp;
    const F *__restrict__ const binary_cd = binary_rule_cd + b_idx * mp * d * mp;
    const int L3 = L * L * L * d;
    const int L2 = L * L * d;
    const int L1 = L * d;

    float tmp_result = logf(0);

    __syncthreads();

    if(gap_start==start+1){
       for(int ss=s_B; ss<p; ss+=d){
          left[ss] = unary[b_idx*(L-1)*p+ start*p + ss];
       }
       __syncthreads();

       if(gap_end==end-1){
           for(int ss=s_B; ss<p; ss+=d)
           {
                right[ss] = unary[b_idx*(L-1)*p+ (end-1)*p + ss];
           }
           __syncthreads();

           for(int ss=s_B; ss<p; ss+=d)
           {
               right[ss] = unary[b_idx*(L-1)*p+ (end-1)*p + ss];
               for(int sc=0;sc<p;sc++){
                  tmp_result = logsumexp(tmp_result, left[ss] + right[sc] + binary_c[s_A*mp*mp + (ss+m)*mp+ (sc+m)]);
              }
           }
       }

       else{
          for(int ss=s_B; ss<m; ss+=d)
          {
             right[ss] = s[ gap_end * L*m + end*m + ss];
          }
         __syncthreads();
          for(int sc=0;sc<p;sc++)
          {
             for(int ss=s_B;ss<m;ss+=d){
                tmp_result = logsumexp(tmp_result, left[sc]+right[ss]+binary_c[s_A*mp*mp+ (sc+m)*mp+ ss]);
             }
          }
       }
    }
    else{
      for(int ss=s_B; ss<m; ss+=d)
      {
         left[ss] = s[start * L*m + gap_start*m + ss];
      }
      __syncthreads();
       if(gap_end==end-1){
           for(int ss=s_B; ss<p; ss+=d)
           {
                right[ss] = unary[ b_idx*(L-1)*p+ (end-1)*p + ss];
           }
            __syncthreads();
           for(int sc=0;sc<p;sc++){
             for(int ss=s_B;ss<m;ss+=d){
                tmp_result = logsumexp(tmp_result, left[ss]+right[sc]+binary_c[s_A*mp*mp+ (ss)*mp+ (sc+m)]);
             }
           }
       }
       else{
          for(int ss=s_B; ss<m; ss+=d)
          {
             right[ss] = s[ gap_end * L*m + end*m + ss];
          }
                 __syncthreads();
          for(int sc=0;sc<m;sc++)
          {
             for(int ss=s_B;ss<m;ss+=d){
                tmp_result = logsumexp(tmp_result, left[sc]+right[ss]+binary_c[s_A*mp*mp+ (sc)*mp+ (ss)]);
             }
          }
       }
    }

    for(int split=start+1; split< gap_start; split++)
    {
         if(split > start+1){
            for(int ss=s_B; ss<m; ss+=d)
            {
                left[ss] = s[start * L * m + split * m + ss];
            }
            __syncthreads();
            right[s_B] =  s_d[split*L3 + gap_start*L2 + gap_end*L1 + end*d +s_B];
            __syncthreads();

            for(int sc=0; sc<m; sc++)
            {
                tmp_result = logsumexp(tmp_result, left[sc]+right[s_B] + binary[s_A*mpd*4 + s_B*mp*4 + sc*4 + 0]);
            }

         }

         else{
            for(int ss=s_B; ss<p; ss+=d)
            {
                left[ss] = unary[b_idx*(L-1)*p + start * p + ss];
            }
            __syncthreads();

            right[s_B] =  s_d[split*L*L*L*d + gap_start*L*L*d+  gap_end*L*d + end*d+s_B];
            __syncthreads();

            for(int sc=0; sc<p; sc++)
            {
                tmp_result = logsumexp(tmp_result, left[sc]+right[s_B]+ binary[s_A*mpd*4 + s_B*mp*4 + (sc+m)*4 +0]);
            }
         }

         if (split < gap_start-1){

            for(int ss=s_B; ss<m; ss+=d)
            {
                right[ss] = s[split * L * m + gap_start * m + ss];
            }
            __syncthreads();
            left[s_B] = s_d[ start* L3 + split * L2 + gap_end * L1 + end*d +s_B];
            __syncthreads();
            for(int sc=0; sc<m; sc++)
            {
                tmp_result = logsumexp(tmp_result, right[sc]+left[s_B]+ binary[s_A*mpd*4 + s_B*mp*4 + (sc)*4 +1]);
            }

         }
         else{
            for(int ss=s_B; ss<p; ss+=d)
            {
                right[ss] = unary[b_idx*(L-1)*p + (gap_start-1) * p + ss];
            }
            __syncthreads();
            left[s_B] = s_d[ start*L3 + split*L2 + gap_end*L1 + end*d +s_B];
            __syncthreads();
            for(int sc=0; sc<p; sc++)
            {
                tmp_result = logsumexp(tmp_result, right[sc]+left[s_B]+ binary[s_A*mpd*4 + s_B*mp*4 + (sc+m)*4 + 1]);
            }
         }
        __syncthreads();
     }

     for(int split=gap_end+1; split <end; split++){
        if(split > gap_end+1){
            left[s_B] = s_d[start*L3 + gap_start*L2 + split*L1 + end*d +s_B];
            __syncthreads();
            for(int ss=s_B; ss<m; ss+=d)
            {
                right[ss] = s[gap_end*L*m + split*m + ss];
            }
            __syncthreads();
            for(int sc=0;sc<m;sc++){
                tmp_result = logsumexp(tmp_result,  left[s_B]+right[sc] + binary[s_A*mpd*4 + s_B*mp*4 + (sc)*4 + 2]);
            }
        }
        else{
            left[s_B] = s_d[start*L3 + gap_start*L2 + split*L1 + end*d +s_B];
            __syncthreads();
            for(int ss=s_B; ss<p; ss+=d)
            {
                right[ss] = unary[ b_idx*(L-1)*p+ (split-1)*p + ss];
            }
            __syncthreads();
            for(int sc=0;sc<p;sc++){
                tmp_result = logsumexp(tmp_result, left[s_B] + right[sc] +  binary[s_A*mpd*4 + s_B*mp*4 + (sc+m)*4 + 2]);
            }
        }

        if(split < end-1)
        {
           left[s_B] = s_d[start*L3 + gap_start*L2 + gap_end*L1 + split*d +s_B];
               __syncthreads();
           for(int ss=s_B; ss<m; ss+=d)
            {
                right[ss] = s[split*L*m + end*m + ss];
            }
            __syncthreads();
            for(int sc=0;sc<m;sc++){
                tmp_result = logsumexp(tmp_result,  left[s_B]+right[sc] +  binary[s_A*mpd*4 + s_B*mp*4 + (sc)*4+3]);
            }
        }
        else{
            left[s_B] = s_d[start*L3 + gap_start*L2 + gap_end*L1 + split*d +s_B];
            __syncthreads();
            for(int ss=s_B; ss<p; ss+=d)
            {
                right[ss] = unary[ b_idx*(L-1)*p+ (end-1)*p + ss];
            }
            __syncthreads();
            for(int sc=0;sc<p;sc++){
                tmp_result = logsumexp(tmp_result, left[s_B] + right[sc] +  binary[s_A*mpd*4 + s_B*mp*4 + (sc+m)*4+ 3]);
            }
        }
        __syncthreads();
     }

    result[s_B] = tmp_result;
    __syncthreads();
    if(s_B==0){
    float final_result = logf(0);
    for(int i=0;i<d;i++){
        final_result = logsumexp(final_result, result[i]);
    }
    alpha_d[b_idx*L*L*L*L*d + start*L3 + gap_start*L2 + gap_end*L1 + end*d + s_A] = final_result;
    left[0] = final_result;
  }

  __syncthreads();

  for(int sa=s_A; sa<m; sa+=d){
      float tmp_result_c = logf(0);
      float self = alpha_d[b_idx*L*L*L*L*d + start*L3 + gap_start*L2 + gap_end*L1 + end*d + s_B];
      if (gap_start+1 == gap_end){
          for(int ss=s_B; ss<p; ss+=d){
             left[ss] = unary[b_idx*(L-1)*p + gap_start*p + ss];
          }
          __syncthreads();
          for(int i = 0; i < p; i++)
          {
              tmp_result_c = logsumexp(tmp_result_c, self + binary_cd[sa*d*mp + s_B*mp + (i+m)] + left[i]);
          }
      }
      else{
          for(int ss=s_B; ss<m; ss+=d){
             left[ss] = [b_idx*(L-1)*p + gap_start*p + ss];
          }
          __syncthreads();
          for(int i = 0; i < p; i++)
          {
              tmp_result_c = logsumexp(tmp_result_c, self + binary_cd[sa*d*mp + s_B*mp + (i+m)] + left[i]);
          }
      }

    result[s_B] = tmp_result;
    __syncthreads();
    if(s_B==0){
    float final_result = logf(0);
    for(int i=0;i<d;i++){
        final_result = logsumexp(final_result, result[i]);
    }
    alpha_d[b_idx*L*L*L*L*d + start*L3 + gap_start*L2 + gap_end*L1 + end*d + s_A] = final_result;
    }


}


template <typename F>
__global__ void kernel_forward_d_argmax(const F *__restrict__ const binary_rule_d,
                                    const F *__restrict__ const binary_rule_dc,
                                 const F *__restrict__ const unary,
                                F *__restrict__ alpha,
        F *__restrict__ alpha_d,
                               const int B, const int L, const int width, const int m, const int p, const int d)
{
    const int b_idx = blockIdx.x / (L-width);
    const int start = blockIdx.x % (L-width);
    const int gap_start_minus_start = (blockIdx.y) / (L-width);
    const int gap_width =  ((blockIdx.y) % ( L-width)) + 1;
    const int end = start + width + gap_width;
    if(end>=L){
        return;
    }
    const int s_A = (blockIdx.z);
    const int s_B = threadIdx.x;
    const int mp = m+p;
    const int mpd = d* (m+p);
    const int gap_start = start + gap_start_minus_start + 1;
    const int gap_end = gap_start + gap_width;

    if(gap_start - start >width-1)
    {
        return;
    }

    __shared__ float result[200];
    __shared__ float result_idx[200];
    __shared__ float left[200];
    __shared__ float right[200];

    const F *__restrict__ const s = alpha + b_idx * L * L * m;
    const F *__restrict__ const s_d = alpha_d + b_idx * L * L * L * L * d;
    const F *__restrict__ const binary = binary_rule_d + b_idx * d * d * mp * 4;
    const F *__restrict__ const binary_c = binary_rule_dc + b_idx * d * mp * mp;
    const int L3 = L * L * L * d;
    const int L2 = L * L * d;
    const int L1 = L * d;

    float tmp_result = logf(0);
    float tmp_idx = -1;
    float tmp = 0;

    if(gap_start==start+1){
       for(int ss=s_B; ss<p; ss+=d){
          left[ss] = unary[b_idx*(L-1)*p+ start*p + ss];
       }
       __syncthreads();
       if(gap_end==end-1){
           for(int ss=s_B; ss<p; ss+=d)
           {
                right[ss] = unary[b_idx*(L-1)*p+ (end-1)*p + ss];
           }
           __syncthreads();

           for(int ss=s_B; ss<p; ss+=d)
           {
               right[ss] = unary[b_idx*(L-1)*p+ (end-1)*p + ss];
               for(int sc=0;sc<p;sc++){
                  tmp = left[ss] + right[sc] + binary_c[s_A*mp*mp + (ss+m)*mp+ (sc+m)];
                  if(tmp > tmp_result){
                    tmp_result = tmp;
                    tmp_idx = -((ss+m)*mp + (sc+m));
                  }
               }
           }
       }

       else{
          for(int ss=s_B; ss<m; ss+=d)
          {
             right[ss] = s[ gap_end * L*m + end*m + ss];
          }
          __syncthreads();
          for(int sc=0;sc<p;sc++)
          {
             for(int ss=s_B;ss<m;ss+=d){
                tmp = left[sc]+right[ss]+binary_c[s_A*mp*mp+ (sc+m)*mp+ ss];
                if(tmp > tmp_result){
                    tmp_result = tmp;
                    tmp_idx = -((sc+m)*mp + ss);
                }
             }
          }
       }
    }
    else{
      for(int ss=s_B; ss<m; ss+=d)
      {
         left[ss] = s[start * L*m + gap_start*m + ss];
      }
    __syncthreads();
       if(gap_end==end-1){
           for(int ss=s_B; ss<p; ss+=d)
           {
                right[ss] = unary[ b_idx*(L-1)*p+ (end-1)*p + ss];
           }
               __syncthreads();
           for(int sc=0;sc<p;sc++){
             for(int ss=s_B;ss<m;ss+=d){
                tmp = left[ss]+right[sc]+binary_c[s_A*mp*mp+ (ss)*mp+ (sc+m)];
                if(tmp > tmp_result){
                    tmp_result = tmp;
                    tmp_idx = -( ss*mp + (sc+m));
                }


             }
           }
       }
       else{
          for(int ss=s_B; ss<m; ss+=d)
          {
             right[ss] = s[ gap_end * L*m + end*m + ss];
          }
          __syncthreads();
          for(int sc=0;sc<m;sc++)
          {
             for(int ss=s_B;ss<m;ss+=d){
                tmp = left[sc]+right[ss]+binary_c[s_A*mp*mp+ (sc)*mp+ (ss)];
                if (tmp > tmp_result){
                    tmp_result = tmp;
                    tmp_idx = -(sc*mp+ss);
                }
             }
          }
       }
    }

    __syncthreads();
    for(int split=start+1; split< gap_start; split++)
    {
         if(split > start+1){
            for(int ss=s_B; ss<m; ss+=d)
            {
                left[ss] = s[start * L * m + split * m + ss];
            }
            __syncthreads();
            right[s_B] =  s_d[split*L3 + gap_start*L2 + gap_end*L1 + end*d +s_B];
            __syncthreads();

            for(int sc=0; sc<m; sc++)
            {

               tmp = left[sc]+right[s_B] + binary[s_A*mpd*4 + s_B*mp*4 + sc*4 + 0];
               if(tmp > tmp_result){
                   tmp_result = tmp;
                   tmp_idx = split * (mp*d*4) + s_B*mp*4 + sc*4 + 0;
               }
            }
         }

         else{
            for(int ss=s_B; ss<p; ss+=d)
            {
                left[ss] = unary[b_idx*(L-1)*p + start * p + ss];
            }
            __syncthreads();
            right[s_B] =  s_d[split*L*L*L*d + gap_start*L*L*d+  gap_end*L*d + end*d+s_B];
            __syncthreads();

            for(int sc=0; sc<p; sc++)
            {
                tmp =  left[sc]+right[s_B]+ binary[s_A*mpd*4 + s_B*mp*4 + (sc+m)*4 +0];
                if(tmp > tmp_result){
                    tmp_result = tmp;
                    tmp_idx = split*d*mp*4 + s_B*mp*4 + (sc+m)*4 + 0;
                }
            }
         }

         if (split < gap_start-1){

            for(int ss=s_B; ss<m; ss+=d)
            {
                right[ss] = s[split * L * m + gap_start * m + ss];
            }
            __syncthreads();
            left[s_B] = s_d[ start* L3 + split * L2 + gap_end * L1 + end*d +s_B];
            __syncthreads();
            for(int sc=0; sc<m; sc++)
            {
                tmp = right[sc]+left[s_B]+ binary[s_A*mpd*4 + s_B*mp*4 + (sc)*4 +1];
                if(tmp > tmp_result){
                   tmp_result = tmp;
                   tmp_idx = split*d*mp*4 + s_B*mp*4 + (sc)*4 +1;
                }
            }

         }
         else{
            for(int ss=s_B; ss<p; ss+=d)
            {
                right[ss] = unary[b_idx*(L-1)*p + (gap_start-1) * p + ss];
            }
           __syncthreads();
            left[s_B] = s_d[ start*L3 + split*L2 + gap_end*L1 + end*d +s_B];
            __syncthreads();
            for(int sc=0; sc<p; sc++)
            {
                tmp = right[sc]+left[s_B]+ binary[s_A*mpd*4 + s_B*mp*4 + (sc+m)*4 + 1];
                if(tmp > tmp_result){
                    tmp_result = tmp;
                    tmp_idx = split*d*mp*4 + s_B*mp*4 + (sc+m)*4 + 1;
                }
            }
         }
         __syncthreads();
     }

     for(int split=gap_end+1; split <end; split++){
        if(split > gap_end+1){
            left[s_B] = s_d[start*L3 + gap_start*L2 + split*L1 + end*d +s_B];
             __syncthreads();
            for(int ss=s_B; ss<m; ss+=d)
            {
                right[ss] = s[gap_end*L*m + split*m + ss];
            }
            __syncthreads();
            for(int sc=0;sc<m;sc++){
                tmp =  left[s_B]+right[sc] +  binary[s_A*mpd*4 + s_B*mp*4 + (sc)*4+2];
                if(tmp > tmp_result){
                    tmp_result = tmp;
                    tmp_idx = split*d*mp*4 + s_B*mp*4 + (sc)*4+2;
                }

            }
        }
        else{
            left[s_B] = s_d[start*L3 + gap_start*L2 + split*L1 + end*d +s_B];
             __syncthreads();
            for(int ss=s_B; ss<p; ss+=d)
            {
                right[ss] = unary[ b_idx*(L-1)*p+ (split-1)*p + ss];
            }
            __syncthreads();
            for(int sc=0;sc<p;sc++){
                tmp = left[s_B] + right[sc] +  binary[s_A*mpd*4 + s_B*mp*4 + (sc+m)*4 + 2];
                if(tmp > tmp_result){
                    tmp_result = tmp;
                    tmp_idx = split*d*mp*4 + s_B*mp*4 + (sc+m)*4 + 2;
                }
            }
        }

        if(split < end-1)
        {
           left[s_B] = s_d[start*L3 + gap_start*L2 + gap_end*L1 + split*d +s_B];
           __syncthreads();
           for(int ss=s_B; ss<m; ss+=d)
            {
                right[ss] = s[split*L*m + end*m + ss];
            }
            __syncthreads();
            for(int sc=0;sc<m;sc++){
                tmp =  left[s_B]+right[sc] +  binary[s_A*mpd*4 + s_B*mp*4 + (sc)*4+3];
                if(tmp > tmp_result){
                    tmp_result = tmp;
                    tmp_idx = split*d*mp*4 + s_B*mp*4 + (sc)*4+3;
                }

            }
        }
        else{
            left[s_B] = s_d[start*L3 + gap_start*L2 + gap_end*L1 + split*d +s_B];
            __syncthreads();
            for(int ss=s_B; ss<p; ss+=d)
            {
                right[ss] = unary[ b_idx*(L-1)*p+ (end-1)*p + ss];
            }
            __syncthreads();
            for(int sc=0;sc<p;sc++){
                tmp = left[s_B] + right[sc] +  binary[s_A*mpd*4 + s_B*mp*4 + (sc+m)*4+ 3];
                if(tmp > tmp_result){
                    tmp_result = tmp;
                    tmp_idx = split*d*mp*4 + s_B*mp*4 + (sc+m)*4+ 3;
                }

            }
        }
       __syncthreads();

     }
    result[s_B] = tmp_result;
    result_idx[s_B] = tmp_idx;
    __syncthreads();
    if(s_B==0){
    float final_result = logf(0);
    float final_idx = -1;
    for(int i=0;i<d;i++){
        if(result[i] > final_result){
            final_result = result[i];
            final_idx = result_idx[i];
        }
    }
    alpha_d[b_idx*L*L*L*L*d + start*L3 + gap_start*L2 + gap_end*L1 + end*d + s_A] = final_result;
    alpha_d[b_idx*L*L*L*L*d + gap_start*L3 + start*L2 + gap_end*L1 + end*d + s_A] = final_idx;
  }
}




template <typename F>
__global__ void kernel_backward_d(const F *__restrict__ const binary_rule_d,
                                    const F *__restrict__ const binary_rule_dc,
                                 const F *__restrict__ const unary,
                                 F *__restrict__ binary_rule_d_grd,
                              F *__restrict__ binary_rule_dc_grd,
                               F *__restrict__  unary_grd,
                                F *__restrict__ alpha,
                                F *__restrict__ alpha_d,
                               const int B, const int L, const int width, const int m, const int p, const int d)
{
    const int b_idx = blockIdx.x / (L-width);
    const int start = blockIdx.x % (L-width);
    const int gap_start_minus_start = (blockIdx.y) / (L-width);
    const int gap_width =  ((blockIdx.y) % ( L-width)) + 1;
    const int end = start + width + gap_width;
    if(end>=L){
        return;
    }
    const int s_A = (blockIdx.z);
    const int s_B = threadIdx.x;
    const int mp = m+p;
    const int mpd = d* (m+p);
    const int gap_start = start + gap_start_minus_start + 1;
    const int gap_end = gap_start + gap_width;

    if(gap_start - start >width-1)
    {
        return;
    }

    __shared__ float left[200];
    __shared__ float right[200];

    F *__restrict__ s = alpha + b_idx * L * L * m;
    F *__restrict__ s_d = alpha_d + b_idx * L * L * L * L * d;
    const F *__restrict__ const binary = binary_rule_d + b_idx * d * d * mp * 4;
    const F *__restrict__ const binary_c = binary_rule_dc + b_idx * d * mp * mp;
    F *__restrict__ binary_grd = binary_rule_d_grd + b_idx * d * d * mp * 4 + s_A * mp * d * 4;
    F *__restrict__ binary_c_grd = binary_rule_dc_grd + b_idx * d * mp * mp + s_A * mp * mp;
    const int L3 = L * L * L * d;
    const int L2 = L * L * d;
    const int L1 = L * d;

    float tmp=0;
    float parent_inside = s_d[start*L3 + gap_start*L2 + gap_end*L1 + end*d + s_A];
    float parent_grd = s_d[gap_start*L3 + start*L2 + gap_end*L1+end*d+s_A];

    left[s_B] = s[start * L * m + gap_start * m + s_B];
    right[s_B] = s[gap_end * L * m + end * m + s_B];
    __syncthreads();


    if(gap_start==start+1){
       for(int ss=s_B; ss<p; ss+=d){
          left[ss] = unary[b_idx*(L-1)*p+ start*p + ss];
       }
       __syncthreads();
       if(gap_end==end-1){
           for(int ss=s_B; ss<p; ss+=d)
           {
                right[ss] = unary[b_idx*(L-1)*p+ (end-1)*p + ss];
           }
           __syncthreads();

           for(int ss=s_B; ss<p; ss+=d)
           {
               right[ss] = unary[b_idx*(L-1)*p+ (end-1)*p + ss];
               for(int sc=0;sc<p;sc++){
                  tmp =  exp(left[ss] + right[sc] + binary_c[s_A*mp*mp + (ss+m)*mp+ (sc+m)] - parent_inside);
                  tmp = tmp * parent_grd;
                  atomicAdd(unary_grd + b_idx*(L-1)*p + start*p + ss, tmp);
                  atomicAdd(unary_grd + b_idx*(L-1)*p + gap_end*p + sc, tmp);
                  atomicAdd(binary_c_grd + (ss+m)*mp + (sc+m),  tmp);
               }
           }
       }

       else{
          for(int ss=s_B; ss<m; ss+=d)
          {
             right[ss] = s[ gap_end * L*m + end*m + ss];
          }
         __syncthreads();
          for(int sc=0;sc<p;sc++)
          {
             for(int ss=s_B;ss<m;ss+=d){
                tmp =  exp(left[sc]+right[ss]+binary_c[s_A*mp*mp+ (sc+m)*mp+ ss] - parent_inside);
                if(tmp > 1){
                    printf("??????1\n");
                }
                tmp = tmp * parent_grd;
                atomicAdd(unary_grd + b_idx*(L-1)*p + start*p + sc, tmp);
                atomicAdd(s + end*L*m + gap_end*m + ss, tmp);
                atomicAdd(binary_c_grd + (sc+m)*mp + ss,  tmp);
             }
          }
       }
    }
    else{
      for(int ss=s_B; ss<m; ss+=d)
      {
         left[ss] = s[start * L*m + gap_start*m + ss];
      }
      __syncthreads();
       if(gap_end==end-1){
           for(int ss=s_B; ss<p; ss+=d)
           {
                right[ss] = unary[ b_idx*(L-1)*p+ (end-1)*p + ss];
           }
               __syncthreads();
           for(int sc=0;sc<p;sc++){
             for(int ss=s_B;ss<m;ss+=d){
                tmp = exp(left[ss]+right[sc]+binary_c[s_A*mp*mp+ (ss)*mp+ (sc+m)] - parent_inside);
                if(tmp > 1){
                    printf("??????2\n");
                }
                tmp = tmp * parent_grd;

                atomicAdd(s + gap_start*m*L + start*m + ss, tmp);
                atomicAdd( unary_grd +  b_idx*(L-1)*p+ (end-1)*p + sc, tmp);
                atomicAdd(binary_c_grd + (ss)*mp + (sc+m),  tmp);
             }
           }
       }
       else{
          for(int ss=s_B; ss<m; ss+=d)
          {
             right[ss] = s[ gap_end * L*m + end*m + ss];
          }
          __syncthreads();
          for(int sc=0;sc<m;sc++)
          {
             for(int ss=s_B;ss<m;ss+=d){
                tmp = exp(left[sc]+right[ss]+binary_c[s_A*mp*mp+ (sc)*mp+ (ss)] - parent_inside);
                if(tmp > 1){
                    printf("??????3\n");
                }
                tmp = tmp * parent_grd;

                atomicAdd(s + gap_start*m*L + start*m + sc, tmp);
                atomicAdd(s + end * L*m + gap_end*m +ss, tmp);
                atomicAdd(binary_c_grd + (sc)*mp + (ss),  tmp);
             }
          }
       }
    }
    __syncthreads();
    for(int split=start+1; split< gap_start; split++)
    {
         if(split > start+1){
            for(int ss=s_B; ss<m; ss+=d)
            {
                left[ss] = s[start * L * m + split * m + ss];
            }
             __syncthreads();
            right[s_B] =  s_d[split*L3 + gap_start*L2 + gap_end*L1 + end*d +s_B];
            __syncthreads();

            for(int sc=0; sc<m; sc++)
            {
                tmp = exp(left[sc]+right[s_B] + binary[s_A*mpd*4 + s_B*mp*4 + sc*4 + 0] - parent_inside);
                if(tmp > 1){
                    printf("??????4\n");
                }
                tmp = tmp * parent_grd;
                atomicAdd(s + split * L *m + start *m + sc, tmp);
                atomicAdd(s_d + gap_start * L3 + split * L2 + gap_end*L1 + end*d +s_B, tmp);
                atomicAdd(binary_grd +   s_B*mp*4 + sc*4 + 0, tmp);
            }

         }

         else{
            for(int ss=s_B; ss<p; ss+=d)
            {
                left[ss] = unary[b_idx*(L-1)*p + start * p + ss];
            }
            __syncthreads();
            right[s_B] =  s_d[split*L*L*L*d + gap_start*L*L*d+  gap_end*L*d + end*d+s_B];
            __syncthreads();

            for(int sc=0; sc<p; sc++)
            {
                tmp = exp(left[sc]+right[s_B]+ binary[s_A*mpd*4 + s_B*mp*4 + (sc+m)*4 +0] - parent_inside);
                if(tmp > 1){
                    printf("??????5\n");
                }
                tmp = tmp * parent_grd;

                atomicAdd(unary_grd + b_idx*(L-1)*p + start * p + sc, tmp);
                atomicAdd(s_d +  gap_start*L*L*L*d + split*L*L*d+  gap_end*L*d + end*d+s_B, tmp);
                atomicAdd(binary_grd + s_B*mp*4 + (sc+m)*4 +0, tmp );
            }
         }

         if (split < gap_start-1){

            for(int ss=s_B; ss<m; ss+=d)
            {
                right[ss] = s[split * L * m + gap_start * m + ss];
            }
            __syncthreads();
            left[s_B] = s_d[ start* L3 + split * L2 + gap_end * L1 + end*d +s_B];
            __syncthreads();
            for(int sc=0; sc<m; sc++)
            {
                tmp = exp(right[sc]+left[s_B]+ binary[s_A*mpd*4 + s_B*mp*4 + (sc)*4 +1] - parent_inside);
                if(tmp > 1){
                    printf("??????6\n");
                }
                tmp = tmp * parent_grd;
                atomicAdd(binary_grd +  s_B*mp*4 + (sc)*4 +1, tmp);
                atomicAdd( s_d + split* L3 + start * L2 + gap_end * L1 + end*d +s_B, tmp );
                atomicAdd( s + gap_start * L * m + split * m + sc, tmp);
            }

         }
         else{
            for(int ss=s_B; ss<p; ss+=d)
            {
                right[ss] = unary[b_idx*(L-1)*p + (gap_start-1) * p + ss];
            }
            __syncthreads();
            left[s_B] = s_d[ start*L3 + split*L2 + gap_end*L1 + end*d +s_B];
            __syncthreads();
            for(int sc=0; sc<p; sc++)
            {
                tmp = exp(right[sc]+left[s_B]+ binary[s_A*mpd*4 + s_B*mp*4 + (sc+m)*4 + 1] - parent_inside);
                if(tmp > 1){
                    printf("??????7\n");
                }
                tmp = tmp * parent_grd;
                atomicAdd(binary_grd + s_B*mp*4 + (sc+m)*4 + 1, tmp);
                atomicAdd( s_d +  split*L3 + start*L2 + gap_end*L1 + end*d +s_B, tmp);
                atomicAdd( unary_grd + b_idx*(L-1)*p + (gap_start-1) * p + sc, tmp);
            }
         }
                 __syncthreads();
     }

     for(int split=gap_end+1; split <end; split++){
        if(split > gap_end+1){
            left[s_B] = s_d[start*L3 + gap_start*L2 + split*L1 + end*d + s_B];
            __syncthreads();
            for(int ss=s_B; ss<m; ss+=d)
            {
                right[ss] = s[gap_end*L*m + split*m + ss];
            }
            __syncthreads();
            for(int sc=0;sc<m;sc++){
                tmp = exp(left[s_B] + right[sc] + binary[s_A*mpd*4 + s_B*mp*4 + sc*4 + 2] - parent_inside);
                if(tmp > 1){
                printf("??????8  %d %d %d %d   %d %d %f %f %f %f %f \n", start, gap_start, gap_end, end, s_A, s_B, tmp, right[sc], left[s_B], binary[s_A*mpd*4 + s_B*mp*4 + (sc)*4 + 2], parent_inside);
                }

                tmp = tmp * parent_grd;
                atomicAdd(binary_grd + s_B*mp*4 + (sc)*4+2, tmp);
                atomicAdd( s +split*L*m + gap_end*m + sc, tmp );
                atomicAdd( s_d + gap_start*L3 + start*L2 + split*L1 + end*d +s_B,tmp );
            }
        }
        else{
            left[s_B] = s_d[start*L3 + gap_start*L2 + split*L1 + end*d +s_B];
                       __syncthreads();
            for(int ss=s_B; ss<p; ss+=d)
            {
                right[ss] = unary[ b_idx*(L-1)*p+ (split-1)*p + ss];
            }
            __syncthreads();
            for(int sc=0;sc<p;sc++){
                tmp = exp(left[s_B] + right[sc] +  binary[s_A*mpd*4 + s_B*mp*4 + (sc+m)*4 + 2] - parent_inside);
                if(tmp > 1){
                    printf("??????9\n");
                }
                tmp = tmp * parent_grd;
                atomicAdd(binary_grd + s_B*mp*4 + (sc+m)*4 + 2, tmp);
                atomicAdd(s_d+ gap_start*L3 + start*L2 + split*L1 + end*d +s_B, tmp);
                atomicAdd(unary_grd + b_idx*(L-1)*p+ (split-1)*p + sc, tmp);

            }
        }

        if(split < end-1)
        {
           left[s_B] = s_d[start*L3 + gap_start*L2 + gap_end*L1 + split*d +s_B];
                      __syncthreads();

           for(int ss=s_B; ss<m; ss+=d)
            {
                right[ss] = s[split*L*m + end*m + ss];
            }
            __syncthreads();
            for(int sc=0;sc<m;sc++){
                tmp = exp( left[s_B]+right[sc] +  binary[s_A*mpd*4 + s_B*mp*4 + sc*4 + 3] - parent_inside);
                if(tmp > 1){
                    printf("??????10\n");
                }
                tmp = tmp * parent_grd;
                atomicAdd(binary_grd +  s_B*mp*4 + (sc)*4+3, tmp);
                atomicAdd(s_d+ gap_start*L3 + start*L2 + gap_end*L1 + split*d +s_B, tmp);
                atomicAdd(s + end*L*m + split*m + sc, tmp);
            }
        }
        else{
            left[s_B] = s_d[start*L3 + gap_start*L2 + gap_end*L1 + split*d +s_B];
                       __syncthreads();

            for(int ss=s_B; ss<p; ss+=d)
            {
                right[ss] = unary[ b_idx*(L-1)*p+ (end-1)*p + ss];
            }
            __syncthreads();
            for(int sc=0;sc<p;sc++){
                tmp = exp(left[s_B] + right[sc] +  binary[s_A*mpd*4 + s_B*mp*4 + (sc+m)*4+ 3] - parent_inside);
                if(tmp > 1){
                    printf("??????11\n");
                }
                tmp = tmp * parent_grd;
                atomicAdd(binary_grd + s_B*mp*4 + (sc+m)*4+ 3, tmp);
                atomicAdd(s_d + gap_start*L3 + start*L2 + gap_end*L1 + split*d +s_B, tmp );
                atomicAdd(unary_grd + b_idx*(L-1)*p+ (end-1)*p + sc, tmp);

            }
        }
        __syncthreads();
     }

}

template <typename F>
__global__ void kernel_backward(const F *__restrict__ const binary_rule,
                                 const F *__restrict__ const unary,
                                 F *__restrict__ binary_grd,
                                 F *__restrict__ unary_grd,
                                 F *__restrict__ alpha,
                                 const int B, const int L, const int width, const int m, const int p)
{
    const int b_idx = blockIdx.x;
    const int start = blockIdx.y;
    const int s_A = blockIdx.z;
    const int s_B = threadIdx.x;
    const int mp = m+p;

    if(start+width >= L){
        return;
    }

    __shared__ float left_inside[200];
    __shared__ float right_inside[200];

    int end = start + width;
    F *__restrict__  s = alpha + b_idx * L * L * m;
    const F *__restrict__ const binary = binary_rule + b_idx * m * mp * mp;
    F *__restrict__ g_binary = binary_grd + b_idx * m * mp * mp;
    float parent_inside = s[start*L*m + end*m + s_A];
    float parent_grd  = s[end*L*m + start*m + s_A];

    right_inside[s_B] = s[(start+1) * L * m + end * m + s_B];
    __syncthreads();


    for (int sc=0; sc<p; sc++)
    {
       float tmp = exp(unary[b_idx*(L-1)*p + (start) *p + sc] + right_inside[s_B] + binary[s_A*(mp)*(mp) + (m+sc)*mp + s_B] - parent_inside) * parent_grd;
       atomicAdd(s + end*L*m + (start+1)*m + s_B, tmp);
       atomicAdd(g_binary + s_A*mp*mp + (sc+m)*mp + s_B, tmp);
       atomicAdd(unary_grd + b_idx*(L-1)*p + start*p + sc, tmp);
    }

    for (int split = start+2; split < start+width-1; split++)
    {
        left_inside[s_B] =  s[start * L * m + split * m + s_B];
            right_inside[s_B] = s[split * L * m + end * m + s_B];
            __syncthreads();
            for(int sc=0; sc<m; sc++){
                float tmp = exp(left_inside[s_B] + right_inside[sc] + binary[s_A*(mp)*(mp) + s_B*mp + sc] - parent_inside)* parent_grd;
                atomicAdd(s + split * L * m + start * m + s_B,  tmp);
                atomicAdd(s +end * L * m + split * m + sc, tmp);
                atomicAdd(&g_binary[s_A*mp*mp + (s_B)*mp + sc], tmp);
            }
            __syncthreads();
    }

    left_inside[s_B] = s[start * L * m + (start+width-1) * m + s_B];
    __syncthreads();
    for (int sc=0; sc<p; sc++){
         float tmp= exp(left_inside[s_B] + unary[b_idx*(L-1)*p + (end-1) *p + sc] + binary[s_A*(mp)*(mp) + s_B*mp +m+sc] - parent_inside) * parent_grd;
         atomicAdd(s + (end-1)*L*m+ (start)*m + s_B, tmp);
         atomicAdd(g_binary + s_A*mp*mp + (s_B)*mp + (sc +m), tmp);
         atomicAdd(unary_grd + b_idx*(L-1)*p + (end-1)*p + sc,  tmp);
    }
}

template <typename F>
__global__ void kernel_backward_len2(const F *__restrict__ const binary_rule,
                                 const F *__restrict__ const unary,
                                 F *__restrict__ binary_grd,
                                 F *__restrict__ unary_grd,
                                F *__restrict__ alpha,
                               const int B, const int L, const int m, const int p)
{
    const int b_idx = blockIdx.x;
    const int start = blockIdx.y;
    const int s_A = blockIdx.z;
    const int s_B = threadIdx.x;
    const int mp = m+p;
    const int width= 2;
    if(start+width >= L){
        return;
    }

    int end = start + width;
    F *__restrict__  s = alpha + b_idx * L * L * m;
    const F *__restrict__ const binary = binary_rule + b_idx * m * mp * mp;
    F *__restrict__  g_binary = binary_grd + b_idx * m * mp * mp;

    float parent_inside = s[(start*L*m) + (end*m) + s_A];
    float parent_grd  = s[(end*L*m) + (start*m) + s_A];
    __shared__ float ss[200];
    __shared__ float sss[200];

    ss[s_B]= unary[b_idx*(L-1)*p + start*p + s_B];
    sss[s_B] = unary[b_idx*(L-1)*p + (start+1)*p + s_B];
    __syncthreads();

    for (int sc=0; sc<p; sc++)
    {
       float tmp = exp(ss[s_B] + sss[sc] + binary[s_A*(mp)*(mp) + (m+s_B)*mp + (sc+m)] - parent_inside) * parent_grd;
       atomicAdd(unary_grd+b_idx*(L-1)*p + start*p + s_B, tmp);
       atomicAdd(unary_grd+b_idx*(L-1)*p + (start+1)*p + sc, tmp);
       atomicAdd(g_binary + s_A*mp*mp + (s_B+m)*mp + (sc+m), tmp);
    }
}




void cuda_forward(const float *binary, const float *binary_close,  const float *binary_dc,  const float *binary_d, const float *unary, float *alpha,  float *alpha_d, int B, int L, int m, int p, int d)
{
    dim3 gridDim(B, L-2+1, m);
    dim3 blockDim(p);
    kernel_forward_len2<<<gridDim, blockDim>>>(binary, unary, alpha, B, L, m, p);

    dim3 gridDim2(B*(L-2), (L-2)*(L-2), d);
    dim3 blockDim2(d);
    kernel_forward_d<<<gridDim2, blockDim2>>>(binary_d, binary_dc, unary, alpha, alpha_d, B, L, 2, m, p, d);

for(int w=3; w<L; w++){
  dim3 gridDim(B, L-w, m);
  dim3 blockDim(m);
  kernel_forward<<<gridDim, blockDim>>>(binary, unary, alpha, B, L, w, m, p);
  dim3 gridDim2(B, L-w, m);
  dim3 blockDim2(d);
  kernel_forward_close<<<gridDim2, blockDim2>>>(binary_close, unary, alpha, alpha_d, B, L, w, m, p, d);
  if(w<L-1){
    dim3 gridDim3(B*(L-w),  (w-1)*(L-w), d);
    dim3 blockDim3(d);
    kernel_forward_d<<<gridDim3, blockDim3>>>(binary_d, binary_dc, unary, alpha, alpha_d, B, L, w, m, p, d);
  }
 }
}



void cuda_forward_argmax(const float *binary, const float *binary_close,  const float *binary_dc,  const float *binary_d, const float *unary, float *alpha,  float *alpha_d, int B, int L, int m, int p, int d)
{
    dim3 gridDim(B, L-2+1, m);
    dim3 blockDim(p);
    kernel_forward_len2_argmax<<<gridDim, blockDim>>>(binary, unary, alpha, B, L, m, p);

dim3 gridDim2(B*(L-2), (L-2)*(L-2), d);
dim3 blockDim2(d);
kernel_forward_d_argmax<<<gridDim2, blockDim2>>>(binary_d, binary_dc, unary, alpha, alpha_d, B, L, 2, m, p, d);

for(int w=3; w<L; w++){
  dim3 gridDim(B, L-w, m);
  dim3 blockDim(m);
  kernel_forward_argmax<<<gridDim, blockDim>>>(binary, unary, alpha, B, L, w, m, p);
  dim3 gridDim2(B, L-w, m);
  dim3 blockDim2(d);
  kernel_forward_close_argmax<<<gridDim2, blockDim2>>>(binary_close, unary, alpha, alpha_d, B, L, w, m, p, d);
  if(w<L-1){
    dim3 gridDim3(B*(L-w),  (w-1)*(L-w), d);
    dim3 blockDim3(d);
    kernel_forward_d_argmax<<<gridDim3, blockDim3>>>(binary_d, binary_dc, unary, alpha, alpha_d, B, L, w, m, p, d);
  }
}
}


void cuda_backward(const float *binary, const float *binary_close,  const float *binary_dc,  const float *binary_d,
                const float *unary,  float *binary_grd, float *binary_close_grd, float *binary_dc_grd,
                 float *binary_d_grd, float *unary_grd, float *alpha,  float *alpha_d, int B, int L, int m, int p, int d)
{

    for(int w=L-1; w>2; w--){
        if(w<L-1){
            dim3 gridDim3(B*(L-w),  (w-1)*(L-w), d);
            dim3 blockDim3(d);
            kernel_backward_d<<<gridDim3, blockDim3>>>(binary_d, binary_dc, unary, binary_d_grd, binary_dc_grd, unary_grd, alpha, alpha_d, B, L, w, m, p, d);
        }
        dim3 gridDim(B, L-w, m);
        dim3 blockDim(m);
        kernel_backward<<<gridDim, blockDim>>>(binary, unary,  binary_grd, unary_grd, alpha, B, L, w, m, p);
        dim3 gridDim2(B, L-w, m);
        dim3 blockDim2(d);
        kernel_backward_close<<<gridDim2, blockDim2>>>(binary_close, unary, binary_close_grd, unary_grd, alpha, alpha_d, B, L, w, m, p, d);

    }
    dim3 gridDim2(B*(L-2), (L-2)*(L-2), d);
    dim3 blockDim2(d);
    kernel_backward_d<<<gridDim2, blockDim2>>>(binary_d, binary_dc, unary, binary_d_grd, binary_dc_grd, unary_grd, alpha, alpha_d, B, L, 2, m, p, d);
    dim3 gridDim(B, L-2+1, m);
    dim3 blockDim(p);
    kernel_backward_len2<<<gridDim, blockDim>>>(binary, unary, binary_grd, unary_grd, alpha, B, L, m, p);
}



