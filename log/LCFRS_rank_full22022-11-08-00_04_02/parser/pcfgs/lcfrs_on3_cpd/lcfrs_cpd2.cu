...
__syncthreads();
    ...
    for(int split = gap_end+1; split <end; split++){
        if((alpha_d_mask[b_idx*L*L*L*L + start*L*L*L + gap_start*L*L + split*L + end] == 1) && (alpha_mask[b_idx*L*L + split*L + gap_start] == 1)){
...
void cuda_forward( float *head_c1,   float *head_c2,   float *head_d1,   float *head_d2,
                   ...
                   float *alpha, float *alpha_mask, float *alpha_d, float *alpha_d_mask,
