...
class InsideCPD_rankspace(torch.autograd.Function):
    ...
    def forward(ctx, head_cl1, head_cr1,
                     ...
                     head_cd1, head_cd2, head_cd3,
                     head_dd1, head_dd2, head_dd3,

                    head_ill_in1, head_ill_in2, head_ill_in3,
                    head_ill_out1, head_ill_out2, head_ill_out3,
    ...
    def backward(ctx, gwk):
        ...
        head_cd3_grd = torch.zeros_like(head_cd3)
        head_dd1_grd = torch.zeros_like(head_dd1)
        head_dd2_grd = torch.zeros_like(head_dd2)
        head_dd3_grd = torch.zeros_like(head_dd3)

        head_ill_in1_grd = torch.zeros_like(head_ill_in1)
        head_ill_in2_grd = torch.zeros_like(head_ill_in2)
        head_ill_in3_grd = torch.zeros_like(head_ill_in3)

        head_ill_out1_grd = torch.zeros_like(head_ill_out1)
        head_ill_out2_grd = torch.zeros_like(head_ill_out2)
        head_ill_out3_grd = torch.zeros_like(head_ill_out3)
        ...
        inside_cuda.backward(
            ...
            head_cd1_grd, head_cd2_grd, head_cd3_grd,
            head_dd1_grd, head_dd2_grd, head_dd3_grd,
            head_ill_in1_grd, head_ill_in2_grd, head_ill_in3_grd,
            head_ill_out1_grd, head_ill_out2_grd, head_ill_out3_grd,
...
class PCFG(PCFG_base):
    ...
    def compute_marginals(self, rules, lens, mbr=True):
        ...
        head_cd3 = rules['head_cd3'].contiguous()
        head_dd2 = rules['head_dd2'].contiguous()
        head_dd3 = rules['head_dd3'].contiguous()
        head_cd2 = rules['head_cd2'].contiguous()

        head_ill_in1 = rules['head_ill_in1'].contiguous()
        head_ill_in2 = rules['head_ill_in2'].contiguous()
        head_ill_in3 = rules['head_ill_in3'].contiguous()
        head_ill_out3 = rules['head_ill_out3'].contiguous()
        head_ill_out2 = rules['head_ill_out2'].contiguous()
        head_ill_out1 = rules['head_ill_out1'].contiguous()
    ...
    def decode(self, rules, lens, raw_word, viterbi=False, mbr=False):
        ...
        inside_cuda2.argmax_on6(marginal_c, marginal_d, alpha_c_mbr, alpha_d_mbr, B, L)
