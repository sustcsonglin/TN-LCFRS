                                ...
                                int B,   int L,   int width,   int m,   int r)
{
    ...
    if(alpha_mask[b_idx*L*L+start*L+end]==0){
        return;
    }
    ...
    float tmp_result = -999999999;

    float rule_score = head_rule[b_idx*m*r + s_A*r + s_R];

    for (int split = start+1; split < start+width; split++)
    {
        if((alpha_mask[b_idx*L*L+start*L+split]==1) && (alpha_mask[b_idx*L*L+split*L+end]==1)){
                                ...
                                int B,   int L,   int width,   int m,   int r)
{
      ...
      if(alpha_mask[b_idx*L*L+start*L+start+width] == 0){
        return;
      }
                                   ...
                                   int B,   int L,   int width,   int m,    int r)
{
    ...
    for (int split = start+1; split < start+width-1; split++)
    {
         ...
         {
             if((alpha_mask[b_idx*L*L+split*L+split2] == 1) && (alpha_d_mask[b_idx*L*L*L*L+ start*L*L*L + split*L*L + split2*L + end] == 1)){
                                    ...
                                    int B,   int L,   int width,   int d,   int r)
{
      ...
      if(alpha_d_mask[b_idx*L*L*L*L + start*L*L*L + gap_start*L*L + gap_end*L + end]==0){
        return;
      }
      if(alpha_c_mask[b_idx*L*L + start*L + gap_start] == 0){
        return;
      }
      if(alpha_c_mask[b_idx*L*L + gap_end*L + end]==0){
      return;
      }
...
__syncthreads();
    ...
    {
        if((alpha_d_mask[b_idx*L*L*L*L + split*L*L*L + gap_start*L*L + gap_end*L + end] == 1) && (alpha_mask[b_idx*L*L + start*L + split] == 1)){
        ...
        if((alpha_d_mask[b_idx*L*L*L*L + start*L*L*L + split*L*L + gap_end*L + end] == 1) && (alpha_mask[b_idx*L*L + split*L + gap_start] == 1)){
    ...
    for(int split = gap_end+1; split <end; split++){
        if((alpha_d_mask[b_idx*L*L*L*L + start*L*L*L + gap_start*L*L + split*L + end] == 1) && (alpha_mask[b_idx*L*L + gap_end*L + split] == 1)){
        ...
        if((alpha_d_mask[b_idx*L*L*L*L + start*L*L*L + gap_start*L*L + gap_end*L + split] == 1) && (alpha_mask[b_idx*L*L + split*L + end] == 1)){
                                ...
                                const int B, const int L, const int width, const int d, const int r)
{
    ...
    for(int split=start+1; split< gap_start; split++)
    {
       ...
       if((alpha_d_mask[b_idx*L*L*L*L + start*L*L*L + split*L*L + gap_end*L + end] == 1) && (alpha_mask[b_idx*L*L + split*L + gap_start] == 1)){
        float tmp = exp(s_d[start*L3 + split*L2 + gap_end*L1 + end*r + s_R] + rule_score + s[split*L*r*4 + gap_start*r*4 + 1*r + s_R] - parent_inside) * parent_grd;
        rule_grd += tmp;
    ...
    for(int split=gap_end+1; split <end; split++){
        ...
        if((alpha_d_mask[b_idx*L*L*L*L + start*L*L*L + gap_start*L*L + gap_end*L + split] == 1) && (alpha_mask[b_idx*L*L + split*L + end] == 1)){
            float tmp = exp(s_d[start*L3 + gap_start*L2 + gap_end*L1 + split*r + s_R] + rule_score + s[split*L*r*4 + end*r*4 + 3*r + s_R] - parent_inside) * parent_grd;
            rule_grd+=tmp;
                                    ...
                                    int B,   int L,  int m, int p,   int r)
{
    ...
    __syncthreads();
    if(s_B==0){
    float final_result1 = -999999999;
    float final_result2 = -999999999;
    float final_result3 = -999999999;
    float final_result4 = -999999999;
    for(int i=0;i<p;i++){
...
void cuda_forward( float *head_c1,   float *head_c2,   float *head_d1,   float *head_d2,
                   ...
                   float *alpha, float *alpha_d, float *alpha_mask, float *alpha_d_mask,
                   ...
                   int B, int L, int m, int p, int d, int r1, int r2, int r3, int r4)
{
    ...
    for(int w=2; w<L; w++){
      ...
      dim3 blockDim(r1);
      kernel_forward<<<gridDim, blockDim>>>(head_c1, alpha, alpha_mask, alpha_lc, alpha_rc, B, L, w, m, r1);

      if (w>2){
        dim3 gridDim2(B, L-w, m);
        dim3 blockDim2(r2);
        kernel_forward_close<<<gridDim2, blockDim2>>>(head_c2, alpha, alpha_mask, alpha_d_mask, alpha_cd, alpha_cc, B, L, w, m, r2);
      }

      if(w<L-1){
        dim3 gridDim6(B, L-w, r1);
        dim3 blockDim6(m);
        kernel_forward_cmr<<<gridDim6, blockDim6>>>(left_c, alpha, alpha_mask, alpha_lc, B, L, w, m, p, r1);
        kernel_forward_cmr<<<gridDim6, blockDim6>>>(right_c, alpha, alpha_mask, alpha_rc, B, L, w, m, p, r1);

        dim3 gridDim7(B, L-w, r3);
        kernel_forward_cmr<<<gridDim7, blockDim6>>>(left_d, alpha, alpha_mask, alpha_ld,  B, L, w, m, p, r3);
        kernel_forward_cmr<<<gridDim7, blockDim6>>>(right_d, alpha, alpha_mask, alpha_rd,  B, L, w, m, p, r3);

        dim3 gridDim8(B, L-w, r2);
        dim3 blockDim8(m);
        kernel_forward_cmr<<<gridDim8, blockDim8>>>(cc, alpha, alpha_mask, alpha_cc, B, L, w, m, p, r2);

        dim3 gridDim9(B, L-w, r4);
        dim3 blockDim9(m);
        kernel_forward_dmr<<<gridDim9, blockDim9>>>(dc, alpha_dc, alpha, alpha_mask, B, L, w, m, p, r4);

        dim3 gridDim3(B*(L-w),  (w-1)*(L-w), d);
        dim3 blockDim3(r3);
        kernel_forward_d1<<<gridDim3, blockDim3>>>(head_d1, alpha_ld, alpha_rd, alpha_d, alpha_d_mask, alpha_mask, B, L, w, d, r3);
        if(w>2){
            dim3 gridDim4(B*(L-w),  (w-1)*(L-w), d);
            dim3 blockDim4(r4);
            kernel_forward_d2<<<gridDim4, blockDim4>>>(head_d2, alpha_dc, alpha_dd, alpha_d, alpha_d_mask, alpha_mask, B, L, w, d, r4);
        }

        dim3 gridDim5(B*(L-w), (w-1)*(L-w), r4);
        dim3 blockDim5(d);
        kernel_forward_dr<<<gridDim5, blockDim5>>>(dd, alpha_d, alpha_d_mask, alpha_dd, B, L, w, d, r4);

        dim3 gridDim55(B*(L-w),  (w-1)*(L-w), r2);
        dim3 blockDim55(d);
        kernel_forward_dr<<<gridDim55, blockDim55>>>(cd, alpha_d, alpha_d_mask, alpha_cd, B, L, w, d, r2);
      }
    }
}
                   ...
                   int B, int L, int m, int p, int d, int r1, int r2, int r3, int r4)
{
     for(int w=L-1; w>1; w--){
        ...
        dim3 gridDim55(B*(L-w),  (w-1)*(L-w), r2);
        dim3 blockDim55(d);
        kernel_backward_dr<<<gridDim55, blockDim55>>>(cd, cd_grd, alpha_d, alpha_d_mask, alpha_cd, B, L, w, d, r2);

        dim3 gridDim5(B*(L-w), (w-1)*(L-w), r4);
        dim3 blockDim5(d);
        kernel_backward_dr<<<gridDim5, blockDim5>>>(dd, dd_grd, alpha_d, alpha_d_mask, alpha_dd, B, L, w, d, r4);

        if(w>2){
            dim3 gridDim4(B*(L-w),  (w-1)*(L-w), d);
            dim3 blockDim4(r4);
            kernel_backward_d2<<<gridDim4, blockDim4>>>(head_d2, head_d2_grd, alpha_dc, alpha_dd, alpha_d, alpha_d_mask, alpha_mask, B, L, w, d, r4);
        }
        dim3 gridDim3(B*(L-w),  (w-1)*(L-w), d);
        dim3 blockDim3(r3);
        kernel_backward_d1<<<gridDim3, blockDim3>>>(head_d1, head_d1_grd, alpha_ld, alpha_rd, alpha_d, alpha_d_mask, alpha_mask, B, L, w, d, r3);

        dim3 gridDim9(B, L-w, r4);
        dim3 blockDim9(m);
        kernel_backward_dmr<<<gridDim9, blockDim9>>>(dc, dc_grd, alpha_dc, alpha, alpha_mask, B, L, w, m, p, r4);

        dim3 gridDim8(B, L-w, r2);
        dim3 blockDim8(m);
        kernel_backward_cmr<<<gridDim8, blockDim8>>>(cc, cc_grd, alpha, alpha_mask, alpha_cc, B, L, w, m, p, r2);

        dim3 gridDim7(B, L-w, r3);
        dim3 blockDim7(m);
        kernel_backward_cmr<<<gridDim7, blockDim7>>>(left_d, left_d_grd, alpha, alpha_mask, alpha_ld,  B, L, w, m, p, r3);
        kernel_backward_cmr<<<gridDim7, blockDim7>>>(right_d, right_d_grd, alpha, alpha_mask, alpha_rd,  B, L, w, m, p, r3);

        dim3 gridDim6(B, L-w, r1);
        dim3 blockDim6(m);
        kernel_backward_cmr<<<gridDim6, blockDim6>>>(left_c, left_c_grd, alpha, alpha_mask, alpha_lc, B, L, w, m, p, r1);
        kernel_backward_cmr<<<gridDim6, blockDim6>>>(right_c, right_c_grd, alpha, alpha_mask, alpha_rc, B, L, w, m, p, r1);
     }

    if (w>2){
        dim3 gridDim2(B, L-w, m);
        dim3 blockDim2(r2);
        kernel_backward_close<<<gridDim2, blockDim2>>>(head_c2, head_c2_grd, alpha, alpha_mask, alpha_d_mask, alpha_cd, alpha_cc, B, L, w, m, r2);
    }

    dim3 gridDim(B, L-w, m);
    dim3 blockDim(r1);
    kernel_backward<<<gridDim, blockDim>>>(head_c1, head_c1_grd, alpha, alpha_mask, alpha_lc, alpha_rc, B, L, w, m, r1);
    }
