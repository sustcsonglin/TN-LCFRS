...
#
#
inside_cuda = load(name="adsaas",
                   sources=["/public/home/yangsl/code/TN-PCFG-main/parser/pcfgs/lcfrs_on5_rank_all/lcfrs_cpd.cpp", "/public/home/yangsl/code/TN-PCFG-main/parser/pcfgs/lcfrs_on5_rank_all/lcfrs_cpd.cu"],
                   verbose=True)
...
class InsideCPD_rankspace(torch.autograd.Function):
    ...
    def forward(ctx, head_cl1, head_cr1,
        ...
        alpha_dd = unary_l.new_zeros(B, L, L, L, L, 4, r4).fill_(0).contiguous()
...
class PCFG(PCFG_base):
    ...
    def decode(self, rules, lens, raw_word, viterbi=False, mbr=False):
        ...
        marginal_d = alpha_cd.transpose(1,2).sum(-1) + alpha_dd.transpose(1, 2).sum([-1, -2])
        ...
        def backtrack(b_idx, start, gap_start, gap_end, end, root=False):
            ...
            if gap_start == -1:
                ...
                # 说明这个continuous span是由两个小的continuous span组成而来的.
                    ...
                    split = idx
                    assert start<split<end, f"({start}, {split}, {end}, {alpha_c_mbr})"
