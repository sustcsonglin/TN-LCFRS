...
#
# try:
#     inside_cuda = load(name="inside_on5_ill",
#                    sources=["/public/home/yangsl/code/TN-PCFG-main/parser/pcfgs/lcfrs_on5_cuda/lcfrs_ill.cpp", "/public/home/yangsl/code/TN-PCFG-main/parser/pcfgs/lcfrs_on5_cuda/lcfrs_ill.cu"],
#                    verbose=True)
#
# except:
#     inside_cuda = load(name="inside_on5_ill",
#                        sources=["/home/yangsl/code/TN-PCFG-main/parser/pcfgs/lcfrs_on5_cuda/lcfrs_ill.cpp", "/home/yangsl/code/TN-PCFG-main/parser/pcfgs/lcfrs_on5_cuda/lcfrs_ill.cu"],
#                        verbose=True)


class InsideDiscoON5Ill(torch.autograd.Function):
    @staticmethod
    def forward(ctx, binary, binary_closed, binary_dc, binary_d, binary_ill, unary,root, lens):
        B, L, p = unary.shape
        m = binary.shape[-1] - p
        d = binary_d.shape[-3]
        L+=1
        ...
        inside_cuda.forward(binary, binary_closed, binary_dc, binary_d, binary_ill, unary, alpha, alpha_d, B, L, m, p, d)
        to_return1 = (alpha[torch.arange(B), 0, lens] + root)
        # to_return1 = alpha[:, 0, -1] + root
        to_return2 = to_return1.logsumexp(-1)
        ctx.save_for_backward(unary, binary, binary_closed, binary_dc, binary_d, binary_ill, root, lens, alpha, alpha_d, to_return1, to_return2)
        return to_return2

    def backward(ctx, gwk):
        unary, binary, binary_closed, binary_dc, binary_d, binary_ill, root, lens, alpha, alpha_d, t1, t2 = ctx.saved_tensors
        ...
        g_binary_ill = binary_ill.new_zeros(*binary_ill.shape)
        ...
        d = binary_d.shape[-3]
        L+=1
        inside_cuda.backward(binary, binary_closed, binary_dc, binary_d, binary_ill, unary,
                             g_binary, g_binary_closed, g_binary_dc, g_binary_d, g_binary_ill, g_unary,
                             alpha, alpha_d, B, L, m, p, d)
        return  g_binary, g_binary_closed, g_binary_dc, g_binary_d, g_binary_ill, g_unary, gwk.unsqueeze(-1) * gradient_root, None
...
class PCFG(PCFG_base):
    ...
    def _inside(self, rules, lens, viterbi=False, mbr=False):
        ...
        binary_ill = rules['binary_ill']
        root = rules['root']
        return {'partition': InsideDiscoON5Ill.apply(binary, binary_closed, binary_dc, binary_d, binary_ill, unary,root, lens)}
    ...
    def decode(self, rules, lens, viterbi=False, mbr=False):
        ...
        alpha_d = unary.new_zeros(B, L, L, L, L, d).fill_(0)
        with torch.no_grad():
            partition = InsideDiscoON5Ill.apply(binary, binary_closed, binary_dc, binary_d, binary_ill, unary, root, lens)

        inside_cuda.argmax(binary, binary_closed, binary_dc, binary_d, binary_ill, unary, alpha, alpha_d, B, L, m, p, d)
        ...
        def backtrack(b_idx, start, gap_start, gap_end, end, symbol):
            ...
            if gap_start == -1:
                ...
                # 说明这个continuous span是由两个小的continuous span组成而来的.
                    ...
                    if idx > 10000000:
                        idx -= 10000000
                        split1 = int(idx / (L * L * d * d))
                        split2 = (int(idx / (L * d * d))) % L
                        split3 = (int(idx / (d*d))) % L
                        assert start < split1 < split2 < split3 < end
                        left_symbol = (int(idx/d)) % d
                        right_symbol = idx % d
                        backtrack(b_idx, start, split1, split2, split3, left_symbol)
                        backtrack(b_idx, split1, split2, split3, end, right_symbol)
