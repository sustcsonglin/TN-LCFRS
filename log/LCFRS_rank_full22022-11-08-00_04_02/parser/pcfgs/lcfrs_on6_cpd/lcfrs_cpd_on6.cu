...
template <typename F>
__global__ void kernel_forward_d3(F *__restrict__  head,
                                F *__restrict__ alpha_dl,
                                F *__restrict__ alpha_dr,
                                F *__restrict__ alpha_d,
                                int B,  int L,  int width,   int d,   int r)
{
    ...
    F *__restrict__   s_l = alpha_dl + b_idx * L * L * L * L * r;
    F *__restrict__   s_r = alpha_dr + b_idx * L * L * L * L * r;
    int L3 = L * L * L * r;
    int L2 = L * L * r;
    int L1 = L * r;
    float tmp_result = logf(0);
    ...
    for(int split=start+1; split< gap_start; split++)
    {
        ...
        {
            tmp_result = logsumexp(tmp_result, s_l[start*L3 + split*L2 + split2*L1 + end*r + s_R] + rule_score + s_r[split*L3 + gap_start*L2 + gap_end*L + split2*r + s_R]);
        }
    }
    result[s_R] = tmp_result;
    __syncthreads();
...
template <typename F>
__global__ void kernel_backward_d3(const F *__restrict__ const head_rule,
                                 F *__restrict__  head_rule_grd,
                                F *__restrict__ alpha_dl,
                                F *__restrict__ alpha_dr,
                                F *__restrict__ alpha_d,
                                const int B, const int L, const int width, const int d, const int r)
{
    ...
    for(int split=start+1; split< gap_start; split++)
    {
        for(int split2 = gap_end+1; split2<end; split2++)
        {
            float tmp = exp(s_l[start*L3 + split*L2 + split2*L1 + end*r + s_R] + rule_score + s_r[split*L3 + gap_start*L2 + gap_end*L + split2*r + s_R] - parent_inside)*parent_grd;
            atomicAdd(s_l + split*L3 + start*L2 + split2*L1 + end*r + s_R, tmp);
            atomicAdd(s_r + gap_start*L3 + split*L2 + gap_end*L + split2*r + s_R, tmp);
            rule_grd += tmp;
        }
    }
...
void cuda_forward( float *head_c1,   float *head_c2,  float *head_d1,  float *head_d2,  float *head_d3,
                   float *left_c,   float *right_c,
                   float *left_d,   float *right_d,
                   float *cc,   float *cd,   float *dc,   float *dd, float *ddl, float *ddr,
                   float *unary,
                   float *alpha,  float *alpha_d, float *alpha_lc, float *alpha_rc, float *alpha_ld, float *alpha_rd,
                   float *alpha_cc, float *alpha_cd, float *alpha_dc, float *alpha_dd, float *alpha_ddl, float *alpha_ddr,
                   int B, int L, int m, int p, int d, int r1, int r2, int r3, int r4, int r5)
{
    ...
    for(int w=2; w<L; w++){
      ...
      if(w<L-1){
        ...
        if(w>=4){
            dim3 gridDim55(B*(L-w),  (w-1)*(L-w), d);
            dim3 blockDim55(r5);
            kernel_forward_d3<<<gridDim55, blockDim55>>>(head_d3, alpha_ddl, alpha_ddr, alpha_d, B, L, w, d, r5);
        }
        ...
        dim3 gridDim56(B*(L-w),  (w-1)*(L-w), r5);
        dim3 blockDim56(d);
        kernel_forward_dr<<<gridDim56, blockDim56>>>(ddl, alpha_d, alpha_ddl, B, L, w, d, r5);
        kernel_forward_dr<<<gridDim56, blockDim56>>>(ddr, alpha_d, alpha_ddr, B, L, w, d, r5);
...
void cuda_backward( float *head_c1,   float *head_c2,   float *head_d1,   float *head_d2, float *head_d3,
                   float *left_c,   float *right_c,
                   float *left_d,   float *right_d,
                   float *cc,   float *cd,   float *dc,   float *dd,  float *ddl, float *ddr, float *unary,
                   float *head_c1_grd,   float *head_c2_grd,   float *head_d1_grd,   float *head_d2_grd, float *head_d3_grd,
                   float *left_c_grd,   float *right_c_grd,
                   float *left_d_grd,   float *right_d_grd,
                   float *cc_grd,   float *cd_grd,   float *dc_grd,   float *dd_grd, float *ddl_grd, float *ddr_grd,   float *unary_grd,
                   float *alpha,  float *alpha_d, float *alpha_lc, float *alpha_rc, float *alpha_ld, float *alpha_rd,
                   float *alpha_cc, float *alpha_cd, float *alpha_dc, float *alpha_dd, float *alpha_ddl, float *alpha_ddr,
                  int B, int L, int m, int p, int d, int r1, int r2, int r3, int r4, int r5)

{
     for(int w=L-1; w>1; w--){
        ...
        dim3 gridDim56(B*(L-w), (w-1)*(L-w), r5);
        dim3 blockDim56(d);
        kernel_backward_dr<<<gridDim56, blockDim56>>>(ddl, ddl_grd, alpha_d, alpha_ddl, B, L, w, d, r5);
        kernel_backward_dr<<<gridDim56, blockDim56>>>(ddr, ddr_grd, alpha_d, alpha_ddr, B, L, w, d, r5);

        if(w>=4){
            dim3 gridDim44(B*(L-w), (w-1)*(L-w), d);
            dim3 blockDim44(r5);
            kernel_backward_d3<<<gridDim44, blockDim44>>>(head_d3, head_d3_grd, alpha_ddl, alpha_ddr, alpha_d, B, L, w, d, r5);
        }
